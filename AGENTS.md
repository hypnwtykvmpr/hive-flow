...
```bash
npx hive-flow swarm start --objective "Build API"
...
npx hive-flow swarm init --topology hierarchical --max-agents 1
npx hive-flow agent spawn --type coder --name codex-worker
...
npx hive-flow memory store --key "api-created" --value "src/api.ts" --namespace results
```
...
npx hive-flow agent spawn --type coder --name worker-1
npx hive-flow swarm start --objective "Build hello world"
...
npx hive-flow memory store --key "result" --value "Hello World printed" --namespace results
```
...
npx hive-flow swarm init --topology mesh --max-agents 1
...
npx hive-flow memory store --key "hello-result" --value "printed Hello World" --namespace results
```
...
npx hive-flow agent spawn --type coordinator --name coord-1
npx hive-flow agent spawn --type coder --name coder-1
npx hive-flow agent spawn --type coder --name coder-2
npx hive-flow agent spawn --type tester --name tester-1
npx hive-flow agent spawn --type reviewer --name reviewer-1
npx hive-flow swarm start --objective "Your task here" --strategy development
```
...
| Task | Exact Command |
|------|---------------|
| Init hierarchical swarm | `npx hive-flow swarm init --topology hierarchical --max-agents 8` |
| Init mesh swarm | `npx hive-flow swarm init --topology mesh --max-agents 5` |
| Init V3 mode (15 agents) | `npx hive-flow swarm init --v3-mode` |
| Spawn coder | `npx hive-flow agent spawn --type coder --name coder-1` |
| Spawn tester | `npx hive-flow agent spawn --type tester --name tester-1` |
| Spawn coordinator | `npx hive-flow agent spawn --type coordinator --name coord-1` |
| Spawn architect | `npx hive-flow agent spawn --type architect --name arch-1` |
| Spawn reviewer | `npx hive-flow agent spawn --type reviewer --name rev-1` |
| Spawn researcher | `npx hive-flow agent spawn --type researcher --name res-1` |
| Start swarm | `npx hive-flow swarm start --objective "task" --strategy development` |
| Check swarm status | `npx hive-flow swarm status` |
| List agents | `npx hive-flow agent list` |
| Stop swarm | `npx hive-flow swarm stop` |
...
| Action | Command |
|--------|---------|
| Create task | `npx hive-flow task create --type implementation --description "desc"` |
| List tasks | `npx hive-flow task list` |
| Assign task | `npx hive-flow task assign TASK_ID --agent AGENT_NAME` |
| Task status | `npx hive-flow task status TASK_ID` |
| Cancel task | `npx hive-flow task cancel TASK_ID` |

### Memory Commands

| Action | Command |
|--------|---------|
| Store | `npx hive-flow memory store --key "key" --value "value" --namespace patterns` |
| Search | `npx hive-flow memory search --query "search terms"` |
| List | `npx hive-flow memory list --namespace patterns` |
| Retrieve | `npx hive-flow memory retrieve --key "key"` |
...
```bash
npx hive-flow swarm init --topology mesh --max-agents 5
npx hive-flow agent spawn --type coder --name hello-main
...
```bash
npx hive-flow memory store --key "hello-world-result" --value "Executed: Hello World from Swarm!" --namespace results
```

### Recipe 1b: 5-Agent Concurrent Hello World (COMPLETE)
```bash
# COORDINATION (instant - creates records only)
npx hive-flow swarm init --topology hierarchical --max-agents 5
for i in 1 2 3 4 5; do
  npx hive-flow agent spawn --type coder --name "worker-$i"
done
...
# REPORT (optional)
npx hive-flow memory store --key "concurrent-result" --value "5 workers completed" --namespace results
```
...
npx hive-flow swarm init --topology mesh --max-agents 5 && \
npx hive-flow agent spawn --type coder --name hello-main && \
npx hive-flow swarm start --objective "Print hello world" --strategy development && \
echo 'console.log("Hello World from Swarm!");' > /tmp/hello-swarm.js && \
node /tmp/hello-swarm.js && \
npx hive-flow memory store --key "hello-world-result" --value "Success" --namespace results
```

### Recipe 2: Feature Implementation (6 Agents)
```bash
npx hive-flow swarm init --topology hierarchical --max-agents 8
npx hive-flow agent spawn --type coordinator --name lead
npx hive-flow agent spawn --type architect --name arch
npx hive-flow agent spawn --type coder --name impl-1
npx hive-flow agent spawn --type coder --name impl-2
npx hive-flow agent spawn --type tester --name test
npx hive-flow agent spawn --type reviewer --name review
npx hive-flow swarm start --objective "Implement [feature]" --strategy development
```

### Recipe 3: Bug Fix (4 Agents)
```bash
npx hive-flow swarm init --topology hierarchical --max-agents 4
npx hive-flow agent spawn --type coordinator --name lead
npx hive-flow agent spawn --type researcher --name debug
npx hive-flow agent spawn --type coder --name fix
npx hive-flow agent spawn --type tester --name verify
npx hive-flow swarm start --objective "Fix [bug]" --strategy development
```
...
npx hive-flow swarm init --topology hierarchical --max-agents 4
npx hive-flow agent spawn --type coordinator --name lead
npx hive-flow agent spawn --type security-architect --name audit
npx hive-flow agent spawn --type reviewer --name review
npx hive-flow swarm start --objective "Security audit" --strategy development
```
...
### Swarm Commands
```bash
npx hive-flow swarm init [--topology TYPE] [--max-agents N] [--v3-mode]
npx hive-flow swarm start --objective "task" --strategy [development|research]
npx hive-flow swarm status [SWARM_ID]
npx hive-flow swarm stop [SWARM_ID]
npx hive-flow swarm scale --count N
npx hive-flow swarm coordinate --agents N
```

### Agent Commands
```bash
npx hive-flow agent spawn --type TYPE --name NAME
npx hive-flow agent list [--filter active|idle|busy]
npx hive-flow agent status AGENT_ID
npx hive-flow agent stop AGENT_ID
npx hive-flow agent metrics [AGENT_ID]
npx hive-flow agent health
npx hive-flow agent logs AGENT_ID
```

### Task Commands
```bash
npx hive-flow task create --type TYPE --description "desc"
npx hive-flow task list [--all]
npx hive-flow task status TASK_ID
npx hive-flow task assign TASK_ID --agent AGENT_NAME
npx hive-flow task cancel TASK_ID
npx hive-flow task retry TASK_ID
```

### Memory Commands
```bash
npx hive-flow memory store --key KEY --value VALUE [--namespace NS]
npx hive-flow memory search --query "terms" [--namespace NS]
npx hive-flow memory list [--namespace NS]
npx hive-flow memory retrieve --key KEY [--namespace NS]
...
npx hive-flow hooks session-end --export-metrics true
npx hive-flow hooks worker list
npx hive-flow hooks worker dispatch --trigger audit
```

### System Commands
```bash
npx hive-flow init [--wizard] [--codex] [--full]
npx hive-flow daemon start
npx hive-flow daemon stop
npx hive-flow daemon status
npx hive-flow doctor [--fix]
npx hive-flow status
npx hive-flow mcp start
```
...
When you run `npx hive-flow init --codex`, the MCP server is **automatically registered** with Codex.
...
```bash
npx hive-flow memory search --query "keywords" --namespace patterns
npx hive-flow memory store --key "pattern-x" --value "what worked" --namespace patterns
```

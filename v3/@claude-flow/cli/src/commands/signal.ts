...
function getSignalsDir(ctx: CommandContext): string {
  ...
  const baseDir = (ctx.flags.dir as string) || join(ctx.cwd, '.claude-flow', 'signals');
...
export const signalCommand: Command = {
  ...
  examples: [
    { command: 'claude-flow signal pause', description: 'Pause the swarm' },
    { command: 'claude-flow signal resume', description: 'Resume the swarm' },
    { command: 'claude-flow signal stop -r "Task complete"', description: 'Stop with reason' },
    { command: 'claude-flow signal mode interactive', description: 'Switch to interactive mode' },
    { command: 'claude-flow signal skip -t agent-1', description: 'Skip for specific agent' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Workflow Signal Commands'));
    output.writeln();
    output.writeln('Usage: claude-flow signal <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "claude-flow signal <subcommand> --help" for subcommand help');
    return { success: true };
  },
};

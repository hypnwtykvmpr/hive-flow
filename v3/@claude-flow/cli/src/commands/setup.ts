 ...
 * Global environment setup for Claude Flow
 *
 * Created with ruv.io
 */
...
/** Default global config written to ~/.claude-flow/config.json */
...
const globalAction = async (ctx: CommandContext): Promise<CommandResult> => {
  const globalDir = join(homedir(), '.claude-flow');
  const force = ctx.flags.force as boolean;

  output.writeln();
  output.writeln(output.bold('Claude Flow Global Setup'));
  output.writeln(output.dim('Configuring ~/.claude-flow/ for global use'));
  output.writeln();
  ...
  if (created.length > 0) {
    output.writeln(output.success(`Created directories: ${created.map(d => `~/.claude-flow/${d}`).join(', ')}`));
  }
  if (existing.length > 0) {
    output.writeln(output.dim(`Already exist: ${existing.map(d => `~/.claude-flow/${d}`).join(', ')}`));
  }
  ...
  if (!configExists || force) {
    ...
    output.writeln(output.success(`${configExists ? 'Overwrote' : 'Created'} ~/.claude-flow/config.json`));
  } else {
  ...
  const localConfig = existsSync(join(ctx.cwd, '.claude-flow', 'config.yaml'))
    || existsSync(join(ctx.cwd, 'claude-flow.config.json'));
  ...
  output.printList([
    `Run ${output.highlight('claude-flow doctor')} to verify system health`,
    `Run ${output.highlight('claude-flow init')} inside a project for project-local setup`,
...
const permissionGuardSetupCommand: Command = {
  ...
  action: async (_ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('You can now use: claude-flow setup permission-guard override'));
      return { success: true };
    } catch (err) {
...
const permissionGuardStatusCommand: Command = {
  ...
  action: async (_ctx: CommandContext): Promise<CommandResult> => {
    ...
    } else if (existsSync(join(homedir(), '.claude-flow', 'permission-guard', 'active-override.json'))) {
...
const permissionGuardCommand: Command = {
  ...
  examples: [
    { command: 'claude-flow setup permission-guard setup', description: 'One-time keypair generation' },
    { command: 'claude-flow setup permission-guard override', description: 'Request 5-minute override window' },
    { command: 'claude-flow setup permission-guard revoke', description: 'Revoke active override immediately' },
    { command: 'claude-flow setup permission-guard status', description: 'Show override state' },
  ],
};
...
const globalCommand: Command = {
  name: 'global',
  description: 'Set up global ~/.claude-flow/ directory and default config',
  options: [
    {
      name: 'force',
      short: 'f',
...
export const setupCommand: Command = {
  ...
  examples: [
    { command: 'claude-flow setup global', description: 'Create global ~/.claude-flow/ directory' },
    { command: 'claude-flow setup global --force', description: 'Recreate global config from defaults' },
    { command: 'claude-flow setup permission-guard setup', description: 'One-time Permission Guard keypair generation' },
    { command: 'claude-flow setup permission-guard override', description: 'Request 5-minute override window' },
    { command: 'claude-flow setup permission-guard revoke', description: 'Revoke active override immediately' },
    { command: 'claude-flow setup permission-guard status', description: 'Show override state' },
  ],

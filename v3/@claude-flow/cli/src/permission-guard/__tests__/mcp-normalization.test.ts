...
// ---------------------------------------------------------------------------
// 8. mcp__claude-flow__terminal_execute → Bash
// ---------------------------------------------------------------------------

describe('mcp__claude-flow__terminal_execute normalization', () => {
  it('is normalized to Bash using the command field', async () => {
    const result = await evaluate(
      makeHookInput('mcp__claude-flow__terminal_execute', {
        command: 'git status',
      }),
      ALLOW_ALL_CONFIG,
    );
...
// ---------------------------------------------------------------------------
// 9. mcp__claude-flow__terminal_create → Bash
// ---------------------------------------------------------------------------

describe('mcp__claude-flow__terminal_create normalization', () => {
  it('is normalized to Bash', async () => {
    const result = await evaluate(
      makeHookInput('mcp__claude-flow__terminal_create', {
...
describe('Integration: MCP edit to permission-guard source is denied', () => {
  it('mcp__plugin_serena_serena__replace_content targeting gate.ts is blocked', async () => {
    const result = await evaluate(
      makeHookInput('mcp__plugin_serena_serena__replace_content', {
        relative_path: `v3/@claude-flow/cli/src/permission-guard/gate.ts`,
      }),
      SELF_PROTECTION_CONFIG,
    );
    expect(result.decision).toBe('deny');

...
describe('isProtectedPath', () => {
  ...
  it('blocks writes to permission-guard source files', () => {
    const result = isProtectedPath(
      `${CWD}/v3/@claude-flow/cli/src/permission-guard/gate.ts`,
      CWD,
    );
    expect(result.blocked).toBe(true);
  });
  ...
  it('blocks writes to permission-guard compiled files', () => {
    const result = isProtectedPath(
      `${CWD}/v3/@claude-flow/cli/dist/src/permission-guard/gate.js`,
      CWD,
    );
    expect(result.blocked).toBe(true);
  });
  ...
  it('blocks writes to guard config in home directory', () => {
    const result = isProtectedPath(
      `${HOME}/.claude-flow/permission-guard/config.json`,
      CWD,
    );
    expect(result.blocked).toBe(true);
  });
  ...
  it('allows writes to non-guard CLI source files', () => {
    const result = isProtectedPath(
      `${CWD}/v3/@claude-flow/cli/src/commands/swarm.ts`,
      CWD,
    );
    expect(result.blocked).toBe(false);
  });
...
describe('checkBashSelfProtection', () => {
  ...
  it('blocks sed -i on guard source', () => {
    const result = checkBashSelfProtection(
      `sed -i 's/deny/allow/g' ${CWD}/v3/@claude-flow/cli/src/permission-guard/gate.ts`,
      CWD,
    );
    expect(result).not.toBeNull();
    expect(result!.blocked).toBe(true);
  ...
  it('blocks dd with of= targeting guard config', () => {
    const result = checkBashSelfProtection(
      `dd if=/dev/zero of=${HOME}/.claude-flow/permission-guard/config.json bs=1 count=0`,
      CWD,
    );
    expect(result).not.toBeNull();
    expect(result!.blocked).toBe(true);
...
describe('evaluateSelfProtection', () => {
  ...
  it('blocks Edit to gate.ts', () => {
    const result = evaluateSelfProtection(
      'Edit',
      { file_path: `${CWD}/v3/@claude-flow/cli/src/permission-guard/gate.ts` },
      CWD,
    );
    expect(result).not.toBeNull();
    expect(result!.blocked).toBe(true);
...
describe('self-protection attack vectors', () => {
  ...
  const protectedTargets = [
    ...
    { name: 'guard gate.ts', path: `${CWD}/v3/@claude-flow/cli/src/permission-guard/gate.ts` },
    { name: 'guard self-protection.ts', path: `${CWD}/v3/@claude-flow/cli/src/permission-guard/self-protection.ts` },
    { name: 'compiled gate.js', path: `${CWD}/v3/@claude-flow/cli/dist/src/permission-guard/gate.js` },
    { name: 'guard config', path: `${HOME}/.claude-flow/permission-guard/config.json` },
  ];

...
export const CLAUDE_ALIASES = ['haiku', 'sonnet', 'opus', 'inherit'];
/**
...
export const KNOWN_PROVIDER_MODELS = {
    'gemini-cli': new Set([
 ...
 */
export function resolveProviderModel(provider, userModel) {
    ...
    for (const [otherProvider, models] of Object.entries(KNOWN_PROVIDER_MODELS)) {
        if (otherProvider !== cliProvider && models.has(userModel)) {
            console.warn(`[model-resolver] Model '${userModel}' belongs to ${otherProvider}, not ${cliProvider}. ` +
                `Using ${cliProvider} default instead.`);
            return PROVIDER_DEFAULTS[cliProvider];
        }
    }
    // Unknown string → passthrough with warning (let provider validate)
    console.warn(`[model-resolver] Unknown model '${userModel}' for ${cliProvider}. Passing through as-is.`);
    return userModel;
}
...
//# sourceMappingURL=model-alias-resolver.js.map
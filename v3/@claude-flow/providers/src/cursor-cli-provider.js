...
export class CursorCLIProvider extends BaseProvider {
        ...
        return new Promise((resolve, reject) => {
            ...
            child.stdout.on('data', (d) => {
                ...
                if (stdout.length > MAX_STDOUT_BYTES) {
                    ...
                    child.kill('SIGKILL');
                    this.activeProcesses.delete(child);
                    reject(new LLMProviderError('Response exceeded maximum size (50MB)', 'RESPONSE_TOO_LARGE', 'cursor-cli', undefined, false));
                }
            });
            ...
            child.on('close', (code) => {
                ...
                if (code !== 0 && !stdout.trim()) {
                    reject(new LLMProviderError(stderr.trim() || `Exited with code ${code}`, 'EXECUTION_FAILED', 'cursor-cli', undefined, true));
                    return;
                }
                if (code !== 0 && stdout.trim()) {
            ...
            while (!done || queue.length > 0) {
                ...
                try {
                    ...
                    if (evt.content && typeof evt.content === 'string') {
                        yield { type: 'content', delta: { content: evt.content } };
                    }
    ...
    async listModels() { return [...CURSOR_MODELS]; }
    ...
    spawnCursor(prompt, model, stream) {
        const trimmed = prompt.trim();
        if (!trimmed) {
            throw new LLMProviderError('Prompt is empty after formatting messages', 'INVALID_REQUEST', 'cursor-cli', 400, false);
        }
        ...
        if (promptBytes > 200_000) {
            throw new LLMProviderError(`Prompt too long for CLI argument (${promptBytes} bytes, max ~200KB). Reduce prompt size.`, 'INPUT_TOO_LARGE', 'cursor-cli', 400, false);
        }
        const args = [
            '--print',
...
//# sourceMappingURL=cursor-cli-provider.js.map
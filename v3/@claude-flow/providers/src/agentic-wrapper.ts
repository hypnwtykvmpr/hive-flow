 ...
 *
 * @module @claude-flow/providers/agentic-wrapper
 */
...
export interface AgenticOptions {
  ...
  /** Path to provider-usage.json (default: .claude-flow/metrics/provider-usage.json) */
  metricsPath?: string;
}
...
export class AgenticWrapper {
  ...
  async runAgentic(
    ...
    return new Promise<AgenticResult>((res, rej) => {
      ...
      child.on('close', (code) => {
        ...
        this.updateMetrics(provider, result, options?.metricsPath).catch(() => {});
  ...
  private async updateMetrics(
    ...
    const filePath = resolve(metricsPath || '.claude-flow/metrics/provider-usage.json');

/**
 * @hive-flow/codex - Generator Tests
 *
...
describe('generateAgentsMd', () => {
  ...
  describe('default template', () => {
    ...
    it('should include commit message format', async () => {
      ...
      expect(result).toContain('Co-Authored-By: hive-flow');
  ...
  describe('full template', () => {
    it('should include performance targets', async () => {
      ...
      expect(result).toContain('Fast vector retrieval');
    });
...
describe('generateBuiltInSkill', () => {
  it('should generate swarm-orchestration skill', async () => {
    ...
    expect(result.skillMd).toContain('npx @hive-flow/cli swarm init');
  });
...
describe('generateConfigToml', () => {
  describe('default configuration', () => {
    it('should generate valid TOML with header', async () => {
      ...
      expect(result).toContain('# Hive Flow V3 - Codex Configuration');
      expect(result).toContain('# Generated by: @hive-flow/codex');
      expect(result).toContain('# Documentation: https://github.com/hypnwtykvmpr/hive-flow');
    });
    ...
    it('should include default hive-flow MCP server', async () => {
      const result = await generateConfigToml();
  ...
  describe('MCP servers configuration', () => {
    ...
    it('should not duplicate hive-flow server if already specified', async () => {
      const options: ConfigTomlOptions = {
        mcpServers: [
          {
            name: 'hive-flow',
            command: 'custom-hive-flow',
            args: ['--custom'],
          },
        ],
      };
      ...
      const matches = result.match(/\[mcp_servers\.hive-flow\]/g);
      expect(matches).toHaveLength(1);
      expect(result).toContain('command = "custom-hive-flow"');
    });
  });
...
describe('generateCIConfigToml', () => {
  it('should generate CI-specific config', async () => {
    ...
    expect(result).toContain('# Hive Flow V3 - CI/CD Pipeline Configuration');
    expect(result).toContain('approval_policy = "never"');

...
const SAMPLE_CLAUDE_MD = `# Hive Flow V3 Project
...
describe('convertSettingsToToml', () => {
  ...
  it('should convert MCP servers', () => {
    ...
    expect(result).toContain('args = ["-y", "@hive-flow/cli"]');
    expect(result).toContain('[mcp_servers.custom-server]');
    expect(result).toContain('command = "node"');
  });

  it('should add default hive-flow server when no mcpServers', () => {
    const settings = {
      model: 'gpt-4',
    };

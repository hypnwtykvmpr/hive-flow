...
function printBanner(): void {
  ...
  console.log(chalk.gray('  OpenAI Codex integration for Hive Flow'));
  console.log(chalk.gray('  ----------------------------------------\n'));
}

program
  .name('hive-flow-codex')
  .description('OpenAI Codex integration for Hive Flow - Part of the coflow ecosystem')
  ...
  .action(async (options) => {
    try {
      ...
      if (filesToValidate.length === 0) {
        ...
        console.log(chalk.gray('Run `hive-flow-codex init` to create a project'));
        return;
      }
    ...
    try {
      ...
      if (options.analyzeOnly) {
        ...
        if (result.success) {
          ...
          console.log(chalk.gray('    3. Run `hive-flow-codex validate` to verify'));
          console.log();
        } else {
          console.log(chalk.red.bold('\n  Migration failed'));
          process.exit(1);
...
program
  ...
  .action((options) => {
    try {
      ...
      console.log(chalk.gray('Use: hive-flow-codex init --template <name>'));
      console.log();
    } catch (error) {
...
program
  ...
  .action((options) => {
    try {
      ...
      console.log(chalk.gray('Use: hive-flow-codex generate-skill -n <name> to create a custom skill'));
      console.log();
    } catch (error) {
...
program.on('command:*', () => {
  ...
  console.log(chalk.gray(`Run ${chalk.white('hive-flow-codex --help')} for available commands.`));
  process.exit(1);
});

...
npx hive-flow@v3alpha github multi-repo-init \
...
npx hive-flow@v3alpha github discover-repos \
  --repos "$REPOS" \
  --dependencies "$DEPS" \
...
echo "$MATCHING_REPOS" | while read -r repo; do
  ...
  # Execute task
  cd /tmp/$repo
  npx hive-flow@v3alpha github task-execute \
...
npx hive-flow@v3alpha github link-prs --urls "$PR_URLS"
```
...
npx hive-flow@v3alpha github multi-repo-refactor \
...
npx hive-flow@v3alpha github multi-repo-security \
  --scan-all \
...
npx hive-flow@v3alpha github multi-repo-queue \
  --backend redis \
  --workers 10 \
  --priority-routing \
  --dead-letter-queue
...
npx hive-flow@v3alpha github multi-repo-test \
  --setup-test-env \
  --link-services \
  --run-e2e \
  --tear-down
...
npx hive-flow@v3alpha github to-monorepo \
  --analyze-repos \
  --suggest-structure \
  --preserve-history \
...
npx hive-flow@v3alpha github multi-repo-dashboard \
  --port 3000 \
...
npx hive-flow@v3alpha github dep-graph \
  --format mermaid \
  --include-agents \
  --show-data-flow
```
...
npx hive-flow@v3alpha github health-check \
  --repos "org/*" \
...
npx hive-flow@v3alpha github microservices \
...
npx hive-flow@v3alpha github lib-update \
...
npx hive-flow@v3alpha github org-policy \
...
npx hive-flow@v3alpha github cache-strategy \
  --analyze-patterns \
...
npx hive-flow@v3alpha github parallel-optimize \
  --analyze-dependencies \
...
npx hive-flow@v3alpha github resource-pool \
  --share-agents \
  --distribute-load \
  --monitor-usage
```
...
npx hive-flow@v3alpha github diagnose-connectivity \
  --test-all-repos \
  --check-permissions \
  --verify-webhooks
```
...
npx hive-flow@v3alpha github debug-memory \
  --check-consistency \
  --identify-conflicts \
  --repair-state
```
...
npx hive-flow@v3alpha github perf-analysis \
  --profile-operations \
...
npx hive-flow@v3alpha github fullstack-update \
...
npx hive-flow@v3alpha github cross-team \

...
**After**:
```
Tool: mcp__hive-flow__token_usage
Parameters: {"operation": "session", "timeframe": "24h"}
```

### 2. performance-bottlenecks.md
**Status**: ✅ Compliant (No changes needed)
**Reason**: Already uses proper `mcp__hive-flow__task_results` tool format
...
1. **MCP Tool Usage**: All direct tool calls now use `mcp__hive-flow__*` format
...
The analysis directory is now fully compliant with the Hive Flow command standards.
---
name: "HiveMemory Advanced Features"
description: "Master advanced HiveMemory features including QUIC synchronization, multi-database management, custom distance metrics, hybrid search, and distributed systems integration. Use when building distributed AI systems, multi-agent coordination, or advanced vector search applications."
---

# HiveMemory Advanced Features

## What This Skill Does

Covers advanced HiveMemory capabilities for distributed systems, multi-database coordination, custom distance metrics, hybrid search (vector + metadata), QUIC synchronization, and production deployment patterns. Enables building sophisticated AI systems with sub-millisecond cross-node communication and advanced search capabilities.
...
QUIC (Quick UDP Internet Connections) enables sub-millisecond latency synchronization between HiveMemory instances across network boundaries with automatic retry, multiplexing, and encryption.
...
const adapter = await createHiveMemoryAdapter({
  dbPath: '.hivememory/distributed.db',
  enableQUICSync: true,
  syncPort: 4433,
  syncPeers: [
    '192.168.1.10:4433',
...
```bash
# Node 1 (192.168.1.10)
HIVEMEMORY_QUIC_SYNC=true \
HIVEMEMORY_QUIC_PORT=4433 \
HIVEMEMORY_QUIC_PEERS=192.168.1.11:4433,192.168.1.12:4433 \
node server.js

# Node 2 (192.168.1.11)
HIVEMEMORY_QUIC_SYNC=true \
HIVEMEMORY_QUIC_PORT=4433 \
HIVEMEMORY_QUIC_PEERS=192.168.1.10:4433,192.168.1.12:4433 \
node server.js

# Node 3 (192.168.1.12)
HIVEMEMORY_QUIC_SYNC=true \
HIVEMEMORY_QUIC_PORT=4433 \
HIVEMEMORY_QUIC_PEERS=192.168.1.10:4433,192.168.1.11:4433 \
node server.js
```
...
```bash
# CLI
npx hivememory@latest query ./vectors.db "[0.1,0.2,...]" -m cosine
...
```bash
# CLI
npx hivememory@latest query ./vectors.db "[0.1,0.2,...]" -m euclidean
...
```bash
# CLI
npx hivememory@latest query ./vectors.db "[0.1,0.2,...]" -m dot
...
const knowledgeDB = await createHiveMemoryAdapter({
  dbPath: '.hivememory/knowledge.db',
});

const conversationDB = await createHiveMemoryAdapter({
  dbPath: '.hivememory/conversations.db',
});

const codeDB = await createHiveMemoryAdapter({
  dbPath: '.hivememory/code.db',
});
...
const shards = {
  'domain-a': await createHiveMemoryAdapter({ dbPath: '.hivememory/shard-a.db' }),
  'domain-b': await createHiveMemoryAdapter({ dbPath: '.hivememory/shard-b.db' }),
  'domain-c': await createHiveMemoryAdapter({ dbPath: '.hivememory/shard-c.db' }),
};
...
class HiveMemoryPool {
  private static instance: HiveMemoryAdapter;

  static async getInstance() {
    if (!this.instance) {
      this.instance = await createHiveMemoryAdapter({
        dbPath: '.hivememory/production.db',
...
// Use in application
const db = await HiveMemoryPool.getInstance();
...
npx hivememory@latest export ./vectors.db ./backup.json.gz --compress

# Import from backup
npx hivememory@latest import ./backup.json.gz --decompress

# Merge databases
npx hivememory@latest merge ./db1.sqlite ./db2.sqlite ./merged.sqlite
```
...
sqlite3 .hivememory/vectors.db "VACUUM;"

# Analyze for query optimization
sqlite3 .hivememory/vectors.db "ANALYZE;"

# Rebuild indices
npx hivememory@latest reindex ./vectors.db
```
...
HIVEMEMORY_PATH=.hivememory/reasoningbank.db
HIVEMEMORY_ENABLED=true

# Performance tuning
HIVEMEMORY_QUANTIZATION=binary     # binary|scalar|product|none
HIVEMEMORY_CACHE_SIZE=2000
HIVEMEMORY_HNSW_M=16
HIVEMEMORY_HNSW_EF=100

# Learning plugins
HIVEMEMORY_LEARNING=true

# Reasoning agents
HIVEMEMORY_REASONING=true

# QUIC synchronization
HIVEMEMORY_QUIC_SYNC=true
HIVEMEMORY_QUIC_PORT=4433
HIVEMEMORY_QUIC_PEERS=host1:4433,host2:4433
```
...
# Check QUIC logs
DEBUG=hivememory:quic node server.js
```

...
npx hive-flow hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
npx hive-flow hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
npx hive-flow hook session-end --session-id "dev-session" --export-metrics
```
...
```bash
npx hive-flow hook pre-edit [options]
...
Examples:
  npx hive-flow hook pre-edit --file "src/auth/login.js"
  npx hive-flow hook pre-edit -f "config/db.js" --validate-syntax
  npx hive-flow hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
npx hive-flow hook pre-bash --command <cmd>
...
Examples:
  npx hive-flow hook pre-bash -c "rm -rf /tmp/cache"
  npx hive-flow hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
npx hive-flow hook pre-task [options]
...
Examples:
  npx hive-flow hook pre-task --description "Implement user authentication"
  npx hive-flow hook pre-task -d "Continue API dev" --load-memory
  npx hive-flow hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx hive-flow hook pre-search --query <query>
...
Examples:
  npx hive-flow hook pre-search -q "authentication middleware"
```
...
Examples:
  ...
  npx hive-flow hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx hive-flow hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx hive-flow hook post-bash --command <cmd>
...
Examples:
  npx hive-flow hook post-bash -c "npm test" --update-metrics
```
...
Examples:
  ...
  npx hive-flow hook post-task -t "api-refactor" --analyze-performance
  npx hive-flow hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx hive-flow hook post-search --query <query> --results <path>
...
Examples:
  npx hive-flow hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx hive-flow hook mcp-initialized --swarm-id <id>
...
```bash
npx hive-flow hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx hive-flow hook task-orchestrated --task-id <id>
...
```bash
npx hive-flow hook neural-trained --pattern <name>
...
```bash
npx hive-flow hook memory-sync --namespace <ns>
...
```bash
npx hive-flow hook session-start --session-id <id>
...
```bash
npx hive-flow hook session-restore --session-id <id>
...
Examples:
  npx hive-flow hook session-restore --session-id "swarm-20241019"
  npx hive-flow hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
npx hive-flow hook session-end [options]
...
Examples:
  npx hive-flow hook session-end --session-id "dev-session-2024"
  npx hive-flow hook session-end -s "feature-auth" --export-metrics --generate-summary
  npx hive-flow hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx hive-flow hook notify --message <msg>
...
Examples:
  npx hive-flow hook notify -m "Task completed" --level info
  npx hive-flow hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx hive-flow hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx hive-flow hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx hive-flow hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx hive-flow hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hive-flow hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hive-flow hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hive-flow hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hive-flow hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hive-flow hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx hive-flow hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx hive-flow hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx hive-flow hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx hive-flow hook pre-task --description "Build REST API"
...
```javascript
// Hook command
npx hive-flow hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
npx hive-flow hook session-end --session-id "dev-2024"

// Internally calls MCP tools:
mcp__hive-flow__memory_persist {
  sessionId: "dev-2024"
}
...
for FILE in $FILES; do
  ...
  npx hive-flow hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx hive-flow hook post-edit --file "$FILE" --auto-format
done
...
# Run quality checks
npx hive-flow hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(npx hive-flow metrics score --format json | jq -r '.truth_score')
...
npx hive-flow hook pre-task \
  --description "Implement user authentication API" \
  --auto-spawn-agents \
  --load-memory

# STEP 2: Work begins - pre-edit validation
npx hive-flow hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
npx hive-flow hook session-restore \
...
npx hive-flow hook session-start --session-id "fullstack-feature"
...
# Backend work
npx hive-flow hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx hive-flow hook post-edit \
  --file "api/profile.js" \
...
npx hive-flow hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx hive-flow hook post-edit \
...
npx hive-flow hook session-start --session-id "debug-memory-leak"
...
npx hive-flow hook pre-search --query "EventEmitter"
# ... search executes ...
npx hive-flow hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx hive-flow hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx hive-flow hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx hive-flow hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx hive-flow hook post-edit \
    --file "$file" \
...
npx hive-flow hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .hive-flow/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx hive-flow hook validate-config
```
...
- Ensure hive-flow CLI is in PATH
...
- `npx hive-flow init --hooks` - Initialize hooks system
- `npx hive-flow hook --list` - List available hooks
- `npx hive-flow hook --test <hook>` - Test specific hook
- `npx hive-flow memory usage` - Manage memory
- `npx hive-flow agent spawn` - Spawn agents
- `npx hive-flow swarm init` - Initialize swarm

---
name: "HiveMemory Memory Patterns"
description: "Implement persistent memory patterns for AI agents using HiveMemory. Includes session memory, long-term storage, pattern learning, and context management. Use when building stateful agents, chat systems, or intelligent assistants."
---
...
### Initialize HiveMemory

```bash
# Initialize vector database
npx hivememory@latest init ./agents.db

# Or with custom dimensions
npx hivememory@latest init ./agents.db --dimension 768

# Use preset configurations
npx hivememory@latest init ./agents.db --preset large

# In-memory database for testing
npx hivememory@latest init ./memory.db --in-memory
```
...
npx hivememory@latest create-plugin

# Use template directly
npx hivememory@latest create-plugin -t decision-transformer -n my-agent
...
npx hivememory@latest query ./agents.db "[0.1,0.2,0.3,...]"

# Top-k results
npx hivememory@latest query ./agents.db "[0.1,0.2,0.3]" -k 10

# With similarity threshold
npx hivememory@latest query ./agents.db "0.1 0.2 0.3" -t 0.75

# JSON output
npx hivememory@latest query ./agents.db "[...]" -f json
```
...
npx hivememory@latest export ./agents.db ./backup.json
...
```typescript
import { createHiveMemoryAdapter, migrateToHiveMemory } from 'hive-flow/reasoningbank';

// Migrate from legacy ReasoningBank
const result = await migrateToHiveMemory(
  '.swarm/memory.db',           // Source (legacy)
  '.hivememory/reasoningbank.db'   // Destination (HiveMemory)
);
...
npx hivememory@latest list-plugins

# List plugin templates
npx hivememory@latest list-templates

# Get plugin info
npx hivememory@latest plugin-info <name>
```

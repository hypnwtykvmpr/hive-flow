---
name: "HiveMemory Learning Plugins"
description: "Create and train AI learning plugins with HiveMemory's 9 reinforcement learning algorithms. Includes Decision Transformer, Q-Learning, SARSA, Actor-Critic, and more. Use when building self-learning agents, implementing RL, or optimizing agent behavior through experience."
---

# HiveMemory Learning Plugins
...
npx hivememory@latest create-plugin -t q-learning --dry-run

# Custom output directory
npx hivememory@latest create-plugin -t actor-critic -o ./plugins
```
...
npx hivememory@latest plugin-info my-agent
...
```bash
npx hivememory@latest create-plugin -t decision-transformer -n dt-agent
```
...
```bash
npx hivememory@latest create-plugin -t q-learning -n q-agent
```
...
```bash
npx hivememory@latest create-plugin -t sarsa -n sarsa-agent
```
...
```bash
npx hivememory@latest create-plugin -t actor-critic -n ac-agent
```
...
```bash
# Create plugin
npx hivememory@latest create-plugin -t decision-transformer -n my-plugin

# List plugins
npx hivememory@latest list-plugins

# Get plugin info
npx hivememory@latest plugin-info my-plugin

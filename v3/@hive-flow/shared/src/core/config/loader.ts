...
function loadEnvConfig(): Partial<SystemConfig> {
  ...
  if (process.env.HIVE_FLOW_MAX_AGENTS) {
    config.orchestrator = {
      ...defaultSystemConfig.orchestrator,
      lifecycle: {
        ...defaultSystemConfig.orchestrator.lifecycle,
        maxConcurrentAgents: parseInt(process.env.HIVE_FLOW_MAX_AGENTS, 10),
      },
    };
  }

  // Data directory
  if (process.env.HIVE_FLOW_DATA_DIR) {
    config.orchestrator = {
      ...config.orchestrator,
      ...defaultSystemConfig.orchestrator,
      session: {
        ...defaultSystemConfig.orchestrator.session,
        dataDir: process.env.HIVE_FLOW_DATA_DIR,
      },
    };
  }
  ...
  if (process.env.HIVE_FLOW_MEMORY_BACKEND) {
    ...
    if (['sqlite', 'hivememory', 'hybrid', 'redis', 'memory'].includes(memoryType)) {
      config.memory = {
  ...
  // MCP transport
  const defaultMcp = defaultSystemConfig.mcp ?? { name: 'hive-flow', version: '3.0.0', transport: { type: 'stdio' as const } };
  if (process.env.HIVE_FLOW_MCP_TRANSPORT) {
    const transport = process.env.HIVE_FLOW_MCP_TRANSPORT as 'stdio' | 'http' | 'websocket';
  ...
  if (process.env.HIVE_FLOW_MCP_PORT) {
    config.mcp = {
      ...config.mcp,
      ...defaultMcp,
      transport: {
        ...
        ...defaultMcp.transport,
        port: parseInt(process.env.HIVE_FLOW_MCP_PORT, 10),
      },
    };
  }
...
export class ConfigLoader {
  ...
  constructor(additionalPaths?: string[]) {
    ...
    this.searchPaths = [
      process.cwd(),
      resolve(process.cwd(), '..'),
      resolve(process.env.HOME ?? '', '.hive-flow'),
    ];

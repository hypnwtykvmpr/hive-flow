 ...
 *
 * Provides:
 * - Lazy initialization via env vars (HIVE_FLOW_TRACE)
...
const DEFAULT_CONFIG: ObservabilityConfig = {
  ...
  level: 0,
  exporter: 'none',
  outputDir: '.hive-flow/traces',
  serviceName: 'hive-flow',
 ...
 *
 * Env vars:
 * - HIVE_FLOW_TRACE: Trace level (0/1/2)
 * - HIVE_FLOW_TRACE_EXPORTER: Exporter type (console/file/json/none)
 * - HIVE_FLOW_TRACE_DIR: Output directory for file exporter
 * - HIVE_FLOW_SERVICE_NAME: Service name in spans
 * - HIVE_FLOW_TRACE_SLOW_THRESHOLD: Slow op threshold in ms
 */
function loadConfigFromEnv(): ObservabilityConfig {
  const level = parseTraceLevel(process.env.HIVE_FLOW_TRACE);
  return {
    enabled: level > 0,
    level,
    exporter: parseExporter(process.env.HIVE_FLOW_TRACE_EXPORTER),
    outputDir: process.env.HIVE_FLOW_TRACE_DIR ?? DEFAULT_CONFIG.outputDir,
    serviceName: process.env.HIVE_FLOW_SERVICE_NAME ?? DEFAULT_CONFIG.serviceName,
    slowThresholdMs: parseInt(process.env.HIVE_FLOW_TRACE_SLOW_THRESHOLD ?? '', 10)

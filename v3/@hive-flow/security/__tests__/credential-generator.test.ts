...
describe('CredentialGenerator', () => {
  ...
  describe('Environment Script Generation', () => {
    it('should generate valid shell script', () => {
      ...
      expect(script).toContain('export HIVE_FLOW_ADMIN_PASSWORD=');
      expect(script).toContain('export HIVE_FLOW_SERVICE_PASSWORD=');
      expect(script).toContain('export HIVE_FLOW_JWT_SECRET=');
      expect(script).toContain('export HIVE_FLOW_SESSION_SECRET=');
      expect(script).toContain('export HIVE_FLOW_ENCRYPTION_KEY=');
    });
  ...
  describe('JSON Config Generation', () => {
    ...
    it('should include all credentials', () => {
      ...
      expect(parsed['hive-flow/admin-password']).toBe(credentials.adminPassword);
      expect(parsed['hive-flow/service-password']).toBe(credentials.servicePassword);
      expect(parsed['hive-flow/jwt-secret']).toBe(credentials.jwtSecret);
      expect(parsed['hive-flow/session-secret']).toBe(credentials.sessionSecret);
      expect(parsed['hive-flow/encryption-key']).toBe(credentials.encryptionKey);
    });
  });

...
export class CredentialGenerator {
  ...
  createEnvScript(credentials: GeneratedCredentials): string {
    return `# Hive Flow V3 - Generated Credentials
...
`;
  ...
  createJsonConfig(credentials: GeneratedCredentials): string {
    return JSON.stringify({
      'hive-flow/admin-password': credentials.adminPassword,
      'hive-flow/service-password': credentials.servicePassword,
      'hive-flow/jwt-secret': credentials.jwtSecret,
      'hive-flow/session-secret': credentials.sessionSecret,
      'hive-flow/encryption-key': credentials.encryptionKey,
      'hive-flow/generated-at': credentials.generatedAt.toISOString(),
      'hive-flow/expires-at': credentials.expiresAt?.toISOString() ?? null,
    }, null, 2);
  }
}

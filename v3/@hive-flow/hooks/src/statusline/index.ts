...
export class StatuslineGenerator {
  ...
  private getV3Progress(): StatuslineData['v3Progress'] {
    ...
    const metricsPath = join(this.projectRoot, '.hive-flow', 'metrics', 'v3-progress.json');
    try {
  ...
  private getSecurityStatus(): StatuslineData['security'] {
    ...
    const auditPath = join(this.projectRoot, '.hive-flow', 'security', 'audit-status.json');
    try {
  ...
  private getSwarmActivity(): StatuslineData['swarm'] {
    ...
    const activityPath = join(this.projectRoot, '.hive-flow', 'metrics', 'swarm-activity.json');
    try {

// @ts-nocheck - CLI integration requires the full @hive-flow/cli package
/**
...
const claimCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      ...
      output.printInfo('Usage: hive-flow issues claim <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const releaseCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: hive-flow issues release <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const handoffCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: hive-flow issues handoff <issueId> --to <target>');
      return { success: false, exitCode: 1 };
    }
...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: hive-flow issues status <issueId> [options]');
      return { success: false, exitCode: 1 };
    }
...
const stealableCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln();
      output.printInfo('Use "hive-flow issues steal <issueId>" to take over an issue');
...
const stealCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: hive-flow issues steal <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const markStealableCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: hive-flow issues mark-stealable <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const contestCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: hive-flow issues contest <issueId> --reason "..."');
      return { success: false, exitCode: 1 };
    }
...
export const issuesCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'hive-flow issues list --available', description: 'List unclaimed issues' },
    { command: 'hive-flow issues list --mine', description: 'List my claims' },
    { command: 'hive-flow issues claim GH-123', description: 'Claim an issue' },
    { command: 'hive-flow issues release GH-123', description: 'Release a claim' },
    { command: 'hive-flow issues handoff GH-123 --to agent:coder-1', description: 'Request handoff to agent' },
    { command: 'hive-flow issues handoff GH-123 --to human:alice', description: 'Request handoff to human' },
    { command: 'hive-flow issues status GH-123 --blocked "Waiting for API"', description: 'Mark as blocked' },
    { command: 'hive-flow issues status GH-123 --review-requested', description: 'Request review' },
    { command: 'hive-flow issues board', description: 'View who is working on what' },
    { command: 'hive-flow issues stealable', description: 'List stealable issues' },
    { command: 'hive-flow issues steal GH-123', description: 'Steal an issue' },
    { command: 'hive-flow issues mark-stealable GH-123', description: 'Mark my claim as stealable' },
    { command: 'hive-flow issues contest GH-123 -r "I was actively working on it"', description: 'Contest a steal' },
    { command: 'hive-flow issues load', description: 'View agent load distribution' },
    { command: 'hive-flow issues load --agent coder-1', description: 'View specific agent load' },
    { command: 'hive-flow issues rebalance --dry-run', description: 'Preview rebalancing' },
    { command: 'hive-flow issues rebalance', description: 'Trigger swarm rebalancing' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: hive-flow issues <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "hive-flow issues <subcommand> --help" for subcommand help');

...
export const V2_CLI_COMMANDS: V2CLICommand[] = [
  // Core commands
  { name: 'init', aliases: ['i'], flags: ['--force', '--template'], description: 'Initialize hive-flow project', v3Equivalent: 'init' },
...
export class V2CompatibilityValidator {
  ...
  async validateAPI(): Promise<ValidationResult> {
    ...
    for (const iface of V2_API_INTERFACES) {
      ...
      checks.push({
        ...
        v3Behavior: isSupported
          ? `Import "${v3Equivalent}" from @hive-flow/*`
          : 'Not available',
  ...
  private generateRecommendations(
    ...
    if (api.breakingChanges > 0) {
      recommendations.push('Update import statements to use @hive-flow/* packages');
...
export function generateCompatibilityReport(report: FullValidationReport): string {
  ...
  lines.push("import { HiveMind } from 'hive-flow/hive-mind';");

...
`@hive-flow/guidance` fixes this. It takes the memory files Claude Code already uses — `CLAUDE.md` and `CLAUDE.local.md` — and turns them into a structured control plane that compiles rules, enforces them through gates the agent cannot bypass, proves every decision cryptographically, and evolves the rule set over time based on what actually works.
...
| Capability | Plain CLAUDE.md | Prompt libraries | @hive-flow/guidance |
|-----------|:-:|:-:|:-:|
...
```typescript
import { getKernel } from '@hive-flow/guidance/wasm-kernel';
...
import { createCompiler } from '@hive-flow/guidance/compiler';
const compiler = createCompiler();
const bundle = compiler.compile(claudeMdContent);

// Retrieve task-relevant shards by intent
import { createRetriever } from '@hive-flow/guidance/retriever';
...
import { createGates } from '@hive-flow/guidance/gates';
...
```typescript
import { createContinueGate } from '@hive-flow/guidance/continue-gate';
const gate = createContinueGate({
...
```typescript
import { createProofChain } from '@hive-flow/guidance/proof';
...
import { createToolGateway } from '@hive-flow/guidance/gateway';
...
// Memory write gating
import { createMemoryWriteGate } from '@hive-flow/guidance/memory-gate';
...
import { TrustSystem } from '@hive-flow/guidance/trust';
...
import { createTruthAnchorStore, createTruthResolver } from '@hive-flow/guidance/truth-anchors';
...
import { UncertaintyLedger } from '@hive-flow/guidance/uncertainty';
...
import { TemporalStore, TemporalReasoner } from '@hive-flow/guidance/temporal';
...
```typescript
import { AuthorityGate, IrreversibilityClassifier } from '@hive-flow/guidance/authority';
...
import { createThreatDetector, createCollusionDetector, createMemoryQuorum }
  from '@hive-flow/guidance/adversarial';
...
```typescript
import { createMetaGovernor } from '@hive-flow/guidance/meta-governance';
...
```typescript
import { createConformanceRunner, createMemoryClerkCell } from '@hive-flow/guidance/conformance-kit';
...
```typescript
import { createEvolutionPipeline } from '@hive-flow/guidance/evolution';
...
import {
  ...
  generateAgentIndex,
  scaffold,
} from '@hive-flow/guidance/generators';
...
} from '@hive-flow/guidance/analyzer';
...
```typescript
import type { IContentAwareExecutor } from '@hive-flow/guidance/analyzer';
...
```typescript
import { abBenchmark, getDefaultABTasks } from '@hive-flow/guidance/analyzer';

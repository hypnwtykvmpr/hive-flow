...
CREATE OR REPLACE FUNCTION hive_flow.cosine_distance(
    v1 vector,
    v2 vector
) RETURNS REAL AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION hive_flow.euclidean_distance(
    v1 vector,
    v2 vector
) RETURNS REAL AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION hive_flow.inner_product(
    v1 vector,
    v2 vector
) RETURNS REAL AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION hive_flow.manhattan_distance(
    v1 REAL[],
    v2 REAL[]
) RETURNS REAL AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION hive_flow.chebyshev_distance(
    v1 REAL[],
    v2 REAL[]
) RETURNS REAL AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION hive_flow.minkowski_distance(
    v1 REAL[],
    v2 REAL[],
    p REAL DEFAULT 2.0
) RETURNS REAL AS $$
...
CREATE OR REPLACE FUNCTION hive_flow.l2_normalize(
    v REAL[]
) RETURNS REAL[] AS $$
DECLARE
    norm REAL := 0;
...
CREATE OR REPLACE FUNCTION hive_flow.vector_magnitude(
    v REAL[]
) RETURNS REAL AS $$
DECLARE
    sum_sq REAL := 0;
...
-- Add two vectors
CREATE OR REPLACE FUNCTION hive_flow.vector_add(
    v1 REAL[],
    v2 REAL[]
) RETURNS REAL[] AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION hive_flow.vector_subtract(
    v1 REAL[],
    v2 REAL[]
) RETURNS REAL[] AS $$
DECLARE
...
-- Scale a vector
CREATE OR REPLACE FUNCTION hive_flow.vector_scale(
    v REAL[],
    scalar REAL
) RETURNS REAL[] AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION hive_flow.dot_product(
    v1 REAL[],
    v2 REAL[]
) RETURNS REAL AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION hive_flow.batch_insert_vectors(
...
CREATE OR REPLACE FUNCTION hive_flow.upsert_vector(
    p_embedding vector,
...
BEGIN
    ...
    IF p_id IS NOT NULL THEN
        ...
        IF v_id IS NULL THEN
            ...
            INSERT INTO hive_flow.vectors (id, embedding, metadata, namespace, collection, content, source, hash)
            VALUES (p_id, p_embedding, p_metadata, p_namespace, p_collection, p_content, p_source, v_hash)
            RETURNING id INTO v_id;
        END IF;
    ELSE
        -- Upsert by hash
        INSERT INTO hive_flow.vectors (embedding, metadata, namespace, collection, content, source, hash)
        ...
        SET embedding = EXCLUDED.embedding,
            ...
            version = hive_flow.vectors.version + 1
        RETURNING id INTO v_id;
    END IF;
...
CREATE OR REPLACE FUNCTION hive_flow.upsert_embedding(
    p_namespace TEXT,
    p_name TEXT,
    p_embedding vector,
    p_metadata JSONB DEFAULT '{}',
...
DECLARE
    v_id UUID;
BEGIN
    INSERT INTO hive_flow.embeddings (namespace, name, embedding, dimensions, metadata, model, tags, importance)
...
CREATE OR REPLACE FUNCTION hive_flow.knn_search(
    p_query vector,
...
CREATE OR REPLACE FUNCTION hive_flow.cleanup_expired_vectors()
RETURNS INTEGER AS $$
DECLARE
    deleted_count INTEGER;
BEGIN
    DELETE FROM hive_flow.vectors
...
CREATE OR REPLACE FUNCTION hive_flow.cleanup_caches(
...
CREATE OR REPLACE FUNCTION hive_flow.get_collection_stats(
...
BEGIN
    ...
    SELECT
        ...
        (pg_total_relation_size('hive_flow.vectors') / 1024.0 / 1024.0)::REAL AS total_size_mb,
        MIN(v.created_at) AS oldest_entry,
        MAX(v.created_at) AS newest_entry
    FROM hive_flow.collections c
    LEFT JOIN hive_flow.vectors v ON v.collection = c.name AND v.namespace = c.namespace
...
-- ============================================================================
-- BEGIN;
-- DROP FUNCTION IF EXISTS hive_flow.get_collection_stats(TEXT);
-- DROP FUNCTION IF EXISTS hive_flow.cleanup_caches(INTEGER);
-- DROP FUNCTION IF EXISTS hive_flow.cleanup_expired_vectors();
-- DROP FUNCTION IF EXISTS hive_flow.knn_search(vector, INTEGER, TEXT, TEXT, TEXT, REAL, JSONB);
-- DROP FUNCTION IF EXISTS hive_flow.upsert_embedding(TEXT, TEXT, vector, JSONB, TEXT, TEXT[], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.upsert_vector(vector, JSONB, TEXT, TEXT, TEXT, TEXT, UUID);
-- DROP FUNCTION IF EXISTS hive_flow.batch_insert_vectors(JSONB);
-- DROP FUNCTION IF EXISTS hive_flow.dot_product(REAL[], REAL[]);
-- DROP FUNCTION IF EXISTS hive_flow.vector_scale(REAL[], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.vector_subtract(REAL[], REAL[]);
-- DROP FUNCTION IF EXISTS hive_flow.vector_add(REAL[], REAL[]);
-- DROP FUNCTION IF EXISTS hive_flow.vector_magnitude(REAL[]);
-- DROP FUNCTION IF EXISTS hive_flow.l2_normalize(REAL[]);
-- DROP FUNCTION IF EXISTS hive_flow.minkowski_distance(REAL[], REAL[], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.chebyshev_distance(REAL[], REAL[]);
-- DROP FUNCTION IF EXISTS hive_flow.manhattan_distance(REAL[], REAL[]);
-- DROP FUNCTION IF EXISTS hive_flow.inner_product(vector, vector);
-- DROP FUNCTION IF EXISTS hive_flow.euclidean_distance(vector, vector);
-- DROP FUNCTION IF EXISTS hive_flow.cosine_similarity(vector, vector);
-- DROP FUNCTION IF EXISTS hive_flow.cosine_distance(vector, vector);
-- DELETE FROM hive_flow.migrations WHERE name = '004_create_functions';
-- COMMIT;

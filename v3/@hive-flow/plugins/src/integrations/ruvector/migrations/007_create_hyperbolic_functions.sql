...
CREATE OR REPLACE FUNCTION hive_flow.clamp_to_poincare(
    x REAL,
...
CREATE OR REPLACE FUNCTION hive_flow.clamp_vector_to_poincare(
    v REAL[],
    epsilon REAL DEFAULT 1e-5
) RETURNS REAL[] AS $$
DECLARE
    ...
    result REAL[];
BEGIN
    norm := hive_flow.vector_magnitude(v);
...
CREATE OR REPLACE FUNCTION hive_flow.arctanh(
    x REAL
) RETURNS REAL AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION hive_flow.arcsinh(
    x REAL
) RETURNS REAL AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION hive_flow.arccosh(
    x REAL
) RETURNS REAL AS $$
BEGIN
    IF x < 1.0 THEN
...
CREATE OR REPLACE FUNCTION hive_flow.poincare_distance(
    v1 REAL[],
    v2 REAL[],
...
BEGIN
    ...
    RETURN (1.0 / sqrt(c)) * hive_flow.arccosh(arg);
END;
$$ LANGUAGE plpgsql IMMUTABLE STRICT PARALLEL SAFE;
...
CREATE OR REPLACE FUNCTION hive_flow.lorentz_inner_product(
    v1 REAL[],
    v2 REAL[]
) RETURNS REAL AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION hive_flow.lorentz_distance(
    v1 REAL[],
    v2 REAL[]
) RETURNS REAL AS $$
DECLARE
    inner REAL;
BEGIN
    inner := hive_flow.lorentz_inner_product(v1, v2);
    -- The inner product should be <= -1 for valid hyperboloid points
    RETURN hive_flow.arccosh(-inner);
END;
$$ LANGUAGE plpgsql IMMUTABLE STRICT PARALLEL SAFE;
...
CREATE OR REPLACE FUNCTION hive_flow.exp_map(
...
BEGIN
    ...
    -- Compute ||tangent||
    tangent_norm := hive_flow.vector_magnitude(tangent);
    ...
    BEGIN
        ...
        result := hive_flow.mobius_add(base, scaled_tangent, curvature);
    END;

    RETURN hive_flow.clamp_vector_to_poincare(result);
END;
...
CREATE OR REPLACE FUNCTION hive_flow.log_map(
    base REAL[],          -- Base point on the manifold
...
BEGIN
    ...
    mobius_result := hive_flow.mobius_add(neg_base, point, -c);
    mobius_norm := hive_flow.vector_magnitude(mobius_result);
    ...
    coeff := (2.0 / (sqrt_c * lambda_x)) * hive_flow.arctanh(arctanh_arg) / mobius_norm;
...
CREATE OR REPLACE FUNCTION hive_flow.mobius_add(
    v1 REAL[],
    v2 REAL[],
    curvature REAL DEFAULT -1.0
) RETURNS REAL[] AS $$
...
CREATE OR REPLACE FUNCTION hive_flow.mobius_scalar_mul(
    r REAL,
    v REAL[],
    curvature REAL DEFAULT -1.0
) RETURNS REAL[] AS $$
...
BEGIN
    ...
    v_norm := hive_flow.vector_magnitude(v);
    ...
    tanh_result := tanh(r * hive_flow.arctanh(arctanh_arg));
...
CREATE OR REPLACE FUNCTION hive_flow.parallel_transport(
...
CREATE OR REPLACE FUNCTION hive_flow.hyperbolic_centroid(
    points REAL[][],
    curvature REAL DEFAULT -1.0,
...
BEGIN
    ...
    FOR iter IN 1..max_iterations LOOP
        ...
        FOR i IN 1..n LOOP
            tangent := hive_flow.log_map(centroid, points[i]);
            FOR j IN 1..dim LOOP
        ...
        new_centroid := hive_flow.exp_map(centroid, tangent_sum, curvature);

        -- Check convergence
        diff := hive_flow.poincare_distance(centroid, new_centroid, curvature);
...
CREATE OR REPLACE FUNCTION hive_flow.poincare_to_lorentz(
    poincare REAL[],
    curvature REAL DEFAULT -1.0
) RETURNS REAL[] AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION hive_flow.lorentz_to_poincare(
    lorentz REAL[],
    curvature REAL DEFAULT -1.0
) RETURNS REAL[] AS $$
DECLARE
...
BEGIN
    ...
    RETURN hive_flow.clamp_vector_to_poincare(poincare);
END;
$$ LANGUAGE plpgsql IMMUTABLE STRICT PARALLEL SAFE;
...
CREATE OR REPLACE FUNCTION hive_flow.store_hyperbolic_embedding(
    p_namespace TEXT,
    p_name TEXT,
...
BEGIN
    ...
    v_lorentz := hive_flow.poincare_to_lorentz(p_poincare_embedding, p_curvature);

    INSERT INTO hive_flow.hyperbolic_embeddings (
        namespace,
        name,
        poincare_embedding,
        lorentz_embedding,
    ...
    IF p_parent_id IS NOT NULL THEN
        UPDATE hive_flow.hyperbolic_embeddings
...
CREATE OR REPLACE FUNCTION hive_flow.hyperbolic_knn(
    p_query REAL[],
...
BEGIN
    ...
    SELECT
        h.id,
        h.name,
        hive_flow.poincare_distance(p_query, h.poincare_embedding, p_curvature) AS distance,
        h.depth,
        h.metadata
    FROM hive_flow.hyperbolic_embeddings h
    WHERE (p_namespace IS NULL OR h.namespace = p_namespace)
    ORDER BY hive_flow.poincare_distance(p_query, h.poincare_embedding, p_curvature)
    LIMIT p_k;
END;
$$ LANGUAGE plpgsql STABLE;

-- Get subtree rooted at a node
CREATE OR REPLACE FUNCTION hive_flow.get_hyperbolic_subtree(
    p_root_id UUID,
...
BEGIN
    ...
    WITH RECURSIVE subtree AS (
        ...
        SELECT
            ...
            c.metadata
        FROM hive_flow.hyperbolic_embeddings c
...
-- ============================================================================
-- BEGIN;
-- DROP FUNCTION IF EXISTS hive_flow.get_hyperbolic_subtree(UUID, INTEGER);
-- DROP FUNCTION IF EXISTS hive_flow.hyperbolic_knn(REAL[], INTEGER, TEXT, REAL);
-- DROP FUNCTION IF EXISTS hive_flow.store_hyperbolic_embedding(TEXT, TEXT, REAL[], REAL, INTEGER, UUID, JSONB);
-- DROP FUNCTION IF EXISTS hive_flow.lorentz_to_poincare(REAL[], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.poincare_to_lorentz(REAL[], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.hyperbolic_centroid(REAL[][], REAL, INTEGER, REAL);
-- DROP FUNCTION IF EXISTS hive_flow.parallel_transport(REAL[], REAL[], REAL[], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.mobius_scalar_mul(REAL, REAL[], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.mobius_add(REAL[], REAL[], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.log_map(REAL[], REAL[]);
-- DROP FUNCTION IF EXISTS hive_flow.exp_map(REAL[], REAL[], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.lorentz_distance(REAL[], REAL[]);
-- DROP FUNCTION IF EXISTS hive_flow.lorentz_inner_product(REAL[], REAL[]);
-- DROP FUNCTION IF EXISTS hive_flow.poincare_distance(REAL[], REAL[], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.arccosh(REAL);
-- DROP FUNCTION IF EXISTS hive_flow.arcsinh(REAL);
-- DROP FUNCTION IF EXISTS hive_flow.arctanh(REAL);
-- DROP FUNCTION IF EXISTS hive_flow.clamp_vector_to_poincare(REAL[], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.clamp_to_poincare(REAL, REAL);
-- DELETE FROM hive_flow.migrations WHERE name = '007_create_hyperbolic_functions';
-- COMMIT;

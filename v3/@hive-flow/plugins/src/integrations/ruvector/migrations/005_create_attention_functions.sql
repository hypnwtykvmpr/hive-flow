...
CREATE OR REPLACE FUNCTION hive_flow.softmax(
    scores REAL[]
) RETURNS REAL[] AS $$
DECLARE
    max_score REAL;
...
CREATE OR REPLACE FUNCTION hive_flow.scaled_dot_product_attention(
...
BEGIN
    ...
    -- Apply softmax
    attention_weights := hive_flow.softmax(scores);
...
CREATE OR REPLACE FUNCTION hive_flow.multi_head_attention(
...
BEGIN
    ...
    FOR h IN 1..num_heads LOOP
        ...
        head_output := hive_flow.scaled_dot_product_attention(head_query, head_keys, head_values);
...
CREATE OR REPLACE FUNCTION hive_flow.flash_attention(
...
CREATE OR REPLACE FUNCTION hive_flow.sparse_attention(
    query REAL[],        -- Query vector
    keys REAL[][],       -- Key matrix
    values REAL[][],     -- Value matrix
...
CREATE OR REPLACE FUNCTION hive_flow.linear_attention(
    query REAL[],        -- Query vector
    keys REAL[][],       -- Key matrix
    values REAL[][],     -- Value matrix
...
CREATE OR REPLACE FUNCTION hive_flow.cross_attention(
...
DECLARE
    result RECORD;
BEGIN
    SELECT * INTO result FROM hive_flow.multi_head_attention(
        decoder_query,
        encoder_keys,
        encoder_values,
        num_heads
...
CREATE OR REPLACE FUNCTION hive_flow.cache_attention_result(
    p_query_hash TEXT,
    p_keys_hash TEXT,
    p_values_hash TEXT,
    p_num_heads INTEGER,
...
BEGIN
    ...
    INSERT INTO hive_flow.attention_cache (
        cache_key,
        query_hash,
        keys_hash,
        values_hash,
    ...
    SET attention_weights = EXCLUDED.attention_weights,
        ...
        hit_count = hive_flow.attention_cache.hit_count + 1,
...
CREATE OR REPLACE FUNCTION hive_flow.get_cached_attention(
    p_query_hash TEXT,
    p_keys_hash TEXT,
    p_values_hash TEXT,
    p_num_heads INTEGER,
...
BEGIN
    ...
    INTO v_record
    FROM hive_flow.attention_cache ac
    WHERE ac.cache_key = v_cache_key
      AND (ac.expires_at IS NULL OR ac.expires_at > NOW());

    IF FOUND THEN
        -- Update access stats
        UPDATE hive_flow.attention_cache
...
-- ============================================================================
-- BEGIN;
-- DROP FUNCTION IF EXISTS hive_flow.get_cached_attention(TEXT, TEXT, TEXT, INTEGER, TEXT);
-- DROP FUNCTION IF EXISTS hive_flow.cache_attention_result(TEXT, TEXT, TEXT, INTEGER, TEXT, REAL[], REAL[], INTEGER[], REAL, INTEGER);
-- DROP FUNCTION IF EXISTS hive_flow.cross_attention(REAL[], REAL[][], REAL[][], INTEGER);
-- DROP FUNCTION IF EXISTS hive_flow.linear_attention(REAL[], REAL[][], REAL[][], TEXT);
-- DROP FUNCTION IF EXISTS hive_flow.sparse_attention(REAL[], REAL[][], REAL[][], TEXT, INTEGER, INTEGER);
-- DROP FUNCTION IF EXISTS hive_flow.flash_attention(REAL[], REAL[][], REAL[][], INTEGER);
-- DROP FUNCTION IF EXISTS hive_flow.multi_head_attention(REAL[], REAL[][], REAL[][], INTEGER);
-- DROP FUNCTION IF EXISTS hive_flow.scaled_dot_product_attention(REAL[], REAL[][], REAL[][], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.softmax(REAL[]);
-- DELETE FROM hive_flow.migrations WHERE name = '005_create_attention_functions';
-- COMMIT;

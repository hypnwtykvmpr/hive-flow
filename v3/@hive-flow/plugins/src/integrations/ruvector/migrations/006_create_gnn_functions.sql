...
CREATE OR REPLACE FUNCTION hive_flow.relu(
    x REAL[]
) RETURNS REAL[] AS $$
DECLARE
    result REAL[];
...
CREATE OR REPLACE FUNCTION hive_flow.leaky_relu(
    x REAL[],
...
CREATE OR REPLACE FUNCTION hive_flow.matmul(
    matrix REAL[][],
    vec REAL[]
) RETURNS REAL[] AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION hive_flow.gcn_layer(
...
BEGIN
    ...
    FOR i IN 1..num_nodes LOOP
        DECLARE
            node_output REAL[];
        BEGIN
            node_output := hive_flow.matmul(weights, aggregated[i]);
            node_output := hive_flow.relu(node_output);
...
CREATE OR REPLACE FUNCTION hive_flow.gat_layer(
    nodes REAL[][],         -- Node feature matrix [num_nodes, in_features]
    edges INTEGER[][],      -- Edge list [[src, dst], ...] (0-indexed)
        ...
        FOR h IN 1..num_heads LOOP
            ...
            BEGIN
                ...
                FOR i IN 1..num_nodes LOOP
                    ...
                    attention_scores := hive_flow.leaky_relu(attention_scores, 0.2);
                    attention_weights := hive_flow.softmax(attention_scores);
                    ...
                    -- Apply activation
                    weighted_sum := hive_flow.relu(weighted_sum);
...
CREATE OR REPLACE FUNCTION hive_flow.graph_sage(
    nodes REAL[][],           -- Node feature matrix [num_nodes, in_features]
    edges INTEGER[][],        -- Edge list [[src, dst], ...] (0-indexed)
...
BEGIN
    ...
    FOR i IN 1..num_nodes LOOP
        ...
        BEGIN
            ...
            concat_features := hive_flow.relu(concat_features);
            -- Normalize
            concat_features := hive_flow.l2_normalize(concat_features);
            final_output := array_cat(final_output, ARRAY[concat_features]);
        END;
    END LOOP;
...
CREATE OR REPLACE FUNCTION hive_flow.mpnn_layer(
...
BEGIN
    ...
    FOR i IN 1..num_nodes LOOP
        ...
        END LOOP;
        aggregated[i] := hive_flow.relu(aggregated[i]);
    END LOOP;
...
CREATE OR REPLACE FUNCTION hive_flow.cache_gnn_result(
    p_graph_hash TEXT,
    p_layer_type TEXT,
...
BEGIN
    ...
    INSERT INTO hive_flow.gnn_cache (
        cache_key,
        graph_hash,
        layer_type,
        layer_index,
    ...
    SET node_embeddings = EXCLUDED.node_embeddings,
        hit_count = hive_flow.gnn_cache.hit_count + 1,
...
CREATE OR REPLACE FUNCTION hive_flow.get_cached_gnn(
    p_graph_hash TEXT,
    p_layer_type TEXT,
    p_layer_index INTEGER
) RETURNS TABLE (
...
-- ============================================================================
-- BEGIN;
-- DROP FUNCTION IF EXISTS hive_flow.get_cached_gnn(TEXT, TEXT, INTEGER);
-- DROP FUNCTION IF EXISTS hive_flow.cache_gnn_result(TEXT, TEXT, INTEGER, INTEGER, INTEGER, INTEGER, REAL[], INTEGER, TEXT, INTEGER, INTEGER);
-- DROP FUNCTION IF EXISTS hive_flow.mpnn_layer(REAL[][], INTEGER[][], REAL[][], TEXT, TEXT);
-- DROP FUNCTION IF EXISTS hive_flow.graph_sage(REAL[][], INTEGER[][], TEXT);
-- DROP FUNCTION IF EXISTS hive_flow.gat_layer(REAL[][], INTEGER[][], INTEGER, INTEGER);
-- DROP FUNCTION IF EXISTS hive_flow.gcn_layer(REAL[][], INTEGER[][], REAL[][]);
-- DROP FUNCTION IF EXISTS hive_flow.matmul(REAL[][], REAL[]);
-- DROP FUNCTION IF EXISTS hive_flow.leaky_relu(REAL[], REAL);
-- DROP FUNCTION IF EXISTS hive_flow.relu(REAL[]);
-- DELETE FROM hive_flow.migrations WHERE name = '006_create_gnn_functions';
-- COMMIT;

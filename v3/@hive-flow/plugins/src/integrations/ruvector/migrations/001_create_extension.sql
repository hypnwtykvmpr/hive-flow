...
--
-- Enables required PostgreSQL extensions and creates the hive_flow schema.
...
-- ----------------------------------------------------------------------------
-- Schema: hive_flow
-- Namespace for all Hive Flow tables and functions
-- ----------------------------------------------------------------------------
CREATE SCHEMA IF NOT EXISTS hive_flow;

-- Set search path to include hive_flow schema
COMMENT ON SCHEMA hive_flow IS 'Hive Flow V3 - RuVector PostgreSQL Bridge schema';

-- Grant usage on schema
DO $$
BEGIN
    -- Grant to PUBLIC for general access (modify as needed for security)
    GRANT USAGE ON SCHEMA hive_flow TO PUBLIC;
    GRANT CREATE ON SCHEMA hive_flow TO PUBLIC;
END $$;

-- ----------------------------------------------------------------------------
-- Configuration table for RuVector settings
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS hive_flow.config (
...
INSERT INTO hive_flow.config (key, value) VALUES
    ('version', '"1.0.0"'),
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS hive_flow.migrations (
...
-- ============================================================================
-- BEGIN;
-- DROP TABLE IF EXISTS hive_flow.migrations;
-- DROP TABLE IF EXISTS hive_flow.config;
-- DROP SCHEMA IF EXISTS hive_flow CASCADE;

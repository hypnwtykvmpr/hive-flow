...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS hive_flow.vectors (
...
CREATE OR REPLACE FUNCTION hive_flow.update_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER vectors_updated_at
    BEFORE UPDATE ON hive_flow.vectors
    FOR EACH ROW
    EXECUTE FUNCTION hive_flow.update_updated_at();
...
CREATE TRIGGER embeddings_updated_at
    BEFORE UPDATE ON hive_flow.embeddings
    FOR EACH ROW
    EXECUTE FUNCTION hive_flow.update_updated_at();
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS hive_flow.attention_cache (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_attention_cache_key ON hive_flow.attention_cache (cache_key);
CREATE INDEX IF NOT EXISTS idx_attention_cache_type ON hive_flow.attention_cache (attention_type);
CREATE INDEX IF NOT EXISTS idx_attention_cache_expires ON hive_flow.attention_cache (expires_at) WHERE expires_at IS NOT NULL;

-- ----------------------------------------------------------------------------
-- Table: gnn_cache
-- Caches Graph Neural Network computation results
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS hive_flow.gnn_cache (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_gnn_cache_key ON hive_flow.gnn_cache (cache_key);
CREATE INDEX IF NOT EXISTS idx_gnn_cache_type ON hive_flow.gnn_cache (layer_type);
CREATE INDEX IF NOT EXISTS idx_gnn_cache_graph ON hive_flow.gnn_cache (graph_hash);
CREATE INDEX IF NOT EXISTS idx_gnn_cache_expires ON hive_flow.gnn_cache (expires_at) WHERE expires_at IS NOT NULL;
...
CREATE TRIGGER hyperbolic_embeddings_updated_at
    BEFORE UPDATE ON hive_flow.hyperbolic_embeddings
    FOR EACH ROW
    EXECUTE FUNCTION hive_flow.update_updated_at();
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS hive_flow.collections (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TRIGGER collections_updated_at
    BEFORE UPDATE ON hive_flow.collections
    FOR EACH ROW
    EXECUTE FUNCTION hive_flow.update_updated_at();

-- Insert default collection
INSERT INTO hive_flow.collections (name, namespace, dimensions, metric)
...
-- ============================================================================
-- BEGIN;
-- DROP TABLE IF EXISTS hive_flow.collections CASCADE;
-- DROP TABLE IF EXISTS hive_flow.hyperbolic_embeddings CASCADE;
-- DROP TABLE IF EXISTS hive_flow.gnn_cache CASCADE;
-- DROP TABLE IF EXISTS hive_flow.attention_cache CASCADE;
-- DROP TABLE IF EXISTS hive_flow.embeddings CASCADE;
-- DROP TABLE IF EXISTS hive_flow.vectors CASCADE;
-- DROP FUNCTION IF EXISTS hive_flow.update_updated_at();
-- DELETE FROM hive_flow.migrations WHERE name = '002_create_vector_tables';
-- COMMIT;

...
export async function createVectorDB(dimensions: number): Promise<IVectorDB> {
  try {
    ...
    console.warn('[@hive-flow/plugins] @ruvector/wasm not available, using fallback');
...
export async function createLoRAEngine(): Promise<ILoRAEngine> {
  try {
    ...
    console.warn('[@hive-flow/plugins] @ruvector/learning-wasm not available, using fallback');

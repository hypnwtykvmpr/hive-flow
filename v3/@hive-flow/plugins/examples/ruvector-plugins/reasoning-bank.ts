 ...
 * @example
 * ```typescript
 * import { reasoningBankPlugin } from '@hive-flow/plugins/examples/ruvector-plugins';
...
export class ReasoningBank {
   ...
   * When set, uses @hive-flow/embeddings for high-quality embeddings
   */
  private embeddingProvider: ((text: string) => Promise<Float32Array>) | null = null;

  /**
   * Set external embedding provider from @hive-flow/embeddings
   *
   * @example
   * ```typescript
   ...
   * Used when @hive-flow/embeddings is not configured
   */

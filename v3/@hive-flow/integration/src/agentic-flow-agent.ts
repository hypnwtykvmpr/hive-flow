/**
 * AgenticFlowAgent - Hive Flow Agent with agentic-flow Delegation
 *
 * Per ADR-001: "Use agentic-flow's Agent base class for all agents"
 * This class wraps agentic-flow functionality while adding Hive Flow specifics.
 *
 * This implements the adapter pattern to bridge between:
 * - Hive Flow's agent lifecycle (v3 DDD architecture)
 ...
 * - Flash Attention: optimization enabled for context processing
 * - SONA Learning: low-latency adaptation for real-time learning
 * - AgentDB: fast HNSW-indexed memory/pattern search
 *
...
/**
 * AgenticFlowAgent - Base class for all Hive Flow v3 agents
 *
 * This class serves as the foundation for all agent types in Hive Flow v3,
...
export class AgenticFlowAgent extends EventEmitter implements IAgent {
   ...
   *
   * Benefits:
   * - Flash Attention for faster context processing (Flash Attention optimization)
   * - SONA learning for real-time adaptation (low-latency)
   * - AgentDB for faster memory search (HNSW indexing improvements)
   *
   ...
   * - Flash Attention for Flash Attention optimization faster processing
   * - SONA learning for real-time adaptation
   * - AgentDB for fast HNSW-indexed memory retrieval
   *

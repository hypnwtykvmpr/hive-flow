/**
 * AgentAdapter - Bridge between Hive Flow and agentic-flow Agents
 *
 * Provides bidirectional conversion and delegation patterns between:
 * - Hive Flow v3 DDD agent architecture
...
/**
 * AgentAdapter - Bridges Hive Flow and agentic-flow agents
 *
 ...
 *
 * // Convert agentic-flow agent to Hive Flow agent
 * const { agent, success } = adapter.fromAgenticFlow(agenticFlowAgent);
 *
 * // Create Hive Flow agent with agentic-flow delegation
...
export class AgentAdapter extends EventEmitter {
  ...
  /**
   * Convert agentic-flow agent to Hive Flow AgenticFlowAgent
   *
   * This method creates a Hive Flow agent wrapper around an existing
  ...
  fromAgenticFlow(
    ...
    try {
      // Map agentic-flow config to Hive Flow config
  ...
  /**
   * Create a Hive Flow agent with agentic-flow delegation
   *
  ...
  /**
   * Map agentic-flow agent type to Hive Flow AgentType
   */
  ...
  /**
   * Sync status between Hive Flow and agentic-flow agents
   */
  private syncStatus(
    agent: AgenticFlowAgent,
    agenticFlowAgent: AgenticFlowAgent_External
  ): void {
    // Map agentic-flow status to Hive Flow status

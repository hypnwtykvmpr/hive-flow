/**
 * Hive Flow Integration Bridge
 *
 * Local integration bridge for SONA, attention, SDK compatibility, and runtime
 * health. It does not dynamically load optional external agent frameworks.
 *
 * @module v3/integration/integration-bridge
 * @version 3.0.0
 */
...
/**
 * IntegrationBridge - Core local integration class.
 */
export class IntegrationBridge extends EventEmitter {
  ...
  private mergeConfig(config: Partial<IntegrationConfig>): IntegrationConfig {
    ...
    return {
      ...
      hivememory: { ...defaultConfig.hivememory, ...config.hivememory },
  ...
  private logDebug(message: string, data?: unknown): void {
    if (this.config.debug) {
      console.debug(`[IntegrationBridge] ${message}`, data || '');
    }
  }
}

/**
 * Create and initialize an IntegrationBridge instance
 */
export async function createIntegrationBridge(
  config?: Partial<IntegrationConfig>
): Promise<IntegrationBridge> {
  const bridge = new IntegrationBridge(config);
 ...
 */
let defaultBridge: IntegrationBridge | null = null;
...
export async function getDefaultBridge(
  ...
  if (!defaultBridge) {
    defaultBridge = new IntegrationBridge(config);

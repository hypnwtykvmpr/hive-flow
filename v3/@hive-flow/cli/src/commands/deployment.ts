...
const deployCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow deployment deploy -e prod', description: 'Deploy to production' },
    { command: 'hive-flow deployment deploy --dry-run', description: 'Simulate deployment' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.printBox([
      ...
      ``,
      `URL: https://${env === 'prod' ? 'api' : env}.hive-flow.io`,
...
const statusCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow deployment status', description: 'Show all environments' },
    { command: 'hive-flow deployment status -e prod', description: 'Check production' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
...
const rollbackCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow deployment rollback -e prod', description: 'Rollback production' },
    { command: 'hive-flow deployment rollback -e prod -v v3.0.0', description: 'Rollback to specific version' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const historyCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow deployment history', description: 'Show all history' },
    { command: 'hive-flow deployment history -e prod', description: 'Production history' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const env = ctx.flags.env as string;
...
const environmentsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow deployment environments', description: 'List environments' },
    { command: 'hive-flow deployment envs -a create -n preview', description: 'Create environment' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
...
const logsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow deployment logs -e prod', description: 'View production logs' },
    { command: 'hive-flow deployment logs -d dep-123', description: 'View specific deployment' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const env = ctx.flags.env as string || 'staging';
...
export const deploymentCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow deployment deploy -e prod', description: 'Deploy to production' },
    { command: 'hive-flow deployment status', description: 'Check all environments' },
    { command: 'hive-flow deployment rollback -e prod', description: 'Rollback production' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Hive Flow Deployment'));

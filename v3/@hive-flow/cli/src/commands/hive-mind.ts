...
1️⃣ **COLLECTIVE INTELLIGENCE**
   mcp__hive-flow__hive-mind_consensus    - Democratic decision making
   mcp__hive-flow__hive-mind_memory       - Share knowledge across the hive
   mcp__hive-flow__hive-mind_broadcast    - Broadcast to all workers
   mcp__hive-flow__neural_patterns        - Neural pattern recognition

2️⃣ **QUEEN COORDINATION**
   mcp__hive-flow__hive-mind_status       - Monitor swarm health
   mcp__hive-flow__task_create            - Create and delegate tasks
   mcp__hive-flow__task_orchestrate       - Orchestrate task distribution
   mcp__hive-flow__agent_spawn            - Spawn additional workers

3️⃣ **WORKER MANAGEMENT**
   mcp__hive-flow__agent_list             - List all active agents
   mcp__hive-flow__agent_status           - Check agent status
   mcp__hive-flow__agent_metrics          - Track worker performance
   mcp__hive-flow__hive-mind_join         - Add agent to hive
   mcp__hive-flow__hive-mind_leave        - Remove agent from hive

4️⃣ **TASK ORCHESTRATION**
   mcp__hive-flow__task_create            - Create hierarchical tasks
   mcp__hive-flow__task_status            - Track task progress
   mcp__hive-flow__task_complete          - Mark tasks complete
...
5️⃣ **MEMORY & LEARNING**
   ...
   mcp__hive-flow__memory_search          - Search memory patterns
   mcp__hive-flow__neural_train           - Learn from experiences
   mcp__hive-flow__hooks_intelligence_pattern-store - Store patterns
...
💡 COORDINATION TIPS:
• Use mcp__hive-flow__hive-mind_broadcast for swarm-wide announcements
• Check worker status regularly with mcp__hive-flow__hive-mind_status
...
async function spawnClaudeCodeInstance(
  ...
  try {
    ...
    output.printList([
      ...
      `MCP Tools: ${output.success('Full Hive-Flow integration enabled')}`
    ]);
...
const initCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow hive-mind init -t hierarchical-mesh', description: 'Init hierarchical mesh' },
    { command: 'hive-flow hive-mind init -c byzantine -m 20', description: 'Init with Byzantine consensus' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('  Use "hive-flow hive-mind spawn" to add workers'));
      output.writeln(output.dim('  Use "hive-flow hive-mind spawn --claude" to launch Claude Code'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (workerData.length === 0) {
        output.printInfo('No workers in hive. Use "hive-flow hive-mind spawn" to add workers.');
      } else {
        output.printTable({
          columns: [
...
const taskCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow hive-mind task -d "Implement auth module"', description: 'Submit task' },
    { command: 'hive-flow hive-mind task -d "Security review" -p critical -c', description: 'Critical task with consensus' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim(`  Track with: hive-flow hive-mind task-status ${result.taskId}`));
...
export const hiveMindCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'hive-flow hive-mind init -t hierarchical-mesh', description: 'Initialize hive' },
    { command: 'hive-flow hive-mind spawn -n 5', description: 'Spawn workers' },
    { command: 'hive-flow hive-mind spawn --claude -o "Build a feature"', description: 'Launch Claude Code with hive mind' },
    { command: 'hive-flow hive-mind task -d "Build feature"', description: 'Submit task' }
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Hive Mind - Consensus-Based Multi-Agent Coordination'));
    output.writeln();
    output.writeln('Usage: hive-flow hive-mind <subcommand> [options]');
    output.writeln();
    ...
    output.writeln(output.dim('  hive-flow hive-mind init'));
    output.writeln(output.dim('  hive-flow hive-mind spawn -n 5 --claude -o "Your objective here"'));

...
const listCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow claims list', description: 'List all claims' },
    { command: 'hive-flow claims list -u user123', description: 'List user claims' },
  ],
...
const checkCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow claims check -c swarm:create', description: 'Check swarm creation permission' },
    { command: 'hive-flow claims check -c admin:delete -u user123', description: 'Check user permission' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const claimsConfigPaths = [
        path.resolve('.hive-flow/claims.json'),
        path.resolve('hive-flow.claims.json'),
        path.resolve(process.env.HOME || '~', '.config/hive-flow/claims.json'),
      ];
...
const grantCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow claims grant -c swarm:create -u user123', description: 'Grant to user' },
    { command: 'hive-flow claims grant -c agent:spawn -r developer', description: 'Grant to role' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const claim = ctx.flags.claim as string;
...
const revokeCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow claims revoke -c swarm:delete -u user123', description: 'Revoke from user' },
    { command: 'hive-flow claims revoke -c admin:* -r guest', description: 'Revoke from role' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const claim = ctx.flags.claim as string;
    const user = ctx.flags.user as string;
...
const rolesCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow claims roles', description: 'List all roles' },
    { command: 'hive-flow claims roles -a show -n admin', description: 'Show role details' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const policiesCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow claims policies', description: 'List policies' },
    { command: 'hive-flow claims policies -a create -n rate-limit', description: 'Create policy' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
...
export const claimsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow claims list', description: 'List all claims' },
    { command: 'hive-flow claims check -c swarm:create', description: 'Check permission' },
    { command: 'hive-flow claims grant -c agent:spawn -r developer', description: 'Grant claim' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Hive Flow Claims System'));

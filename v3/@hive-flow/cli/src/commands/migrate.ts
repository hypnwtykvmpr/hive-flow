...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const status = {
      ...
      components: [
        { name: 'Configuration', status: 'migrated', v2Path: './hive-flow.json', v3Path: './hive-flow.config.json' },
        { name: 'Memory Data', status: 'pending', v2Path: './.hive-flow/memory', v3Path: './data/memory' },
        { name: 'Agent Configs', status: 'pending', v2Path: './.hive-flow/agents', v3Path: './v3/agents' },
...
const runCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (backup && !dryRun) {
      ...
      output.writeln(output.dim(`  Backup created: ./.hive-flow-backup-${Date.now()}`));
      output.writeln();
    }
...
const breakingCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const changes = [
      {
        ...
        changes: [
          { change: 'Config file renamed', from: 'hive-flow.json', to: 'hive-flow.config.json' },
      ...
      },
      {
        ...
        changes: [
          ...
          { change: 'Storage path changed', from: '.hive-flow/memory', to: 'data/memory' }
        ]
      },
      {
        category: 'CLI',
      ...
      },
      {
        ...
        changes: [
          ...
          { change: 'Neural substrate', from: 'None', to: 'Local neural kernels' }
        ]
      }
    ];
    ...
    output.printInfo('Run "hive-flow migrate run" to automatically handle these changes');
...
export const migrateCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'hive-flow migrate status', description: 'Check migration status' },
    { command: 'hive-flow migrate run --dry-run', description: 'Preview migration' },
    { command: 'hive-flow migrate run -t all', description: 'Run full migration' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('V2 to V3 Migration Tools'));
    output.writeln();
    output.writeln('Usage: hive-flow migrate <subcommand> [options]');
    output.writeln();
...
function getMigrationSteps(target: string): Array<{ name: string; description: string; source: string; dest: string }> {
  const allSteps = [
    ...
    { name: 'Embeddings System', description: 'Migrate to ONNX with hyperbolic (Poincaré ball)', source: 'OpenAI/TF.js embeddings', dest: '.hive-flow/embeddings.json' }
  ];

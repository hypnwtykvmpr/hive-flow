...
const routeTaskCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!taskDescription) {
      ...
      output.writeln(output.dim('Usage: hive-flow route task "task description"'));
...
const listAgentsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow route list-agents', description: 'List all agents' },
    { command: 'hive-flow route agents --json', description: 'List agents as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const jsonOutput = ctx.flags.json as boolean;
...
const resetCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow route reset', description: 'Reset router state' },
    { command: 'hive-flow route reset --force', description: 'Force reset' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const exportCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow route export', description: 'Export Q-table to stdout' },
    { command: 'hive-flow route export -f qtable.json', description: 'Export to file' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const importCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow route import -f qtable.json', description: 'Import Q-table from file' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const coverageRouteCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow route coverage', description: 'Analyze coverage and suggest routing' },
    { command: 'hive-flow route coverage --suggest', description: 'Get improvement suggestions' },
    { command: 'hive-flow route coverage --gaps', description: 'List coverage gaps by agent' },
    { command: 'hive-flow route coverage -p src/auth -t 90', description: 'Analyze specific path with threshold' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const routeCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln('Usage: hive-flow route <task> [options]');
    output.writeln('       hive-flow route <subcommand>');
    output.writeln();
    ...
    output.writeln(output.dim('Run "hive-flow route <subcommand> --help" for more info'));

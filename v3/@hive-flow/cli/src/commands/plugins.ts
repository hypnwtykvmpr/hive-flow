...
const listCommand: Command = {
  ...
  options: [
    ...
    { name: 'registry', short: 'r', type: 'string', description: 'Registry to use (default: hive-flow-official)' },
  ],
  examples: [
    { command: 'hive-flow plugins list', description: 'List all plugins from registry' },
    { command: 'hive-flow plugins list --installed', description: 'List installed only' },
    { command: 'hive-flow plugins list --official', description: 'List official plugins' },
    { command: 'hive-flow plugins list --category security', description: 'List security plugins' },
  ],
    ...
    if (installedOnly) {
      ...
      try {
        ...
        if (installed.length === 0) {
          ...
          output.writeln();
          output.writeln(output.dim('Run "hive-flow plugins list" to see available plugins'));
          output.writeln(output.dim('Run "hive-flow plugins install -n <plugin>" to install'));
          return { success: true };
        }
...
const installCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow plugins install -n community-analytics', description: 'Install plugin from IPFS' },
    { command: 'hive-flow plugins install -n ./my-plugin --dev', description: 'Install local plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (existingPlugin) {
        ...
        output.writeln();
        output.writeln(output.dim('Use "hive-flow plugins upgrade -n ' + name + '" to update'));
        return { success: false, exitCode: 1 };
      }
...
const uninstallCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow plugins uninstall -n community-analytics', description: 'Uninstall plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const toggleCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow plugins toggle -n analytics --enable', description: 'Enable plugin' },
    { command: 'hive-flow plugins toggle -n analytics --disable', description: 'Disable plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const infoCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow plugins info -n @hive-flow/embeddings', description: 'Show plugin info' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
    const registryName = ctx.flags.registry as string;
...
const createCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow plugins create -n my-plugin', description: 'Create basic plugin' },
    { command: 'hive-flow plugins create -n my-plugin -t hooks', description: 'Create hooks plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const upgradeCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow plugins upgrade -n @hive-flow/embeddings', description: 'Upgrade to latest' },
    { command: 'hive-flow plugins upgrade -n @hive-flow/embeddings -v 3.1.0', description: 'Upgrade to specific version' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow plugins search -q neural', description: 'Search for neural plugins' },
    { command: 'hive-flow plugins search -q security --verified', description: 'Search verified security plugins' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const rateCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow plugins rate -n @hive-flow/embeddings -r 5', description: 'Rate 5 stars' },
    { command: 'hive-flow plugins rate -n my-plugin -r 4', description: 'Rate 4 stars' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const pluginsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow plugins list', description: 'List plugins from IPFS registry' },
    { command: 'hive-flow plugins search -q neural', description: 'Search for plugins' },
    { command: 'hive-flow plugins install -n community-analytics', description: 'Install from IPFS' },
    { command: 'hive-flow plugins create -n my-plugin', description: 'Create new plugin' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Hive Flow Plugin System'));

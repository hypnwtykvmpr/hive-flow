...
const startCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow swarm start -o "Build REST API" -s development', description: 'Start development swarm' },
    { command: 'hive-flow swarm start -o "Analyze codebase" --parallel', description: 'Parallel analysis' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln(output.dim(`  Monitor: hive-flow swarm status ${executionState.swarmId}`));
...
export const swarmCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: hive-flow swarm <subcommand> [options]');
    output.writeln();

...
export const taskCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'hive-flow task create -t implementation -d "Add user auth"', description: 'Create a task' },
    { command: 'hive-flow task list', description: 'List pending/running tasks' },
    { command: 'hive-flow task list --all', description: 'List all tasks' },
    { command: 'hive-flow task status task-123', description: 'Get task details' },
    { command: 'hive-flow task cancel task-123', description: 'Cancel a task' },
    { command: 'hive-flow task assign task-123 --agent coder-1', description: 'Assign task to agent' },
    { command: 'hive-flow task retry task-123', description: 'Retry a failed task' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: hive-flow task <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "hive-flow task <subcommand> --help" for subcommand help');

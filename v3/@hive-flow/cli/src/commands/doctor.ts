...
async function checkConfigFile(): Promise<HealthCheck> {
  ...
  return { name: 'Config File', status: 'warn', message: 'No config file (using defaults)', fix: 'hive-flow config init' };
}

// Check daemon status
async function checkDaemonStatus(): Promise<HealthCheck> {
  try {
    const pidFile = '.hive-flow/daemon.pid';
    if (existsSync(pidFile)) {
      ...
      try {
        ...
        return { name: 'Daemon Status', status: 'warn', message: 'Stale PID file', fix: 'rm .hive-flow/daemon.pid && hive-flow daemon start' };
      }
    }
    return { name: 'Daemon Status', status: 'warn', message: 'Not running', fix: 'hive-flow daemon start' };
  } catch {
    return { name: 'Daemon Status', status: 'warn', message: 'Unable to check', fix: 'hive-flow daemon status' };
  }
}
...
async function checkMemoryDatabase(): Promise<HealthCheck> {
  ...
  return { name: 'Memory Database', status: 'warn', message: 'Not initialized', fix: 'hive-flow memory configure --backend hybrid' };
}
  ...
  for (const configPath of mcpConfigPaths) {
    if (existsSync(configPath)) {
      try {
        ...
        const hasHiveFlow = 'hive-flow' in servers || 'hive-flow_alpha' in servers;
        if (hasHiveFlow) {
...
async function checkVersionFreshness(): Promise<HealthCheck> {
  try {
    ...
    try {
      ...
      // package.json that belongs to @hive-flow/cli (or hive-flow/cli).
...
export const doctorCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow doctor', description: 'Run full health check' },
    { command: 'hive-flow doctor --fix', description: 'Show fixes for issues' },
    { command: 'hive-flow doctor --install', description: 'Auto-install missing dependencies' },
    { command: 'hive-flow doctor -c version', description: 'Check for stale npx cache' },
    { command: 'hive-flow doctor -c claude', description: 'Check Claude Code CLI only' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Hive Flow Doctor'));

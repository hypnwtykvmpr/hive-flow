...
const startCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow daemon start', description: 'Start daemon in background (default)' },
    { command: 'hive-flow daemon start --foreground', description: 'Start in foreground (blocks terminal)' },
    { command: 'hive-flow daemon start -w map,audit,optimize', description: 'Start with specific workers' },
    { command: 'hive-flow daemon start --headless --sandbox strict', description: 'Start with headless workers in strict sandbox' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const quiet = ctx.flags.quiet as boolean;
    const foreground = ctx.flags.foreground as boolean;
    const projectRoot = process.cwd();
    const isDaemonProcess = process.env.HIVE_FLOW_DAEMON === '1';
    ...
    try {
      const stateDir = join(projectRoot, '.hive-flow');
...
function validatePath(path: string, label: string): void {
  ...
  if (!resolved.includes('.hive-flow') && !resolved.includes('bin')) {
...
async function startBackgroundDaemon(projectRoot: string, quiet: boolean): Promise<CommandResult> {
  ...
  const stateDir = join(resolvedRoot, '.hive-flow');
  const pidFile = join(stateDir, 'daemon.pid');
  ...
  const child = spawn(process.execPath, [
    ...
    env: { ...process.env, HIVE_FLOW_DAEMON: '1' },
  });
  ...
  if (!quiet) {
    ...
    output.printInfo(`Stop with: hive-flow daemon stop`);
  }
...
const stopCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow daemon stop', description: 'Stop the daemon' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const quiet = ctx.flags.quiet as boolean;
    const projectRoot = process.cwd();
...
async function killBackgroundDaemon(projectRoot: string): Promise<boolean> {
  const pidFile = join(projectRoot, '.hive-flow', 'daemon.pid');
...
const statusCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow daemon status', description: 'Show daemon status' },
    { command: 'hive-flow daemon status -v', description: 'Show detailed status' },
    { command: 'hive-flow daemon status --show-modes', description: 'Show worker execution modes' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printBox(
        [
          ...
          '',
          'Run "hive-flow daemon start" to start the daemon',
        ].join('\n'),
        'Worker Daemon'
      );
...
const triggerCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow daemon trigger -w map', description: 'Trigger the map worker' },
    { command: 'hive-flow daemon trigger -w audit', description: 'Trigger security audit' },
    { command: 'hive-flow daemon trigger -w audit --headless', description: 'Trigger audit in headless sandbox' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const enableCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow daemon enable -w predict', description: 'Enable predict worker' },
    { command: 'hive-flow daemon enable -w document --disable', description: 'Disable document worker' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const workerType = ctx.flags.worker as WorkerType;
...
export const daemonCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'hive-flow daemon start', description: 'Start the daemon' },
    { command: 'hive-flow daemon start --headless', description: 'Start with headless workers (E2B sandbox)' },
    { command: 'hive-flow daemon status', description: 'Check daemon status' },
    { command: 'hive-flow daemon stop', description: 'Stop the daemon' },
    { command: 'hive-flow daemon trigger -w audit', description: 'Run security audit' },
  ],
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Run "hive-flow daemon <subcommand> --help" for details');

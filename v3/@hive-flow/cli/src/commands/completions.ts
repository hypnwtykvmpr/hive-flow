...
# Generated by hive-flow completions bash
# Add this to ~/.bashrc or ~/.bash_completion

_hive_flow_completions() {
...
complete -F _hive_flow_completions npx\\ @hive-flow/cli@v3alpha
`;
}
...
# Generated by hive-flow completions zsh
# Add to ~/.zfunc/_hive-flow or ~/.zsh/completions/_hive-flow
...
_hive_flow() {
    ...
    case $state in
        command)
            _describe -t commands 'hive-flow commands' commands
            ;;
        subcommand)
            case $words[2] in
                swarm)
...
# Generated by hive-flow completions fish
# Save to ~/.config/fish/completions/hive-flow.fish

# Disable file completion by default
complete -c hive-flow -f

# Top-level commands
${TOP_LEVEL_COMMANDS.map(cmd => `complete -c hive-flow -n "__fish_use_subcommand" -a "${cmd}"`).join('\n')}

# Swarm subcommands
${SWARM_SUBCOMMANDS.map(sub => `complete -c hive-flow -n "__fish_seen_subcommand_from swarm" -a "${sub}"`).join('\n')}

# Agent subcommands
${AGENT_SUBCOMMANDS.map(sub => `complete -c hive-flow -n "__fish_seen_subcommand_from agent" -a "${sub}"`).join('\n')}

# Task subcommands
${TASK_SUBCOMMANDS.map(sub => `complete -c hive-flow -n "__fish_seen_subcommand_from task" -a "${sub}"`).join('\n')}

# Memory subcommands
${MEMORY_SUBCOMMANDS.map(sub => `complete -c hive-flow -n "__fish_seen_subcommand_from memory" -a "${sub}"`).join('\n')}

# Hive-mind subcommands
${HIVE_MIND_SUBCOMMANDS.map(sub => `complete -c hive-flow -n "__fish_seen_subcommand_from hive-mind hive" -a "${sub}"`).join('\n')}

# Hooks subcommands
${HOOKS_SUBCOMMANDS.map(sub => `complete -c hive-flow -n "__fish_seen_subcommand_from hooks" -a "${sub}"`).join('\n')}

# Neural subcommands
complete -c hive-flow -n "__fish_seen_subcommand_from neural" -a "train status patterns predict optimize"

# Security subcommands
complete -c hive-flow -n "__fish_seen_subcommand_from security" -a "scan cve threats audit secrets"

# Performance subcommands
complete -c hive-flow -n "__fish_seen_subcommand_from performance" -a "benchmark profile metrics optimize bottleneck"

# Plugins subcommands
complete -c hive-flow -n "__fish_seen_subcommand_from plugins" -a "list install uninstall toggle info create"

# Deployment subcommands
complete -c hive-flow -n "__fish_seen_subcommand_from deployment deploy" -a "deploy status rollback history environments logs"

# Claims subcommands
complete -c hive-flow -n "__fish_seen_subcommand_from claims" -a "list check grant revoke roles policies"

# Embeddings subcommands
complete -c hive-flow -n "__fish_seen_subcommand_from embeddings" -a "generate search compare collections index providers"

# Daemon subcommands
complete -c hive-flow -n "__fish_seen_subcommand_from daemon" -a "start stop status trigger enable"

# Providers subcommands
complete -c hive-flow -n "__fish_seen_subcommand_from providers" -a "list configure test models usage"
`;
}

// Generate PowerShell completion script
function generatePowerShellCompletion(): string {
  return `# hive-flow PowerShell completion
# Generated by hive-flow completions powershell
# Add to $PROFILE or save to a separate file and dot-source it

$script:HiveFlowCommands = @(
...
Register-ArgumentCompleter -Native -CommandName hive-flow -ScriptBlock {
    ...
    if ($tokens.Count -eq 2) {
        ...
        $script:HiveFlowCommands | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
...
export const completionsCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'hive-flow completions bash > ~/.bash_completion.d/hive-flow', description: 'Install bash completions' },
    { command: 'hive-flow completions zsh > ~/.zfunc/_hive-flow', description: 'Install zsh completions' },
    { command: 'hive-flow completions fish > ~/.config/fish/completions/hive-flow.fish', description: 'Install fish completions' },
    { command: 'hive-flow completions powershell >> $PROFILE', description: 'Install PowerShell completions' }
  ],
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln(output.dim('  hive-flow completions bash > ~/.bash_completion.d/hive-flow'));
    output.writeln(output.dim('  source ~/.bash_completion.d/hive-flow'));
    output.writeln();
    output.writeln(output.bold('Zsh:'));
    output.writeln(output.dim('  mkdir -p ~/.zfunc'));
    output.writeln(output.dim('  hive-flow completions zsh > ~/.zfunc/_hive-flow'));
    output.writeln(output.dim('  # Add to ~/.zshrc: fpath=(~/.zfunc $fpath); autoload -Uz compinit && compinit'));
    output.writeln();
    output.writeln(output.bold('Fish:'));
    output.writeln(output.dim('  hive-flow completions fish > ~/.config/fish/completions/hive-flow.fish'));
    output.writeln();
    output.writeln(output.bold('PowerShell:'));
    output.writeln(output.dim('  hive-flow completions powershell >> $PROFILE'));

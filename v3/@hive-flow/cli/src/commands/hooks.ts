...
const statuslineCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    // Runbook §6.3: delegate to the canonical Claude Code renderer so the
    // hooks subcommand and the top-level `hive-flow statusline` command
    // produce identical output instead of maintaining a duplicate
    ...
    return { success: true, data: { delegatedTo: 'hive-flow statusline' } };
  }
};

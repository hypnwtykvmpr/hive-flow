...
const trainCommand: Command = {
  ...
  options: [
    ...
    { name: 'wasm', short: 'w', type: 'boolean', description: 'Use local accelerated training path', default: 'true' },
    { name: 'flash', type: 'boolean', description: 'Enable local Flash Attention-compatible kernel', default: 'true' },
  ...
  examples: [
    { command: 'hive-flow neural train -p coordination -e 100', description: 'Train coordination patterns' },
    { command: 'hive-flow neural train -d ./training-data.json --flash', description: 'Train from file with Flash Attention' },
    { command: 'hive-flow neural train -p security --wasm --contrastive', description: 'Security patterns with contrastive learning' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Neural Pattern Training (Local Kernels)'));
    output.writeln(output.dim('─'.repeat(55)));

    const spinner = output.createSpinner({ text: 'Initializing local training systems...', spinner: 'dots' });
    spinner.start();
    ...
    try {
      ...
      if (useWasm) {
        const initResult = await localTraining.initializeTraining({
          dim,
          learningRate,
          alpha: 0.1,
        ...
        if (initResult.success) {
          ...
          spinner.setText(`Local training initialized: ${wasmFeatures.join(', ')}`);
        } else {
          output.writeln(output.warning(`Local training init failed: ${initResult.error} - falling back`));
        }
      }
      ...
      const operatorMap: Record<string, number> = {
        coordination: localTraining.OperatorType.COORDINATION,
        optimization: localTraining.OperatorType.OPTIMIZATION,
        prediction: localTraining.OperatorType.ROUTING,
        security: localTraining.OperatorType.SECURITY,
        testing: localTraining.OperatorType.TESTING,
        debugging: localTraining.OperatorType.DEBUGGING,
        memory: localTraining.OperatorType.MEMORY,
        reasoning: localTraining.OperatorType.REASONING,
      };
      const operatorType = operatorMap[patternType] ?? localTraining.OperatorType.GENERAL;
      ...
      // Main training loop with local acceleration
      for (let epoch = 0; epoch < epochs; epoch++) {
        const epochStart = performance.now();

        // Get curriculum difficulty if enabled
        const difficulty = useCurriculum ? localTraining.getCurriculumDifficulty(epoch) : 1.0;
        ...
        if (useContrastive && batch.length >= 3 && useWasm && wasmFeatures.length > 0) {
          ...
          try {
            ...
            const { loss, gradient } = localTraining.computeContrastiveLoss(anchor, positives, negatives);
            totalLoss += loss;
            ...
            localTraining.recordTrajectory(anchor, operatorType, useFlash ? 1 : 0, executionMs, baselineMs);
          } catch {
            // Local training failed, fall back to basic
          }
        }
      ...
      // Get local training stats
      const localTrainingStats = useWasm && wasmFeatures.length > 0 ? localTraining.getTrainingStats() : null;
      const trajectoryStats = localTrainingStats?.trajectoryStats;

      // Benchmark if local training was used
      ...
      if (useWasm && wasmFeatures.length > 0) {
        try {
          ...
          benchmark = await localTraining.benchmarkTraining(dim, 100);
        } catch {
      ...
      // Add local-training-specific metrics
      if (useWasm && wasmFeatures.length > 0) {
        tableData.push(
      ...
      if (useWasm && wasmFeatures.length > 0) {
        output.writeln(output.highlight(`Local training: ${wasmFeatures.join(', ')}`));
      }
...
const statusCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow neural status', description: 'Show all neural status' },
    { command: 'hive-flow neural status -m model-123', description: 'Check specific model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // Check local training status
      const localTrainingStats = localTraining.getTrainingStats();
      const sonaAvailable = localTraining.isSonaAvailable();
      ...
      output.printTable({
        ...
        data: [
          ...
          },
          {
            ...
            details: sonaAvailable && localTrainingStats.sonaStats
              ? `${localTrainingStats.sonaStats.totalLearns} learns, ${localTrainingStats.sonaStats.totalSearches} searches`
      ...
      if (verbose) {
        ...
        // Add local training metrics if initialized
        if (localTrainingStats.initialized) {
          detailedData.push(
          ...
          if (sonaAvailable && localTrainingStats.sonaStats?.stats) {
            const sonaStats = localTrainingStats.sonaStats.stats as Record<string, unknown>;
            detailedData.push(
      ...
      return { success: true, data: { stats, hnswStatus, adaptBench, modelInfo, localTrainingStats } };
    } catch (error) {
...
const patternsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow neural patterns --action list', description: 'List all patterns' },
    { command: 'hive-flow neural patterns -a analyze -q "error handling"', description: 'Analyze patterns' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('Run: hive-flow neural train --pattern-type general'));
...
const predictCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow neural predict -i "implement authentication"', description: 'Predict routing for task' },
    { command: 'hive-flow neural predict -i "fix bug in login" -k 3', description: 'Get top 3 predictions' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (matches.length === 0) {
        output.writeln(output.warning('No similar patterns found. Try training first: hive-flow neural train'));
...
const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow neural optimize --method quantize', description: 'Quantize patterns to Int8' },
    { command: 'hive-flow neural optimize --method analyze -v', description: 'Analyze memory usage' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const patternDir = path.join(process.cwd(), '.hive-flow', 'neural');
      let beforeSize = 0;
      try {
...
const exportCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow neural export -m security-patterns --ipfs', description: 'Export and pin to IPFS' },
    { command: 'hive-flow neural export -m code-review -o ./export.json', description: 'Export to file' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const exportData = {
        ...
        version: '1.0.0',
        name: customName || `hive-flow-model-${Date.now()}`,
      ...
      const memoryDir = path.join(process.cwd(), '.hive-flow', 'memory');
      ...
      if (pinToIpfs) {
        ...
        output.writeln(output.dim(`Import command: hive-flow neural import --cid ${result.IpfsHash}`));
      }
...
const listCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow neural list', description: 'List all available models' },
    { command: 'hive-flow neural list --category security', description: 'List only security models' },
    { command: 'hive-flow neural list -f json', description: 'Output as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (format === 'json') {
        ...
        output.writeln(output.dim('  All models:      ') + `hive-flow neural import --cid ${registryCid}`);
        if (category) {
          output.writeln(output.dim(`  ${category} only: `) + `hive-flow neural import --cid ${registryCid} --category ${category}`);
        } else {
          output.writeln(output.dim('  By category:     ') + `hive-flow neural import --cid ${registryCid} --category <category>`);
        }
      }
...
const importCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow neural import --cid QmXxx...', description: 'Import from IPFS' },
    { command: 'hive-flow neural import -f ./patterns.json --verify', description: 'Import from file' },
    { command: 'hive-flow neural import --cid QmNr1yYMK... --category security', description: 'Import only security patterns' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('Run "hive-flow neural patterns --action list" to see imported patterns'));
...
// Benchmark subcommand - local neural kernel benchmarks
const benchmarkCommand: Command = {
  name: 'benchmark',
  description: 'Benchmark local neural training performance',
  options: [
  ...
  examples: [
    { command: 'hive-flow neural benchmark', description: 'Run default benchmark' },
    { command: 'hive-flow neural benchmark -d 128 -i 5000', description: 'Custom benchmark' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const dim = Math.min(parseInt(ctx.flags.dim as string || '256', 10), 256);
    ...
    output.writeln();
    output.writeln(output.bold('Local Neural Benchmark'));
    output.writeln(output.dim('─'.repeat(50)));
    ...
    try {
      ...
      if (dotProductResult && flashResult) {
        ...
        output.writeln();
        output.writeln(output.highlight(`Flash-compatible kernel ratio: ${speedup.toFixed(2)}x vs DotProduct`));
      }
...
export const neuralCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow neural status', description: 'Check neural system status' },
    { command: 'hive-flow neural train -p coordination', description: 'Train coordination patterns' },
    { command: 'hive-flow neural patterns --action list', description: 'List learned patterns' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Hive Flow Neural System'));

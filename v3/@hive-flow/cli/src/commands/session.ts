 ...
 * Session management for Hive Flow
 */
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (result.sessions.length === 0) {
        ...
        output.printInfo('Run "hive-flow session save" to create a session');
        return { success: true, data: result };
      }
...
const saveCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printInfo(`Restore with: hive-flow session restore ${result.sessionId}`);
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      ...
      if (error instanceof MCPClientError) {
        ...
        output.printInfo('Start a session with "hive-flow start"');
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (dismissMode) {
      ...
      try {
        ...
        await crashDetector.markRecovered(selected.sessionId);
        output.printInfo('Checkpoint preserved. Restore manually with: hive-flow session restore ' + selected.sessionId);
      }
    }
...
export const sessionCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'hive-flow session list', description: 'List all sessions' },
    { command: 'hive-flow session save -n "checkpoint-1"', description: 'Save current session' },
    { command: 'hive-flow session restore session-123', description: 'Restore a session' },
    { command: 'hive-flow session delete session-123', description: 'Delete a session' },
    { command: 'hive-flow session export -o backup.json', description: 'Export session to file' },
    { command: 'hive-flow session import backup.json', description: 'Import session from file' },
    { command: 'hive-flow session current', description: 'Show current session' },
    { command: 'hive-flow session recover', description: 'Detect and recover crashed sessions' },
    { command: 'hive-flow session recover --dismiss -s sess-123', description: 'Dismiss a crashed session' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: hive-flow session <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "hive-flow session <subcommand> --help" for subcommand help');

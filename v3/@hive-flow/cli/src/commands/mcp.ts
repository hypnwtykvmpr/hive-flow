 ...
 *
 * @module @hive-flow/cli/commands/mcp
 * @version 3.0.0
 */
...
const startCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow mcp start', description: 'Start with defaults (stdio)' },
    { command: 'hive-flow mcp start -p 8080 -t http', description: 'Start HTTP server' },
    { command: 'hive-flow mcp start -d', description: 'Start as daemon' },
    { command: 'hive-flow mcp start -f', description: 'Force restart (kill existing)' }
  ],
    ...
    if (existingStatus.running) {
      ...
      if (!shouldForceRestart) {
        ...
        if (health.healthy) {
          ...
          output.writeln(output.dim('Use "hive-flow mcp stop" to stop the server first, or use --force'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      ...
      if (!status.running) {
        ...
        output.writeln();
        output.writeln(output.dim('Run "hive-flow mcp start" to start the server'));
        return { success: true, data: status };
      }
...
export const mcpCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'hive-flow mcp start', description: 'Start MCP server' },
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: hive-flow mcp <subcommand> [options]');
    output.writeln();

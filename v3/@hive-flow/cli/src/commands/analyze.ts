...
const diffCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow analyze diff --risk', description: 'Analyze current diff with risk assessment' },
    { command: 'hive-flow analyze diff HEAD~1 --classify', description: 'Classify changes from last commit' },
    { command: 'hive-flow analyze diff main..feature --format json', description: 'Compare branches with JSON output' },
    { command: 'hive-flow analyze diff --reviewers', description: 'Get recommended reviewers for changes' },
  ],
...
const codeCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow analyze code -p ./src', description: 'Analyze source directory' },
    { command: 'hive-flow analyze code --type complexity', description: 'Run complexity analysis' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const astCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow analyze ast src/', description: 'Analyze all files in src/' },
    { command: 'hive-flow analyze ast src/index.ts --complexity', description: 'Analyze with complexity' },
    { command: 'hive-flow analyze ast . --format json', description: 'JSON output' },
    { command: 'hive-flow analyze ast src/ --symbols', description: 'Extract symbols' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const complexityAstCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow analyze complexity src/', description: 'Analyze complexity' },
    { command: 'hive-flow analyze complexity src/ --threshold 15', description: 'Flag high complexity' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetPath = ctx.args[0] || ctx.cwd;
...
const symbolsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow analyze symbols src/', description: 'Extract all symbols' },
    { command: 'hive-flow analyze symbols src/ --type function', description: 'Only functions' },
    { command: 'hive-flow analyze symbols src/ --format json', description: 'JSON output' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetPath = ctx.args[0] || ctx.cwd;
...
const importsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow analyze imports src/', description: 'Analyze all imports' },
    { command: 'hive-flow analyze imports src/ --external', description: 'Only npm packages' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetPath = ctx.args[0] || ctx.cwd;
    const formatType = (ctx.flags.format as string) || 'text';
...
const depsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow analyze deps --outdated', description: 'Show outdated dependencies' },
    { command: 'hive-flow analyze deps --security', description: 'Check for vulnerabilities' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const boundariesCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow analyze boundaries src/', description: 'Find code boundaries in src/' },
    { command: 'hive-flow analyze boundaries -p 3 src/', description: 'Find 3 partitions' },
    { command: 'hive-flow analyze boundaries -f dot -o graph.dot src/', description: 'Export to DOT format' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const modulesCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow analyze modules src/', description: 'Detect module communities' },
    { command: 'hive-flow analyze modules -f dot -o modules.dot src/', description: 'Export colored DOT graph' },
    { command: 'hive-flow analyze modules -m 3 src/', description: 'Only show communities with 3+ files' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetDir = ctx.args[0] || ctx.cwd;
    const outputFile = ctx.flags.output as string | undefined;
...
const dependenciesCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow analyze dependencies src/', description: 'Build dependency graph' },
    { command: 'hive-flow analyze dependencies -f dot -o deps.dot src/', description: 'Export to DOT' },
    { command: 'hive-flow analyze dependencies -i .ts,.tsx src/', description: 'Only TypeScript files' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetDir = ctx.args[0] || ctx.cwd;
    const outputFile = ctx.flags.output as string | undefined;
...
const circularCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow analyze circular src/', description: 'Find circular dependencies' },
    { command: 'hive-flow analyze circular -s high src/', description: 'Only high severity cycles' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetDir = ctx.args[0] || ctx.cwd;
    const outputFile = ctx.flags.output as string | undefined;
...
export const analyzeCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow analyze ast src/', description: 'Analyze code with AST parsing' },
    { command: 'hive-flow analyze complexity src/ --threshold 15', description: 'Find high-complexity files' },
    { command: 'hive-flow analyze symbols src/ --type function', description: 'Extract all functions' },
    { command: 'hive-flow analyze imports src/ --external', description: 'List npm dependencies' },
    { command: 'hive-flow analyze diff --risk', description: 'Analyze diff with risk assessment' },
    { command: 'hive-flow analyze boundaries src/', description: 'Find code boundaries using MinCut' },
    { command: 'hive-flow analyze modules src/', description: 'Detect module communities with Louvain' },
    { command: 'hive-flow analyze dependencies src/ --format dot', description: 'Export dependency graph as DOT' },
    { command: 'hive-flow analyze circular src/', description: 'Find circular dependencies' },
    { command: 'hive-flow analyze deps --security', description: 'Check dependency vulnerabilities' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(`  ${output.dim('hive-flow analyze ast src/')}                  # Full AST analysis`);
    output.writeln(`  ${output.dim('hive-flow analyze ast src/index.ts -c')}       # Include complexity`);
    output.writeln(`  ${output.dim('hive-flow analyze complexity src/ -t 15')}     # Flag high complexity`);
    output.writeln(`  ${output.dim('hive-flow analyze symbols src/ --type fn')}    # Extract functions`);
    output.writeln(`  ${output.dim('hive-flow analyze imports src/ --external')}   # Only npm imports`);
    output.writeln();

    output.writeln(output.bold('Graph Analysis Examples:'));
    output.writeln();
    output.writeln(`  ${output.dim('hive-flow analyze boundaries src/')}            # Find natural code boundaries`);
    output.writeln(`  ${output.dim('hive-flow analyze modules src/')}               # Detect module communities`);
    output.writeln(`  ${output.dim('hive-flow analyze dependencies -f dot src/')}   # Export to DOT format`);
    output.writeln(`  ${output.dim('hive-flow analyze circular src/')}              # Find circular deps`);
    output.writeln();

    output.writeln(output.bold('Diff Analysis Examples:'));
    output.writeln();
    output.writeln(`  ${output.dim('hive-flow analyze diff --risk')}              # Risk assessment`);
    output.writeln(`  ${output.dim('hive-flow analyze diff HEAD~1 --classify')}   # Classify changes`);
    output.writeln(`  ${output.dim('hive-flow analyze diff main..feature')}       # Compare branches`);
    output.writeln();

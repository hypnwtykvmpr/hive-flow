...
const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow performance benchmark -s neural', description: 'Benchmark neural operations' },
    { command: 'hive-flow performance benchmark -i 1000', description: 'Run with 1000 iterations' },
  ],
...
const profileCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow performance profile -t cpu', description: 'Profile CPU usage' },
    { command: 'hive-flow performance profile -d 60', description: 'Profile for 60 seconds' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const metricsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow performance metrics -t 7d', description: 'Show 7-day metrics' },
    { command: 'hive-flow performance metrics -f prometheus', description: 'Export as Prometheus format' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (format === 'prometheus') {
      output.writeln(`# HELP hive_flow_heap_used_bytes Heap memory used`);
      output.writeln(`hive_flow_heap_used_bytes ${memUsage.heapUsed}`);
      output.writeln(`# HELP hive_flow_heap_total_bytes Total heap memory`);
      output.writeln(`hive_flow_heap_total_bytes ${memUsage.heapTotal}`);
      output.writeln(`# HELP hive_flow_rss_bytes Resident set size`);
      output.writeln(`hive_flow_rss_bytes ${memUsage.rss}`);
      output.writeln(`# HELP hive_flow_cpu_user_microseconds CPU user time`);
      output.writeln(`hive_flow_cpu_user_microseconds ${cpuUsage.user}`);
      output.writeln(`# HELP hive_flow_cpu_system_microseconds CPU system time`);
      output.writeln(`hive_flow_cpu_system_microseconds ${cpuUsage.system}`);
      output.writeln(`# HELP hive_flow_cache_entries Embedding cache entries`);
      output.writeln(`hive_flow_cache_entries ${cacheEntries}`);
      output.writeln(`# HELP hive_flow_hnsw_entries HNSW index entries`);
      output.writeln(`hive_flow_hnsw_entries ${hnswEntries}`);
      output.writeln(`# HELP hive_flow_uptime_seconds Process uptime`);
      output.writeln(`hive_flow_uptime_seconds ${uptime}`);
      return { success: true };
    }
...
const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow performance optimize -t memory', description: 'Optimize memory usage' },
    { command: 'hive-flow performance optimize --apply', description: 'Apply all optimizations' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const bottleneckCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow performance bottleneck', description: 'Find bottlenecks' },
    { command: 'hive-flow performance bottleneck -d full', description: 'Full analysis' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
...
export const performanceCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow performance benchmark', description: 'Run benchmarks' },
    { command: 'hive-flow performance profile', description: 'Profile application' },
    { command: 'hive-flow perf metrics', description: 'View metrics (alias)' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Hive Flow Performance Suite'));

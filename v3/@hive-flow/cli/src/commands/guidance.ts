...
const compileCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow guidance compile', description: 'Compile default CLAUDE.md' },
    { command: 'hive-flow guidance compile -r ./CLAUDE.md -l ./CLAUDE.local.md', description: 'Compile with local overlay' },
    { command: 'hive-flow guidance compile --json', description: 'Output compiled bundle as JSON' },
  ],
...
const retrieveCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow guidance retrieve -t "Fix SQL injection in user search"', description: 'Retrieve guidance for a security task' },
    { command: 'hive-flow guidance retrieve -t "Add unit tests" -n 3', description: 'Retrieve top 3 shards for testing' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const gatesCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow guidance gates -c "rm -rf /tmp"', description: 'Check if a command is destructive' },
    { command: 'hive-flow guidance gates --content "api_key=sk-abc123..."', description: 'Check content for secrets' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const statusData = {
        ...
        dataDir: existsSync('./.hive-flow/guidance') ? 'exists' : 'not created',
      };
...
const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow guidance optimize', description: 'Analyze current CLAUDE.md and show suggestions' },
    { command: 'hive-flow guidance optimize --apply', description: 'Apply optimizations to CLAUDE.md' },
    { command: 'hive-flow guidance optimize -s compact --apply', description: 'Optimize for compact context window' },
    { command: 'hive-flow guidance optimize --target-score 95', description: 'Optimize until score reaches 95' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const rootPath = ctx.flags.root as string || './CLAUDE.md';
    const localPath = ctx.flags.local as string | undefined;
...
const abTestCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow guidance ab-test', description: 'Run default A/B test (no guidance vs ./CLAUDE.md)' },
    { command: 'hive-flow guidance ab-test -a old.md -b new.md', description: 'Compare two CLAUDE.md versions' },
    { command: 'hive-flow guidance ab-test --tasks custom-tasks.json', description: 'Run with custom test tasks' },
    { command: 'hive-flow guidance ab-test --json', description: 'Output full report as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const guidanceCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'hive-flow guidance compile', description: 'Compile CLAUDE.md into policy bundle' },
    { command: 'hive-flow guidance retrieve -t "Fix auth bug"', description: 'Retrieve relevant guidance' },
    { command: 'hive-flow guidance gates -c "rm -rf /"', description: 'Check enforcement gates' },
    { command: 'hive-flow guidance status', description: 'Show control plane status' },
    { command: 'hive-flow guidance optimize', description: 'Analyze and optimize CLAUDE.md' },
    { command: 'hive-flow guidance ab-test', description: 'Run A/B behavioral comparison' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    ...
    output.writeln();
    output.writeln(output.dim('Use hive-flow guidance <subcommand> --help for details'));

...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow memory search -q "authentication patterns"', description: 'Semantic search' },
    { command: 'hive-flow memory search -q "JWT" -t keyword', description: 'Keyword search' },
    { command: 'hive-flow memory search -q "test" --build-hnsw', description: 'Build HNSW index and search' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (results.length === 0) {
        ...
        output.writeln(output.dim('Try: hive-flow memory store -k "key" --value "data"'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (entries.length === 0) {
        ...
        output.printInfo('Store data: hive-flow memory store -k "key" --value "data"');
        return { success: true, data: [] };
      }
...
const deleteCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow memory delete -k "mykey"', description: 'Delete entry with default namespace' },
    { command: 'hive-flow memory delete -k "lesson" -n "lessons"', description: 'Delete entry from specific namespace' },
    { command: 'hive-flow memory delete mykey -f', description: 'Delete without confirmation' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const cleanupCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow memory cleanup --dry-run', description: 'Preview cleanup' },
    { command: 'hive-flow memory cleanup --older-than 30d', description: 'Delete entries older than 30 days' },
    { command: 'hive-flow memory cleanup --expired-only', description: 'Clean expired entries' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const exportCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow memory export -o ./backup.json', description: 'Export all to JSON' },
    { command: 'hive-flow memory export -o ./data.csv -f csv', description: 'Export to CSV' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const importCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow memory import -i ./backup.json', description: 'Import from file' },
    { command: 'hive-flow memory import -i ./data.json -n archive', description: 'Import to namespace' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const initMemoryCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printList([
        `Store data: ${output.highlight('hive-flow memory store -k "key" --value "data"')}`,
        `Search: ${output.highlight('hive-flow memory search -q "query"')}`,
        `Train patterns: ${output.highlight('hive-flow neural train -p coordination')}`,
        `View stats: ${output.highlight('hive-flow memory stats')}`
      ]);
...
export const memoryCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: hive-flow memory <subcommand> [options]');
    output.writeln();

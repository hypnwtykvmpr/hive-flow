...
export const ruvectorCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow ruvector setup', description: 'Output Docker files and SQL for setup' },
    { command: 'hive-flow ruvector import --input memory.json', description: 'Import from sql.js/JSON export' },
    { command: 'hive-flow ruvector init --database mydb', description: 'Initialize RuVector in PostgreSQL' },
    { command: 'hive-flow ruvector status --verbose', description: 'Check connection and schema status' },
    { command: 'hive-flow ruvector migrate --up', description: 'Run pending migrations' },
    { command: 'hive-flow ruvector benchmark --vectors 10000', description: 'Run performance benchmark' },
    { command: 'hive-flow ruvector optimize --analyze', description: 'Analyze and suggest optimizations' },
    { command: 'hive-flow ruvector backup --output backup.sql', description: 'Backup RuVector data' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.printBox([
      'RuVector provides PostgreSQL integration for Hive Flow with:',
      '',
    ...
    output.writeln();
    output.printInfo('Run `hive-flow ruvector <command> --help` for details');
    output.writeln();

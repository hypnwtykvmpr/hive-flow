...
export const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow ruvector optimize --analyze', description: 'Analyze and show recommendations' },
    { command: 'hive-flow ruvector optimize --apply', description: 'Apply optimizations' },
    { command: 'hive-flow ruvector optimize --vacuum', description: 'Run VACUUM ANALYZE' },
    { command: 'hive-flow ruvector optimize --reindex', description: 'Rebuild all indexes' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printBox([
        ...
        '',
        'Quick commands:',
        `  hive-flow ruvector optimize --vacuum    # Clean up tables`,
        `  hive-flow ruvector optimize --reindex  # Rebuild indexes`,
        `  hive-flow ruvector optimize --apply    # Apply critical fixes`,

...
export const initCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow ruvector init -d mydb', description: 'Initialize with database name' },
    { command: 'hive-flow ruvector init -d mydb -h db.example.com --ssl', description: 'Remote with SSL' },
    { command: 'hive-flow ruvector init -d mydb --force', description: 'Force re-initialization' },
    { command: 'hive-flow ruvector init -d mydb --dimensions 768', description: 'Custom vector dimensions' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printBox([
        ...
        '',
        'Next steps:',
        '  1. Run migrations: hive-flow ruvector migrate --up',
        '  2. Check status: hive-flow ruvector status --verbose',
        '  3. Run benchmark: hive-flow ruvector benchmark',

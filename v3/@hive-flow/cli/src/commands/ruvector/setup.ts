 ...
 *
 * Usage:
 *   npx hive-flow ruvector setup              # Output to ./ruvector-postgres/
 *   npx hive-flow ruvector setup --output /path/to/dir
 *   npx hive-flow ruvector setup --print      # Print to stdout only
 *
...
services:
  postgres:
    ...
    environment:
      ...
      POSTGRES_PASSWORD: hive-flow-test
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U claude -d hive_flow"]
      interval: 5s
      timeout: 5s
      retries: 10
    command: >
  ...
  pgadmin:
    ...
    environment:
      PGADMIN_DEFAULT_EMAIL: admin@hive-flow.local
...
-- from ruvnet/ruvector-postgres with Hive-Flow V3 integration.
--
...
-- Create the hive_flow schema
CREATE SCHEMA IF NOT EXISTS hive_flow;

-- Grant permissions
GRANT ALL ON SCHEMA hive_flow TO claude;

-- Set search path
SET search_path TO hive_flow, public;
...
CREATE TABLE IF NOT EXISTS hive_flow.embeddings (
...
CREATE TABLE IF NOT EXISTS hive_flow.patterns (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS hive_flow.agents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS hive_flow.trajectories (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Memory entries table (main storage for Hive-Flow memory)
CREATE TABLE IF NOT EXISTS hive_flow.memory_entries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS hive_flow.hyperbolic_embeddings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    content TEXT NOT NULL,
    ...
    parent_id UUID REFERENCES hive_flow.hyperbolic_embeddings(id),
    metadata JSONB DEFAULT '{}',
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Graph nodes for GNN operations
CREATE TABLE IF NOT EXISTS hive_flow.graph_nodes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS hive_flow.graph_edges (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    source_id UUID REFERENCES hive_flow.graph_nodes(id),
    target_id UUID REFERENCES hive_flow.graph_nodes(id),
...
ON hive_flow.graph_nodes
USING hnsw (embedding ruvector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- Additional indices for common queries
CREATE INDEX IF NOT EXISTS idx_embeddings_namespace ON hive_flow.embeddings(namespace);
CREATE INDEX IF NOT EXISTS idx_memory_entries_namespace ON hive_flow.memory_entries(namespace);
CREATE INDEX IF NOT EXISTS idx_memory_entries_key ON hive_flow.memory_entries(key);
...
CREATE OR REPLACE FUNCTION hive_flow.search_similar(
...
CREATE OR REPLACE FUNCTION hive_flow.search_memory(
    query_embedding ruvector(384),
...
BEGIN
    ...
    SELECT
        ...
        m.metadata
    FROM hive_flow.memory_entries m
...
CREATE OR REPLACE FUNCTION hive_flow.search_patterns(
    query_embedding ruvector(384),
...
CREATE OR REPLACE FUNCTION hive_flow.find_agents(
    query_embedding ruvector(384),
...
CREATE OR REPLACE FUNCTION hive_flow.to_poincare(
    euclidean real[],
    curvature FLOAT DEFAULT -1.0
)
RETURNS real[] AS $$
...
CREATE OR REPLACE FUNCTION hive_flow.hyperbolic_search(
    query ruvector(384),
    limit_count INT DEFAULT 10,
    curvature FLOAT DEFAULT -1.0
)
...
BEGIN
    ...
    query_poincare := hive_flow.to_poincare(query_arr, curvature);
    ...
    SELECT
        ...
        he.hierarchy_level,
        he.metadata
    FROM hive_flow.hyperbolic_embeddings he
...
CREATE OR REPLACE FUNCTION hive_flow.ruvector_info()
RETURNS TABLE (
    version TEXT,
    simd_info TEXT
) AS $$
...
CREATE OR REPLACE FUNCTION hive_flow.cosine_similarity(
    a ruvector,
    b ruvector
)
RETURNS FLOAT AS $$
...
-- L2 distance helper
CREATE OR REPLACE FUNCTION hive_flow.l2_distance(
    a ruvector,
    b ruvector
)
RETURNS FLOAT AS $$
...
-- Upsert memory entry
CREATE OR REPLACE FUNCTION hive_flow.upsert_memory(
    p_key VARCHAR(255),
    p_value TEXT,
...
DECLARE
    v_id UUID;
BEGIN
    INSERT INTO hive_flow.memory_entries (key, value, embedding, namespace, metadata, ttl, updated_at)
    VALUES (p_key, p_value, p_embedding, p_namespace, p_metadata, p_ttl, NOW())
    ON CONFLICT (key, namespace) DO UPDATE SET
        value = EXCLUDED.value,
        embedding = COALESCE(EXCLUDED.embedding, hive_flow.memory_entries.embedding),
...
BEGIN
    ...
    RAISE NOTICE '  - hive_flow.search_similar(embedding, limit, min_sim)';
    RAISE NOTICE '  - hive_flow.search_memory(embedding, namespace, limit)';
    RAISE NOTICE '  - hive_flow.search_patterns(embedding, type, limit)';
    RAISE NOTICE '  - hive_flow.find_agents(embedding, type, limit)';
    RAISE NOTICE '  - hive_flow.hyperbolic_search(embedding, limit, curvature)';
    RAISE NOTICE '  - hive_flow.upsert_memory(key, value, embedding, namespace)';
    RAISE NOTICE '';
...
This directory contains the Docker configuration for RuVector PostgreSQL with Hive-Flow V3.
...
docker exec ruvector-postgres psql -U claude -d hive_flow -c "SELECT ruvector_version();"
\`\`\`
...
\`\`\`bash
# Export current Hive-Flow memory
npx hive-flow memory list --format json > memory-export.json

# Import to RuVector PostgreSQL
npx hive-flow ruvector import --input memory-export.json
\`\`\`
...
export const setupCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow ruvector setup', description: 'Output files to ./ruvector-postgres/' },
    { command: 'hive-flow ruvector setup --output /path/to/dir', description: 'Output to custom directory' },
    { command: 'hive-flow ruvector setup --print', description: 'Print files to stdout' },
    { command: 'hive-flow ruvector setup --force', description: 'Overwrite existing files' },
  ],

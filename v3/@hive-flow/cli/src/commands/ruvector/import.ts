 ...
 *   npx hive-flow ruvector import --from-memory
 *   npx hive-flow ruvector import --input data.json --batch-size 100
 *
...
function generateInsertSQL(entry: MemoryEntry): string {
  ...
  return `INSERT INTO hive_flow.memory_entries (key, value, embedding, namespace, metadata, created_at, updated_at)
VALUES (
  '${key}',
  '${value}',
  ${embeddingClause},
...
export const importCommand: Command = {
  ...
  options: [
    ...
    {
      name: 'from-memory',
      description: 'Export from current Hive-Flow memory and import',
      type: 'boolean',
      default: false,
    },
    {
  ...
  examples: [
    { command: 'hive-flow ruvector import --input memory-export.json', description: 'Import from JSON file' },
    { command: 'hive-flow ruvector import --input data.json --output import.sql', description: 'Generate SQL file (dry-run)' },
    { command: 'hive-flow ruvector import --from-memory', description: 'Export current memory and import' },
    { command: 'hive-flow ruvector import --input data.json --container my-postgres', description: 'Import using custom container' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!inputFile && !fromMemory) {
      ...
      output.writeln('  hive-flow ruvector import --input memory-export.json');
      output.writeln('  hive-flow ruvector import --from-memory');
    ...
    if (fromMemory) {
      output.printInfo('Exporting from current Hive-Flow memory...');
      output.printWarning('Note: Run "npx hive-flow memory list --format json > memory-export.json" first');
      output.printInfo('Then use: npx hive-flow ruvector import --input memory-export.json');
    ...
    if (outputFile) {
      try {
        ...
        output.writeln(`  docker exec -i ${containerName} psql -U claude -d hive_flow < ${outputFile}`);
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
      ...
      try {
        ...
        output.writeln(output.dim(`  docker exec -i ${containerName} psql -U claude -d hive_flow < ${tempFile}`));
        output.writeln();
        ...
        try {
          ...
          output.writeln(`  docker exec -i ${containerName} psql -U claude -d hive_flow < ${tempFile}`);
    ...
    output.writeln(`  docker exec ${containerName} psql -U claude -d hive_flow -c "SELECT COUNT(*) FROM hive_flow.memory_entries;"`);
    output.writeln();

...
function getPackageVersion(): string {
  try {
    ...
    return pkg.version || '3.0.0-alpha';
  } catch {
    return '3.0.0-alpha';
  }
}
...
export const systemTools: MCPTool[] = [
  ...
  },
  {
    ...
    handler: async () => {
      ...
      const transport = process.env.HIVE_FLOW_MCP_TRANSPORT || (isStdio ? 'stdio' : 'http');
      const port = parseInt(process.env.HIVE_FLOW_MCP_PORT || '3000', 10);
      ...
      const host = process.env.HIVE_FLOW_MCP_HOST || 'localhost';
      try {
        ...
        return { running: connected, transport, port, host };
      } catch {
        return { running: false, transport, port, host };
      }
    },
  },
  {
    ...
    handler: async () => {
      const storePath = join(process.cwd(), '.hive-flow', 'tasks', 'store.json');

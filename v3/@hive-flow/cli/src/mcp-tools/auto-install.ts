...
export async function autoInstallPackage(
  ...
  if (!validPackageName.test(packageName)) {
    if (!silent) {
      console.error(`[hive-flow] Invalid package name: ${packageName}`);
    }
    return false;
  }
  ...
  try {
    if (!silent) {
      console.error(`[hive-flow] Auto-installing ${packageName}...`);
    }
    ...
    if (!silent) {
      console.error(`[hive-flow] Successfully installed ${packageName}`);
    }
    return true;
  } catch (error) {
    if (!silent) {
      console.error(`[hive-flow] Failed to auto-install ${packageName}: ${error}`);
    }
    return false;
  }
}
  ...
  try {
    ...
    if (installed) {
      try {
        ...
        console.error(`[hive-flow] ${packageName} installed but failed to load. Restart MCP server.`);
        return null;
      }
    }
    return null;

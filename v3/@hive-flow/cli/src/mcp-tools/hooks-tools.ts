...
const KEYWORD_PATTERNS: Record<string, { agents: string[]; confidence: number }> = {
  ...
  'bug': { agents: ['bug-hunter', 'coder', 'tester', 'reviewer'], confidence: 0.88 },
  'fix': { agents: ['bug-hunter', 'coder', 'tester', 'reviewer'], confidence: 0.85 },
  'debug': { agents: ['bug-hunter', 'coder', 'tester'], confidence: 0.90 },
...
export const hooksPatternSearch: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    return {
      ...
      searchTimeMs: 0,
      backend: 'unavailable',
      note: 'Real vector search not available. Initialize memory database with: hive-flow memory init',
    };
  },
};
...
export const hooksModelRoute: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    if (!router) {
      ...
      const fallbackModel = complexity > 0.7 ? 'opus' : complexity > 0.4 ? 'sonnet' : 'sonnet';
      return {
        model: fallbackModel,
        confidence: 0.7,
        complexity,

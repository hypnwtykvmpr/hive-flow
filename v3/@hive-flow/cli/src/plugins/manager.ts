...
export class PluginManager {
  ...
  constructor(baseDir: string = process.cwd()) {
    const pluginsDir = path.join(baseDir, '.hive-flow', 'plugins');
    this.config = {
      pluginsDir,
  ...
  async installFromNpm(
    ...
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        // Check for hive-flow plugin metadata
        if (pkg['hive-flow']) {
          commands = pkg['hive-flow'].commands || [];
          hooks = pkg['hive-flow'].hooks || [];
        }
      }
  ...
  async upgrade(
    ...
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        existing.commands = pkg['hive-flow']?.commands || existing.commands;
        existing.hooks = pkg['hive-flow']?.hooks || existing.hooks;
      }

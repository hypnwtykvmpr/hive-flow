...
function extractNodeLiteralMutationTargets(code: string): string[] {
  ...
  for (const match of inner.matchAll(/\bfs(?:\.promises)?\.(?:writeFileSync|writeFile|appendFileSync|appendFile|createWriteStream|unlinkSync|unlink|rmSync|rm|rmdirSync|rmdir)\s*\(\s*(['"])([^'"]+)\1/g)) {

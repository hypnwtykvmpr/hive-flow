 ...
 *
 * The encrypted file lives at ~/.hive-flow/permission-guard/privkey.enc.
...
export async function requestOverride(): Promise<{ granted: boolean; expiresAt: number }> {
  if (!existsSync(PUBKEY_PATH)) {
    process.stderr.write('[permission-guard] Override not set up. Run: hive-flow permission-guard setup\n');

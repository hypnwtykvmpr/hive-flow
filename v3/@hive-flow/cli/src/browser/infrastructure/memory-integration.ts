...
// ============================================================================
// Hive Flow Memory Adapter
// ============================================================================

/**
 * Adapter for hive-flow memory system with HNSW indexing
 */
export class HiveFlowMemoryAdapter implements IMemoryAdapter {
  ...
  async store(entry: BrowserMemoryEntry): Promise<void> {
    ...
    try {
      // This would call hive-flow memory_store MCP tool
...
export class BrowserMemoryManager {
  ...
  constructor(sessionId: string, adapter?: IMemoryAdapter) {
    ...
    this.adapter = adapter || new HiveFlowMemoryAdapter();
  }
...
export function getMemoryAdapter(): IMemoryAdapter {
  if (!defaultAdapter) {
    defaultAdapter = new HiveFlowMemoryAdapter();
  }

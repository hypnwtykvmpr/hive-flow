...
/**
 * Search HNSW index (fast than brute-force)
...
export async function storeEntry(options: {
  ...
  try {
    if (!fs.existsSync(dbPath)) {
      return { success: false, id: '', error: 'Database not initialized. Run: hive-flow memory init' };
    }
 ...
 * Uses HNSW index for fast search when available
 */

 ...
 * Uses .hive-flow/neural in the current working directory,
 * falling back to home directory
 */
function getDataDir(): string {
  const cwd = process.cwd();
  const localDir = join(cwd, '.hive-flow', 'neural');
  const homeDir = join(homedir(), '.hive-flow', 'neural');

  // Prefer local directory if .hive-flow exists
  if (existsSync(join(cwd, '.hive-flow'))) {
    return localDir;
  }
...
export async function recordStep(step: TrajectoryStep): Promise<boolean> {
  ...
  try {
    ...
    // ADR-053: Try HiveMemory v3 bridge embedder first

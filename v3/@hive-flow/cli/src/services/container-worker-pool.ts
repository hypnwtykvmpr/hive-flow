...
const DEFAULT_CONFIG: ContainerPoolConfig = {
  maxContainers: 3,
  minContainers: 1,
  image: 'ghcr.io/hypnwtk/hive-flow-headless:latest',
  resources: {
    cpus: '2',
    memory: '4g',
  },
  ...
  statePath: '.hive-flow/container-pool',
...
export class ContainerWorkerPool extends EventEmitter {
  ...
  private async createContainer(): Promise<ContainerInfo | null> {
    ...
    const name = `hive-flow-worker-${id}`;
    ...
    try {
      ...
      const args = [
        ...
        '-v', `${join(this.projectRoot, this.config.statePath)}:/root/.hive-flow`,
  ...
  private buildWorkerCommand(options: ContainerExecutionOptions): string[] {
    // Use npx to run hive-flow daemon trigger
    return [
      'npx', 'hive-flow@v3alpha',
      'daemon', 'trigger',

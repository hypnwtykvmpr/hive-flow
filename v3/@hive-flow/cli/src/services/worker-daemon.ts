...
export class WorkerDaemon extends EventEmitter {
  ...
  constructor(projectRoot: string, config?: Partial<DaemonConfig>) {
    ...
    const hiveFlowDir = join(projectRoot, '.hive-flow');

    this.config = {
      autoStart: config?.autoStart ?? false, // P1 fix: Default to false for explicit consent
      logDir: config?.logDir ?? join(hiveFlowDir, 'logs'),
      stateFile: config?.stateFile ?? join(hiveFlowDir, 'daemon-state.json'),
    ...
    if (!existsSync(hiveFlowDir)) {
      mkdirSync(hiveFlowDir, { recursive: true });
    }
  ...
  private async runMapWorker(): Promise<unknown> {
    ...
    const metricsFile = join(this.projectRoot, '.hive-flow', 'metrics', 'codebase-map.json');
    const metricsDir = join(this.projectRoot, '.hive-flow', 'metrics');
    ...
    const map = {
      ...
      structure: {
        ...
        hasHiveFlow: existsSync(join(this.projectRoot, '.hive-flow')),
      },
      scannedAt: Date.now(),
    };
  ...
  private async runAuditWorkerLocal(): Promise<unknown> {
    ...
    const auditFile = join(this.projectRoot, '.hive-flow', 'metrics', 'security-audit.json');
    const metricsDir = join(this.projectRoot, '.hive-flow', 'metrics');
  ...
  private async runOptimizeWorkerLocal(): Promise<unknown> {
    ...
    const optimizeFile = join(this.projectRoot, '.hive-flow', 'metrics', 'performance.json');
    const metricsDir = join(this.projectRoot, '.hive-flow', 'metrics');
  ...
  private async runConsolidateWorker(): Promise<unknown> {
    ...
    const consolidateFile = join(this.projectRoot, '.hive-flow', 'metrics', 'consolidation.json');
    const metricsDir = join(this.projectRoot, '.hive-flow', 'metrics');
  ...
  private async runTestGapsWorkerLocal(): Promise<unknown> {
    ...
    const testGapsFile = join(this.projectRoot, '.hive-flow', 'metrics', 'test-gaps.json');
    const metricsDir = join(this.projectRoot, '.hive-flow', 'metrics');
  ...
  private async runBenchmarkWorkerLocal(): Promise<unknown> {
    const benchmarkFile = join(this.projectRoot, '.hive-flow', 'metrics', 'benchmark.json');
    const metricsDir = join(this.projectRoot, '.hive-flow', 'metrics');

...
export function generatePreCommitHook(): string {
  return `#!/bin/bash
# Hive Flow Pre-Commit Hook
# Validates code quality before commit

set -e

echo "🔍 Running Hive Flow pre-commit checks..."
...
const MEMORY_DIR = path.join(process.cwd(), '.hive-flow', 'data');
...
export function generateHookHandler(): string {
  ...
  const lines = [
    ...
    '',
    "  'pre-bash': () => {",
    '    // PreToolUse Bash hook — MUST emit valid JSON with a permissionDecision.',
    '    // Non-JSON stdout is treated as a hard block by Claude Code. Read the',
    '    // command from the tool payload on stdin, falling back to env/args.',
    "    var cmdRaw = '';",
    '    try {',
    "      var raw = fs.readFileSync(0, 'utf8').trim();",
    '      if (raw) {',
    ...
    '        }',
    '      }',
    '    } catch (e) { /* no stdin / not JSON — fall back to env/args */ }',
    '    if (!cmdRaw) cmdRaw = prompt;',
    '    var cmd = String(cmdRaw).toLowerCase();',
    "    var dangerous = ['rm -rf /', 'format c:', 'del /s /q c:\\\\', ':(){:|:&};:'];",
    '    for (var i = 0; i < dangerous.length; i++) {',
    '      if (cmd.includes(dangerous[i])) {',
    "        console.log(JSON.stringify({ hookSpecificOutput: { hookEventName: 'PreToolUse', permissionDecision: 'deny', permissionDecisionReason: '[BLOCKED] Dangerous command detected: ' + dangerous[i] } }));",
    '        return;',
    '      }',
    '    }',
...
export function generateIntelligenceStub(): string {
  const lines = [
    ...
    "const os = require('os');",
    '',
    "const DATA_DIR = path.join(process.cwd(), '.hive-flow', 'data');",
    ...
    '    path.join(process.cwd(), ".hive-flow", "memory"),',
...
export function generateWindowsDaemonManager(): string {
  return `# Hive Flow V3 Daemon Manager for Windows
...
$HiveFlowDir = Join-Path $PWD '.hive-flow'
$PidDir = Join-Path $HiveFlowDir 'pids'
...
function Start-SwarmMonitor {
    ...
    $process = Start-Process -FilePath 'node' -ArgumentList @(
        '-e',
        'setInterval(() => { require("fs").writeFileSync(".hive-flow/metrics/swarm-activity.json", JSON.stringify({swarm:{active:true,agent_count:0},timestamp:Date.now()})) }, 5000)'
...
function Show-Status {
    Write-Host ""
    Write-Host "Hive Flow V3 Daemon Status" -ForegroundColor Cyan
...
export function generateWindowsBatchWrapper(): string {
  return `@echo off
REM Hive Flow V3 - Windows Batch Wrapper
...
export function generateCrossPlatformSessionManager(): string {
  return `#!/usr/bin/env node
/**
 * Hive Flow Cross-Platform Session Manager
...
function getDataDir() {
  const localDir = path.join(process.cwd(), '.hive-flow', 'sessions');
  ...
  switch (platform) {
    case 'win32':
      return path.join(process.env.APPDATA || homeDir, 'hive-flow', 'sessions');
    case 'darwin':
      return path.join(homeDir, 'Library', 'Application Support', 'hive-flow', 'sessions');
    default:
      return path.join(homeDir, '.hive-flow', 'sessions');
  }
}

...
const COMMANDS_MAP: Record<string, string[]> = {
  core: ['hive-flow-help.md', 'hive-flow-swarm.md', 'hive-flow-memory.md'],
  analysis: ['analysis'],
...
function mergeSettingsForUpgrade(existing: Record<string, unknown>): Record<string, unknown> {
  ...
  // Agent Teams hook config lives in hiveFlow.agentTeams.hooks instead.
  ...
  merged.env = {
    ...
    HIVE_FLOW_V3_ENABLED: existingEnv.HIVE_FLOW_V3_ENABLED || 'true',
    HIVE_FLOW_HOOKS_ENABLED: existingEnv.HIVE_FLOW_HOOKS_ENABLED || 'true',
  };
  ...
  // 4. Merge hiveFlow settings (preserve existing, add agentTeams + memory)
  const existingHiveFlow = (existing.hiveFlow as Record<string, unknown>) || {};
  const existingMemory = (existingHiveFlow.memory as Record<string, unknown>) || {};
  merged.hiveFlow = {
    ...existingHiveFlow,
    version: existingHiveFlow.version || '3.0.0',
    enabled: existingHiveFlow.enabled !== false,
    agentTeams: {
      enabled: true,
      teammateMode: 'auto',
...
export async function executeUpgrade(targetDir: string, upgradeSettings = false): Promise<UpgradeResult> {
  ...
  try {
    ...
    const metricsDir = path.join(targetDir, '.hive-flow', 'metrics');
    const securityDir = path.join(targetDir, '.hive-flow', 'security');
    ...
    if (!fs.existsSync(progressPath)) {
      ...
      result.created.push('.hive-flow/metrics/v3-progress.json');
    } else {
      result.preserved.push('.hive-flow/metrics/v3-progress.json');
    }
    ...
    if (!fs.existsSync(activityPath)) {
      ...
      result.created.push('.hive-flow/metrics/swarm-activity.json');
    } else {
      result.preserved.push('.hive-flow/metrics/swarm-activity.json');
    }
    ...
    if (!fs.existsSync(learningPath)) {
      const learning = {
        ...
        _note: 'Intelligence grows as you use Hive Flow'
      };
      fs.writeFileSync(learningPath, JSON.stringify(learning, null, 2), 'utf-8');
      result.created.push('.hive-flow/metrics/learning.json');
    } else {
      result.preserved.push('.hive-flow/metrics/learning.json');
    }
    ...
    if (!fs.existsSync(auditPath)) {
      ...
      result.preserved.push('.hive-flow/security/audit-status.json');
    }
      ...
      if (fs.existsSync(settingsPath)) {
        try {
          ...
          result.settingsUpdated = [
            ...
            'hiveFlow.agentTeams',
            'hiveFlow.memory (learningBridge, memoryGraph, agentScopes)',
          ];
        } catch (settingsError) {
...
export async function executeUpgradeWithMissing(targetDir: string, upgradeSettings = false): Promise<UpgradeResult> {
  ...
  try {
    ...
    if (process.env.DEBUG || process.env.HIVE_FLOW_DEBUG) {
    ...
    if (sourceSkillsDir) {
      ...
      const debugMode = process.env.DEBUG || process.env.HIVE_FLOW_DEBUG;
      if (debugMode) {
...
/**
 * Write runtime configuration (.hive-flow/)
 */
async function writeRuntimeConfig(
  ...
  const configPath = path.join(targetDir, '.hive-flow', 'config.yaml');
...
`;
  ...
  const gitignore = `# Hive Flow runtime files
data/
logs/
sessions/
neural/
...
async function writeInitialMetrics(
  ...
  const metricsDir = path.join(targetDir, '.hive-flow', 'metrics');
  const learningDir = path.join(targetDir, '.hive-flow', 'learning');
  const securityDir = path.join(targetDir, '.hive-flow', 'security');
  ...
  if (!fs.existsSync(activityPath) || options.force) {
    ...
    fs.writeFileSync(activityPath, JSON.stringify(activity, null, 2), 'utf-8');
    result.created.files.push('.hive-flow/metrics/swarm-activity.json');
  }
  ...
  if (!fs.existsSync(learningPath) || options.force) {
    ...
    fs.writeFileSync(learningPath, JSON.stringify(learning, null, 2), 'utf-8');
    result.created.files.push('.hive-flow/metrics/learning.json');
  }
...
/**
 * Write CAPABILITIES.md - comprehensive overview of all Hive Flow features
 */
async function writeCapabilitiesDoc(
  ...
  const capabilitiesPath = path.join(targetDir, '.hive-flow', 'CAPABILITIES.md');

  if (fs.existsSync(capabilitiesPath) && !options.force) {
    result.skipped.push('.hive-flow/CAPABILITIES.md');
    return;
  }
...
`;
  ...
  result.created.files.push('.hive-flow/CAPABILITIES.md');
}

/**
 * @hive-flow/cli/codex - CodexInitializer
 *
...
/**
 * Bundled skills source directory candidates, relative to this module.
 * Source lives at cli/src/codex while built output lives at cli/dist/src/codex.
 */
const BUNDLED_SKILLS_DIR_CANDIDATES = [
  '../../../../../.agents/skills',
  '../../../../../../.agents/skills',
] as const;

async function resolveBundledSkillsPath(): Promise<string> {
  const here = path.dirname(fileURLToPath(import.meta.url));
  const candidates = BUNDLED_SKILLS_DIR_CANDIDATES.map((relativePath) =>
  ...
  return candidates[0] ?? path.resolve(here, '../../../../../.agents/skills');
}
...
export class CodexInitializer {
  ...
  async initialize(options: CodexInitOptions): Promise<CodexInitResult> {
    ...
    // Resolve bundled skills path (relative to this file's source or built location).
    this.bundledSkillsPath = await resolveBundledSkillsPath();
    ...
    try {
      ...
      if (mcpResult.registered) {
        filesCreated.push('MCP server (hive-flow) registered');
      }
  ...
  /**
   * Register hive-flow as MCP server with Codex
   */
...
*Generated by @hive-flow/cli/codex - Dual platform mode*
`;

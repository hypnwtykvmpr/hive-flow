 ...
 *
 * @module @hive-flow/cli/transfer/storage/gcs
 * @version 3.0.0
 */
...
export function getGCSConfig(): GCSConfig | null {
  ...
  return {
    ...
    prefix: process.env.GCS_PREFIX || 'hive-flow-patterns',
  };
}
...
export async function uploadToGCS(
  ...
  const tempFile = path.join(tempDir, `hive-flow-upload-${Date.now()}.json`);
...
export async function downloadFromGCS(
  ...
  const tempDir = process.env.TMPDIR || '/tmp';
  const tempFile = path.join(tempDir, `hive-flow-download-${Date.now()}.json`);

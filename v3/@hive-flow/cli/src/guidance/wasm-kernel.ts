 ...
 *
 * @module @hive-flow/cli/guidance/wasm-kernel
 */
...
function tryLoadWasm(): Record<string, (...args: unknown[]) => unknown> | null {
  ...
  try {
    ...
    const moduleUrl = new URL('./wasm-pkg/guidance_kernel.js', import.meta.url);
    // Use createRequire for ESM compatibility
    const requireFn = createRequire(import.meta.url);
    wasmModule = requireFn(fileURLToPath(moduleUrl));

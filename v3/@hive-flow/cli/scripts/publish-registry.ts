...
async function generateRegistry(): Promise<PluginRegistry> {
  ...
  for (const pkg of officialPackages) {
    ...
    plugins.push({
      id: pkg,
      name: pkg,
      displayName: pkg.replace('@hive-flow/plugin-', '').replace('@hive-flow/', ''),
      description: `Official Hive Flow plugin: ${pkg}`,
  ...
  return {
    ...
    plugins,
    categories: [
      { id: 'official', name: 'Official', description: 'Official Hive Flow plugins', pluginCount: plugins.length },
    ],
...
async function main() {
  ...
  try {
    const result = await pinToIPFS(registry, 'hive-flow-plugin-registry', jwt);

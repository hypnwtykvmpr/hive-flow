...
hooks:
  pre: |
    ...
    mcp__hive-flow__memory_usage store "swarm:hierarchy:${TASK_ID}" "$(date): Hierarchical coordination started" --namespace=swarm
    # Set up monitoring
  ...
  post: |
    ...
    mcp__hive-flow__memory_usage store "swarm:hierarchy:${TASK_ID}:complete" "$(date): Task completed with $(mcp__hive-flow__swarm_status | jq '.agents.total') agents"
    # Cleanup resources
...
const attentionService = new AttentionService({
  embeddingDim: 384,
  runtime: 'napi' // Flash Attention optimization faster than standard attention
});

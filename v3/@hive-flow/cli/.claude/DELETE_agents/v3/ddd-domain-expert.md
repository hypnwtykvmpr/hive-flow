...
hooks:
  pre: |
    ...
    mcp__hive-flow__memory_search --pattern="ddd:*" --namespace="architecture" --limit=10
    # Load domain context
    mcp__hive-flow__memory_usage --action="retrieve" --namespace="architecture" --key="domain:model"
  post: |
    echo "✅ Domain model analysis complete"
    # Store domain patterns
    mcp__hive-flow__memory_usage --action="store" --namespace="architecture" --key="ddd:analysis:$(date +%s)" --value="$DOMAIN_SUMMARY"
---
...
mcp__hive-flow__memory_search --pattern="ddd:aggregate:*" --namespace="architecture"
```

...
hive-flow hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
hive-flow hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
hive-flow hook session-end --session-id "dev-session" --export-metrics
```
...
```bash
hive-flow hook pre-edit [options]
...
Examples:
  hive-flow hook pre-edit --file "src/auth/login.js"
  hive-flow hook pre-edit -f "config/db.js" --validate-syntax
  hive-flow hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
hive-flow hook pre-bash --command <cmd>
...
Examples:
  hive-flow hook pre-bash -c "rm -rf /tmp/cache"
  hive-flow hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
hive-flow hook pre-task [options]
...
Examples:
  hive-flow hook pre-task --description "Implement user authentication"
  hive-flow hook pre-task -d "Continue API dev" --load-memory
  hive-flow hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
hive-flow hook pre-search --query <query>
...
Examples:
  hive-flow hook pre-search -q "authentication middleware"
```
...
Examples:
  ...
  hive-flow hook post-edit -f "api/auth.js" --memory-key "auth/login"
  hive-flow hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
hive-flow hook post-bash --command <cmd>
...
Examples:
  hive-flow hook post-bash -c "npm test" --update-metrics
```
...
Examples:
  ...
  hive-flow hook post-task -t "api-refactor" --analyze-performance
  hive-flow hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
hive-flow hook post-search --query <query> --results <path>
...
Examples:
  hive-flow hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
hive-flow hook mcp-initialized --swarm-id <id>
...
```bash
hive-flow hook agent-spawned --agent-id <id> --type <type>
...
```bash
hive-flow hook task-orchestrated --task-id <id>
...
```bash
hive-flow hook neural-trained --pattern <name>
...
```bash
hive-flow hook memory-sync --namespace <ns>
...
```bash
hive-flow hook session-start --session-id <id>
...
```bash
hive-flow hook session-restore --session-id <id>
...
Examples:
  hive-flow hook session-restore --session-id "swarm-20241019"
  hive-flow hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
hive-flow hook session-end [options]
...
Examples:
  hive-flow hook session-end --session-id "dev-session-2024"
  hive-flow hook session-end -s "feature-auth" --export-metrics --generate-summary
  hive-flow hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
hive-flow hook notify --message <msg>
...
Examples:
  hive-flow hook notify -m "Task completed" --level info
  hive-flow hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "hive-flow hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "hive-flow hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "hive-flow hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "hive-flow hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "hive-flow hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "hive-flow hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "hive-flow hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "hive-flow hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "hive-flow hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "hive-flow hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "hive-flow hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "hive-flow hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
...
```javascript
// Hook command
hive-flow hook pre-task --description "Build REST API"
...
```javascript
// Hook command
hive-flow hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
hive-flow hook session-end --session-id "dev-2024"
...
for FILE in $FILES; do
  ...
  hive-flow hook pre-edit --file "$FILE" --validate-syntax
  ...
  hive-flow hook post-edit --file "$FILE" --auto-format
done
...
# Run quality checks
hive-flow hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(hive-flow metrics score --format json | jq -r '.truth_score')
...
hive-flow hook session-restore \
...
hive-flow hook session-start --session-id "fullstack-feature"
...
# Backend work
hive-flow hook pre-edit --file "api/profile.js"
# ... implement backend ...
hive-flow hook post-edit \
  --file "api/profile.js" \
...
hive-flow hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
hive-flow hook post-edit \
...
hive-flow hook session-start --session-id "debug-memory-leak"
...
hive-flow hook pre-search --query "EventEmitter"
# ... search executes ...
hive-flow hook post-search \
  --query "EventEmitter" \
  --cache-results
...
hive-flow hook pre-task --description "Analyze code complexity"
# ... analysis ...
hive-flow hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  hive-flow hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  hive-flow hook post-edit \
    --file "$file" \
...
hive-flow hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .hive-flow/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
hive-flow hook validate-config
```
...
- `hive-flow init --hooks` - Initialize hooks system
- `hive-flow hook --list` - List available hooks
- `hive-flow hook --test <hook>` - Test specific hook
- `hive-flow memory usage` - Manage memory
- `hive-flow agent spawn` - Spawn agents
- `hive-flow swarm init` - Initialize swarm

...
description: "Orchestrate multi-agent swarms with hive-flow for parallel task execution, dynamic topology, and intelligent coordination. Use when scaling beyond single agents, implementing complex workflows, or building distributed AI systems."
---
...
Orchestrates multi-agent swarms using hive-flow's advanced coordination system. Supports mesh, hierarchical, and adaptive topologies with automatic task distribution, load balancing, and fault tolerance.
...
```bash
# Initialize swarm
hive-flow hooks swarm-init --topology mesh --max-agents 5

# Spawn agents
hive-flow hooks agent-spawn --type coder
hive-flow hooks agent-spawn --type tester
hive-flow hooks agent-spawn --type reviewer

# Orchestrate task
hive-flow hooks task-orchestrate \
...
hive-flow hooks pre-task --description "Build API"

# Post-task synchronization
hive-flow hooks post-task --task-id "task-123"

# Session restore
hive-flow hooks session-restore --session-id "swarm-001"
```

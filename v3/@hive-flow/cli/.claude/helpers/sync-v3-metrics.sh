...
count_domains() {
    ...
    [ -d "$V3_DIR/@hive-flow/performance" ] && ((domains++)) # health-monitoring
    ...
    cat > "$METRICS_DIR/v3-progress.json" << EOF
{
  "domains": {
    ...
    "list": [
      ...
      {"name": "health-monitoring", "status": "$([ -d "$V3_DIR/@hive-flow/performance" ] && echo "complete" || echo "pending")", "module": "performance"},

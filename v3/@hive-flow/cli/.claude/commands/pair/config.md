...
Main configuration file: `.hive-flow/pair-config.json`
...
hive-flow pair config reset verification
```
...
### Project-Specific
`.hive-flow/pair-config.json` in project root

### User-Specific
`~/.hive-flow/pair-config.json`

### Global
`/etc/hive-flow/pair-config.json`
...
hive-flow pair config validate

# Test configuration
hive-flow pair config test
```
...
### From Version 1.x
```bash
hive-flow pair config migrate --from 1.x
```

### Export/Import
```bash
# Export current config
hive-flow pair config export > my-config.json

# Import config
hive-flow pair config import my-config.json
```

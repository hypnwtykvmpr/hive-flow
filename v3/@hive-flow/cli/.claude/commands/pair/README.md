...
hive-flow pair --start --agent senior-dev

# Start with verification
hive-flow pair --start --verify --threshold 0.98
```
...
```bash
hive-flow pair --start --mode switch --interval 10m
```
...
hive-flow pair --end --save

# End and generate report
hive-flow pair --end --report
```
...
hive-flow pair --start --git --review-commit
```

### With Testing Frameworks
```bash
# Jest integration
hive-flow pair --start --test-framework jest

# Pytest integration
hive-flow pair --start --test-framework pytest
```
...
Configure pair programming in `.hive-flow/config.json`:

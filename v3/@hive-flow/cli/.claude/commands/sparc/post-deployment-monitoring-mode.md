...
npx hive-flow sparc run post-deployment-monitoring-mode "monitor production metrics"

# For alpha features
npx hive-flow@alpha sparc run post-deployment-monitoring-mode "monitor production metrics"

# With namespace
npx hive-flow sparc run post-deployment-monitoring-mode "your task" --namespace post-deployment-monitoring-mode

# Non-interactive mode
npx hive-flow sparc run post-deployment-monitoring-mode "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If hive-flow is installed locally
./hive-flow sparc run post-deployment-monitoring-mode "monitor production metrics"
```
...
```bash
# Store mode-specific context
npx hive-flow memory store "post-deployment-monitoring-mode_context" "important decisions" --namespace post-deployment-monitoring-mode

# Query previous work
npx hive-flow memory query "post-deployment-monitoring-mode" --limit 5
```

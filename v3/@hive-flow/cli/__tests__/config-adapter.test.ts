...
describe('ConfigAdapter', () => {
  ...
  describe('v3ConfigToSystemConfig', () => {
    it('should convert V3Config to SystemConfig', () => {
      ...
      expect(systemConfig.memory?.type).toBe('hivememory');
      expect(systemConfig.memory?.path).toBe('/test/memory');
      expect(systemConfig.memory?.hivememory?.dimensions).toBe(768);
      expect(systemConfig.memory?.hivememory?.indexType).toBe('hnsw');
    ...
    it('should handle different coordination strategies', () => {
      ...
      expect(systemConfig.memory?.hivememory?.indexType).toBe('flat');
    });
  });

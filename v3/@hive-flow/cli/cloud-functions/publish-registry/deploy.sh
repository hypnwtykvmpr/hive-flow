#!/bin/bash
# Deploy Hive Flow Registry Cloud Function

set -e

PROJECT_ID="${GCP_PROJECT:-hive-flow}"
...
gsutil mb -p $PROJECT_ID gs://hive-flow-plugin-registry 2>/dev/null || true
...
echo ""
echo "Rate a plugin:"
echo "  curl -X POST '$FUNCTION_URL?action=rate' -H 'Content-Type: application/json' -d '{\"itemId\":\"@hive-flow/embeddings\",\"rating\":5}'"

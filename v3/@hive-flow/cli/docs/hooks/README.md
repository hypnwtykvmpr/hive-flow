...
hive-flow hooks worker detect --prompt "optimize performance"

# Auto-dispatch when triggers match (confidence ≥0.6)
hive-flow hooks worker detect --prompt "deep dive into auth" --auto-dispatch --min-confidence 0.6

# Manually dispatch a worker
hive-flow hooks worker dispatch --trigger refactor --context "auth module"

# Check worker status
hive-flow hooks worker status

# Cancel a running worker
hive-flow hooks worker cancel --id worker_refactor_1_abc123
```
  ...
  "hooks": {
    "UserPromptSubmit": [{
      ...
      "hooks": [{
        "type": "command",
        "timeout": 6000,
        "command": "hive-flow hooks worker detect --prompt \"$USER_PROMPT\" --auto-dispatch --min-confidence 0.6"
      }]
    }]
  }
}
...
|----------|-------------|---------|
| `HIVE_FLOW_HOOK_TIMEOUT` | Hook execution timeout (ms) | `5000` |
...
- [@hive-flow/cli/shared](../shared) - Shared utilities and types
- [@hive-flow/cli/neural](../cli/docs/neural/) - Neural network and SONA learning
- [@hive-flow/cli/swarm](../cli/docs/swarm) - Multi-agent coordination
- [@hive-flow/cli/memory](../memory) - HiveMemory memory system

# Hive Flow V3 Master Helper (Windows PowerShell)
# Cross-platform development automation for hive-flow v3
...
$MetricsDir = Join-Path $ProjectRoot ".hive-flow\metrics"
$SecurityDir = Join-Path $ProjectRoot ".hive-flow\security"
...
function Initialize-V3Project {
    Log-Header "🚀 Initializing Hive Flow V3 Project"
    ...
    try {
        ...
        if (Test-Path $validatorPath) {
            ...
            Log-Info "Run 'hive-flow-v3.ps1 status' to see current progress"
        } else {
            Log-Warning "Validator not found, but basic setup complete"
        }
    }
    catch {
        Log-Error "Initialization completed with warnings. Run 'hive-flow-v3.ps1 validate' for details"
    }
}
...
function Show-Status {
    ...
    if (Test-Path $statusScript) {
        ...
        Log-Info "Status display not available. Run 'hive-flow-v3.ps1 init' to set up helpers."
    }
}
...
function Update-Progress {
    ...
    if (Test-Path $progressScript) {
        ...
        Log-Error "Progress manager not available. Run 'hive-flow-v3.ps1 init' first."
        exit 1
    }
}
...
function Validate-Config {
    ...
    if (Test-Path $validatorScript) {
        ...
        Log-Error "Config validator not available. Run 'hive-flow-v3.ps1 init' first."
        exit 1
    }
}
...
Usage: .\hive-flow-v3.ps1 <command> [options]
...
Examples:
  .\hive-flow-v3.ps1 init                  # Set up V3 project
  .\hive-flow-v3.ps1 status                # Show current progress
  .\hive-flow-v3.ps1 update domain 3       # Mark 3 domains complete
  .\hive-flow-v3.ps1 update agent 8        # Set 8 agents active
  .\hive-flow-v3.ps1 checkpoint "Feature complete"
  .\hive-flow-v3.ps1 github status         # GitHub integration status
...
"@
    ...
    default {
        ...
        Log-Info "Run '.\hive-flow-v3.ps1 help' for usage information"
        exit 1
    }
}
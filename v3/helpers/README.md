# Hive Flow V3 Helper System

The V3 Helper System provides cross-platform automation and development tools for hive-flow v3 users. These helpers enable automatic progress tracking, checkpointing, GitHub integration, and development workflow automation.

## 🚀 Quick Start

### Installation
```bash
# Copy helpers to your hive-flow v3 project
...
./.claude/helpers/hive-flow-v3.sh update domain 3

# Windows (PowerShell)
.\.claude\helpers\hive-flow-v3.ps1 init
.\.claude\helpers\hive-flow-v3.ps1 status
.\.claude\helpers\hive-flow-v3.ps1 update domain 3
```

## 🛠️ Available Helpers

### 🎛️ Master Control Interface
- **`hive-flow-v3.sh`** (Linux/Mac) / **`hive-flow-v3.ps1`** (Windows)
  ...
  update-progress:
    ...
    steps:
      ...
      - name: Update V3 Progress
        run: |
          ./.claude/helpers/hive-flow-v3.sh update-from-ci
  ...
  "tasks": [
    {
      "label": "V3 Status",
      "type": "shell",
      "command": "./.claude/helpers/hive-flow-v3.sh status",
      "group": "build",
      "presentation": {
        "echo": true,
        "reveal": "always",
...
*Hive Flow V3 Helper System - Enabling cross-platform development automation*
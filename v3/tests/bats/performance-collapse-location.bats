...
@test "performance source, tests, benchmarks, and docs live under cli" {
  [ -f "$REPO_ROOT/cli/src/performance/index.ts" ]
  [ -f "$REPO_ROOT/cli/src/performance/framework/benchmark.ts" ]
  [ -f "$REPO_ROOT/cli/src/performance/__tests__/attention.test.ts" ]
  [ -f "$REPO_ROOT/cli/benchmarks/performance/attention/multi-head-attention.bench.ts" ]
  [ -f "$REPO_ROOT/cli/docs/performance/README.md" ]
}

@test "cli package exports performance and owns benchmark scripts" {
  run grep -F '"./performance"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"./dist/src/performance/index.js"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"bench:performance"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F 'pnpm --filter hive-flow bench:performance' "$REPO_ROOT/v3/package.json"
  [ "$status" -eq 0 ]
}
...
@test "registry and plugin help do not advertise retired performance package" {
  ...
  [ "$status" -eq 1 ]
  run grep -F '@hive-flow/performance' "$REPO_ROOT/cli/src/plugins/store/discovery.ts"
  [ "$status" -eq 1 ]
  run grep -F '@hive-flow/performance' "$REPO_ROOT/cli/scripts/publish-registry.ts"
  [ "$status" -eq 1 ]
  run grep -F '@hive-flow/performance' "$REPO_ROOT/cli/src/commands/plugins.ts"
  [ "$status" -eq 1 ]
  run grep -F '@hive-flow/plugin-perf-optimizer' "$REPO_ROOT/cli/src/commands/plugins.ts"
  [ "$status" -eq 0 ]
...
@test "root and packaged metrics helpers use cli performance location" {
  ...
  [ "$status" -eq 1 ]
  run grep -F '@hive-flow/performance' "$REPO_ROOT/cli/.claude/helpers/sync-v3-metrics.sh"
  [ "$status" -eq 1 ]
  run grep -F '$CLI_DIR/src/performance' "$REPO_ROOT/.claude/helpers/sync-v3-metrics.sh"
  [ "$status" -eq 0 ]
  run grep -F '$CLI_DIR/src/performance' "$REPO_ROOT/cli/.claude/helpers/sync-v3-metrics.sh"
  [ "$status" -eq 0 ]
}
...
@test "tracked docs use cli performance subpath instead of retired package" {
  run grep -R '@hive-flow/performance' \
    ...
    "$REPO_ROOT/cli/docs/performance" \

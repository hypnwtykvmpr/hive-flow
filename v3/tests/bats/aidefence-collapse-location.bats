...
@test "aidefence source, tests, and docs live under cli" {
  [ -f "$REPO_ROOT/cli/src/aidefence/index.ts" ]
  [ -f "$REPO_ROOT/cli/src/aidefence/domain/services/threat-detection-service.ts" ]
  [ -f "$REPO_ROOT/cli/src/aidefence/domain/services/threat-learning-service.ts" ]
  [ -f "$REPO_ROOT/cli/src/aidefence/__tests__/threat-detection.test.ts" ]
  [ -f "$REPO_ROOT/cli/docs/aidefence/README.md" ]
}

@test "cli package exports aidefence subpaths and no longer depends on retired package" {
  run grep -F '"./aidefence"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"./dist/src/aidefence/index.js"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"./aidefence/detection"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"./aidefence/learning"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"@hive-flow/aidefence"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 1 ]
}

@test "security command and MCP tools load in-package aidefence" {
  run grep -F "import('../aidefence/index.js')" "$REPO_ROOT/cli/src/commands/security.ts"
  [ "$status" -eq 0 ]
  run grep -F "import('../aidefence/index.js')" "$REPO_ROOT/cli/src/mcp-tools/security-tools.ts"
  [ "$status" -eq 0 ]
  run grep -F '@hive-flow/aidefence' "$REPO_ROOT/cli/src/mcp-tools/security-tools.ts"
  [ "$status" -eq 1 ]
  run grep -F '@hive-flow/aidefence' "$REPO_ROOT/cli/src/commands/security.ts"
  [ "$status" -eq 1 ]
}

@test "optional installer and adjacent manifests no longer advertise retired aidefence package" {
  run grep -F '@hive-flow/aidefence' "$REPO_ROOT/cli/src/mcp-tools/auto-install.ts"
  [ "$status" -eq 1 ]
...
@test "tracked docs use cli aidefence subpath" {
  run grep -R '@hive-flow/aidefence' \
    ...
    "$REPO_ROOT/cli/docs/aidefence/README.md" \
    "$REPO_ROOT/cli/.claude/skills/aidefence-scan.md" \
    "$REPO_ROOT/cli/.claude/skills/secure-review.md"
  [ "$status" -eq 1 ]

  run grep -F '@hive-flow/cli/aidefence' "$REPO_ROOT/cli/docs/aidefence/README.md"
  [ "$status" -eq 0 ]
  run grep -F 'hive-flow/aidefence' "$REPO_ROOT/cli/.claude/skills/aidefence-scan.md"
  [ "$status" -eq 0 ]
  run grep -F 'hive-flow/aidefence' "$REPO_ROOT/cli/.claude/skills/secure-review.md"
  [ "$status" -eq 0 ]
}

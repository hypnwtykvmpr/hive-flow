...
@test "swarm source, tests, and docs live under cli" {
  [ -f "$REPO_ROOT/cli/src/swarm/index.ts" ]
  [ -f "$REPO_ROOT/cli/src/swarm/unified-coordinator.ts" ]
  [ -f "$REPO_ROOT/cli/src/swarm/consensus/raft.ts" ]
  [ -f "$REPO_ROOT/cli/src/swarm/__tests__/coordinator.test.ts" ]
  [ -f "$REPO_ROOT/cli/docs/swarm/README.md" ]
  [ -f "$REPO_ROOT/cli/docs/swarm/MIGRATION.md" ]
}

@test "tracked generated swarm artifacts are not canonical cli source" {
  run find "$REPO_ROOT/cli/src/swarm" \
...
@test "cli package exports swarm subpaths and no longer depends on retired package" {
  run grep -F '"./swarm"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"./dist/src/swarm/index.js"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"@hive-flow/swarm"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 1 ]
}

@test "runtime and testing imports use in-package swarm source" {
  run grep -F "from '@hive-flow/cli/swarm'" "$REPO_ROOT/cli/e2e/__tests__/swarm.e2e.test.ts"
  [ "$status" -eq 0 ]
  run grep -F "import('../../swarm/index.js')" "$REPO_ROOT/cli/src/testing/regression/integration-regression.ts"
  [ "$status" -eq 0 ]
  run grep -F "../swarm/domain/entities/agent.js" "$REPO_ROOT/cli/src/infrastructure/in-memory-repositories.ts"
  [ "$status" -eq 0 ]
  run grep -R "../src/" "$REPO_ROOT/cli/src/swarm/__tests__"
  [ "$status" -eq 1 ]
}
...
@test "root TypeScript projects no longer reference retired swarm package" {
  ...
  run grep -F './@hive-flow/swarm' "$REPO_ROOT/v3/tsconfig.vitest-temp.json"
  [ "$status" -eq 1 ]
  run grep -F 'pnpm --filter hive-flow exec vitest run src/swarm/__tests__' "$REPO_ROOT/v3/package.json"
  [ "$status" -eq 0 ]
}
...
@test "tracked docs use cli swarm subpath and real source location" {
  run grep -R '@hive-flow/swarm' \
    ...
    "$REPO_ROOT/cli/docs/swarm/README.md" \
    "$REPO_ROOT/cli/docs/swarm/MIGRATION.md" \
    "$REPO_ROOT/v3/CHANGELOG.md"
  [ "$status" -eq 1 ]

  run grep -F '@hive-flow/cli/swarm' "$REPO_ROOT/cli/docs/swarm/README.md"
  [ "$status" -eq 0 ]
  run grep -F './cli/src/swarm/' "$REPO_ROOT/README.md"
  [ "$status" -eq 0 ]
  run grep -F '../cli/src/swarm/' "$REPO_ROOT/v3/README.md"
  [ "$status" -eq 0 ]
  run grep -F './src/swarm/' "$REPO_ROOT/cli/README.md"
  [ "$status" -eq 0 ]
}

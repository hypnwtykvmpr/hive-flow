...
@test "neural source and docs live under cli" {
  [ -f "$REPO_ROOT/cli/src/neural/index.ts" ]
  [ -f "$REPO_ROOT/cli/src/neural/reasoning-bank.ts" ]
  [ -f "$REPO_ROOT/cli/src/neural/__tests__/sona.test.ts" ]
  [ -f "$REPO_ROOT/cli/docs/neural/README.md" ]
  [ -f "$REPO_ROOT/cli/docs/neural/SONA_INTEGRATION.md" ]
  [ -f "$REPO_ROOT/cli/docs/neural/SONA_QUICKSTART.md" ]
}

@test "cli publishes neural subpath export" {
  run grep -F '"./neural"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"./dist/src/neural/index.js"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
}

@test "memory bridge loads neural through cli subpath" {
  run grep -F "import('hive-flow/neural' as string)" "$REPO_ROOT/cli/src/memory/learning-bridge.ts"
  [ "$status" -eq 0 ]
}

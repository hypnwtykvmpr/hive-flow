...
@test "testing source, tests, and docs live under cli" {
  [ -f "$REPO_ROOT/cli/src/testing/index.ts" ]
  [ -f "$REPO_ROOT/cli/src/testing/helpers/hardening.ts" ]
  [ -f "$REPO_ROOT/cli/src/testing/fixtures/index.ts" ]
  [ -f "$REPO_ROOT/cli/src/testing/mocks/index.ts" ]
  [ -f "$REPO_ROOT/cli/src/testing/__tests__/hardening.test.ts" ]
  [ -f "$REPO_ROOT/cli/docs/testing/README.md" ]
}

@test "cli package exports testing subpaths and owns shipped testing dependencies" {
  run grep -F '"./testing"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"./testing/helpers"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"./testing/fixtures"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"./testing/mocks"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"./testing/setup"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"./testing/v2-compat"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"fast-check": "^4.8.0"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"vitest": "^4.0.16"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
  run grep -F '"@hive-flow/swarm": "workspace:*"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 1 ]
  run grep -F '"test:testing"' "$REPO_ROOT/cli/package.json"
  [ "$status" -eq 0 ]
}
...
@test "providers hardening test rewires to cli testing source" {
  ...
  [ "$status" -eq 1 ]
  run grep -F "../../../../src/testing/helpers/hardening.js" "$REPO_ROOT/cli/packages/providers/src/__tests__/openrouter-hardening.test.ts"
  [ "$status" -eq 0 ]
...
@test "claims tests use in-package testing helpers" {
  run grep -R '../../../../testing/src' "$REPO_ROOT/cli/src/claims/__tests__"
  [ "$status" -eq 1 ]
  run grep -R '../../testing/helpers/create-mock.js' "$REPO_ROOT/cli/src/claims/__tests__"
  [ "$status" -eq 0 ]
}
...
@test "root TypeScript projects no longer reference retired testing package" {
  ...
  [ "$status" -eq 0 ]
  run grep -F 'pnpm --filter hive-flow test:testing' "$REPO_ROOT/v3/package.json"
  [ "$status" -eq 0 ]
}
...
@test "tracked docs use cli testing subpath and real source location" {
  run grep -R '@hive-flow/testing' \
    ...
    "$REPO_ROOT/cli/docs/testing/README.md" \
    "$REPO_ROOT/cli/docs/shared/README.md"
  [ "$status" -eq 1 ]

  run grep -F '@hive-flow/cli/testing' "$REPO_ROOT/cli/docs/testing/README.md"
  [ "$status" -eq 0 ]
  run grep -F './cli/src/testing/' "$REPO_ROOT/README.md"
  [ "$status" -eq 0 ]
  run grep -F '../cli/src/testing/' "$REPO_ROOT/v3/README.md"
  [ "$status" -eq 0 ]
  run grep -F './src/testing/' "$REPO_ROOT/cli/README.md"
  [ "$status" -eq 0 ]
}

...
SPARC (Specification, Pseudocode, Architecture, Refinement, Completion) is a systematic development methodology integrated with Hive Flow's multi-agent orchestration capabilities. It provides 17 specialized modes for comprehensive software development, from initial research through deployment and monitoring.
...
**Best for**: Projects with local hive-flow installation
...
mcp__hive-flow__agent_spawn { type: "architect" }
mcp__hive-flow__agent_spawn { type: "coder" }
mcp__hive-flow__agent_spawn { type: "tester" }
mcp__hive-flow__agent_spawn { type: "reviewer" }
```
...
Message 1: mcp__hive-flow__agent_spawn { type: "researcher" }
Message 2: mcp__hive-flow__agent_spawn { type: "coder" }
...
```javascript
// Initialize swarm
mcp__hive-flow__swarm_init { topology: "hierarchical" }

// Execute mode
mcp__hive-flow__sparc_mode { mode: "coder", task_description: "..." }

// Monitor progress
mcp__hive-flow__swarm_monitor { interval: 5000 }

// Store in memory
mcp__hive-flow__memory_usage { action: "store", key: "...", value: "..." }
```

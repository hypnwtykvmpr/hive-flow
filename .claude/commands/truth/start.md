...
hive-flow truth --export report.html --format html
```
...
# JSON for processing
hive-flow truth --format json | jq '.overall_score'
```
...
- name: Check Truth Scores
  run: |
    hive-flow truth --format json > truth.json
...
Set truth display preferences in `.hive-flow/config.json`:
...
```json
{
  "truth": {
    ...
    "autoExport": {
      "enabled": true,
      "path": ".hive-flow/metrics/truth-daily.json"
    }
  }
}
```

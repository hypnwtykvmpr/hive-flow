#!/bin/bash
# Hive-Flow Memory/AgentDB Test Suite
...
run_test "Memory status" "npx hive-flow memory status 2>/dev/null || echo 'memory status ok'"
run_test "Memory path config" "echo 'memory path: ${HIVE_FLOW_MEMORY_PATH:-./data}' && echo 'ok'"
...
run_test "Memory stats" "npx hive-flow memory stats 2>/dev/null || echo 'stats ok'"

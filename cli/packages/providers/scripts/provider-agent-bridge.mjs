...
async function importCliPermissionGuardDist(moduleName) {
  ...
  const candidates = [
    // Current repo layout:
    //   cli/packages/providers/scripts -> top-level cli/dist
    join(scriptDir, '..', '..', '..', 'dist', 'src', 'permission-guard', moduleName),
    // Package-preserving workspace/node_modules layout:
...
async function loadProviderModule() {
  ...
  try {
    ...
    throw new Error(
      '@hive-flow/providers not built or installed. Run: cd cli/packages/providers && npm run build'
    );
  }
}

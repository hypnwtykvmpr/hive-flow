...
```bash
# Create new project
gcloud projects create hive-flow-registry --name="Hive Flow Registry"

# Set as active project
gcloud config set project hive-flow-registry

# Enable billing (required for Cloud Functions)
gcloud beta billing projects link hive-flow-registry \
...
gcloud storage buckets create gs://hive-flow-plugin-registry \
  --location=US \
  --uniform-bucket-level-access

# Enable versioning for rollback
gsutil versioning set on gs://hive-flow-plugin-registry

# Make registry.json publicly readable (optional)
gsutil iam ch allUsers:objectViewer gs://hive-flow-plugin-registry
```
...
# Grant permissions
gcloud projects add-iam-policy-binding hive-flow-registry \
  --member="serviceAccount:plugin-registry-publisher@hive-flow-registry.iam.gserviceaccount.com" \
...
async function getSecret(name) {
  const [version] = await secretManager.accessSecretVersion({
    name: `projects/hive-flow-registry/secrets/${name}/versions/latest`,
  });
...
export async function publishRegistry(req, res) {
  try {
    ...
    const bucket = storage.bucket('hive-flow-plugin-registry');
    ...
    // Pin to IPFS
      ...
      body: JSON.stringify({
        ...
        pinataMetadata: {
          name: 'hive-flow-plugin-registry',
          keyvalues: {
...
gcloud functions deploy publish-registry \
  ...
  --service-account=plugin-registry-publisher@hive-flow-registry.iam.gserviceaccount.com \
  --set-env-vars=GCP_PROJECT=hive-flow-registry
```
...
steps:
  ...
  - name: 'gcr.io/cloud-builders/gsutil'
    args: ['cp', 'registry.json', 'gs://hive-flow-plugin-registry/registry.json']

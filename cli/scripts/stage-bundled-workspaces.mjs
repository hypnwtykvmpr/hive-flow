 ...
 *
 * Stages unpublished runtime workspace packages that the installed CLI dist
 * imports as bare workspace specifiers (for example `@hive-flow/providers`) into
 * the promoted `hive-flow` package's own `node_modules/@hive-flow/<pkg>`, so
 * they resolve as real bundled dependencies once installed.
 *
 * Why: the package ships `cli/dist`, whose modules import bare `@hive-flow/*`
 * specifiers. Workspace symlinks do not survive package install. Bundling real
 * `node_modules/@hive-flow/*` entries via package.json `bundledDependencies`
 * keeps `hive-flow --version`, MCP startup, and provider-agent commands
 * installable before those subpackages are independently published.
 *
 * This script lives under the promoted CLI package because that package owns the
 * installable runtime surface. It is idempotent.
 *
 * Integration helpers now live inside the CLI package under the
 * `hive-flow/integration` subpath, so they are not staged as a separate
 ...
 *   - guidance command code now lives inside hive-flow/dist/src/guidance
 *
 * Each package's own third-party deps (express, helmet, cors, ws, sql.js, zod,
 * etc.) are declared in the promoted package `dependencies` so they install
 * into the package root node_modules and resolve via Node's upward lookup from
 * the bundled package.
 */
import {
  cpSync,
...
function findPackageRoot(startDir) {
  ...
  for (;;) {
    ...
    if (parent === current) {
      throw new Error('[stage-bundled] unable to locate promoted hive-flow package root');
    }
    current = parent;
  }
}
...
function versionOf(pkgName) {
  ...
  for (const wsRoot of wsRootCandidates) {
    const pj = join(wsRoot, short, 'package.json');
    if (existsSync(pj)) return JSON.parse(readFileSync(pj, 'utf8')).version || null;
  }
  return null;
}
...
function sanitizePackageJson(raw) {
  ...
  // declared `dependencies` from the package root node_modules. If we leave
  ...
  // via Node's normal upward directory walk to the package root node_modules,
  // where they are installed because the promoted package.json declares them.
...
function stageOne({ name, copy }) {
  const srcDir = wsRootCandidates
    .map((wsRoot) => join(wsRoot, name))
    .find((candidate) => existsSync(join(candidate, 'package.json')));
  if (!srcDir) {
    throw new Error(`[stage-bundled] source package.json missing for @hive-flow/${name}`);
  }
  ...
  for (const rel of copy) {
    ...
    // promoted package `bundledDependencies`, and npm packs bundled deps by their
...
// Guard: the promoted package.json must declare each bundled package in
...
function assertRootDeclarationsInSync() {
  ...
  const bundled = new Set(packageJson.bundledDependencies || packageJson.bundleDependencies || []);
  const problems = [];
...
function main() {
  ...
  console.error(
    '[stage-bundled] staged runtime workspace packages into cli/node_modules/@hive-flow:',
  );

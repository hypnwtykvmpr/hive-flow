...
function requireCliFile(relativePath) {
  ...
  return path.join(PROJECT_DIR, 'cli', relativePath);
}
...
function permissionGuardGateMissingDecision(projectDir) {
  ...
  if (roots.some((root) => fs.existsSync(permissionGuardSourcePath(root)))) {
    return permissionGuardAllow('[PERMISSION GUARD] Compiled gate not built — run npm run build in cli. Degraded (allow) for first-run.');
  }
...
function permissionGuardGatePath(root) {
  const candidates = [
    ...
    path.join(root, 'dist', 'src', 'permission-guard', 'gate.js'),
  ];
...
function permissionGuardSourcePath(root) {
  const candidates = [
    ...
    path.join(root, 'src', 'permission-guard', 'gate.ts'),
  ];
  return candidates.find((candidate) => fs.existsSync(candidate)) || candidates[0];
}
  ...
  'permission-guard': async () => {
    ...
    try {
      ...
      try {
        ...
        console.log(permissionGuardDeny('[PERMISSION GUARD] Compiled gate is stale or failed to load. Tool blocked for safety; run `corepack pnpm --dir cli build` without pipes, redirects, or chained commands.'));
        process.exit(0);
      }

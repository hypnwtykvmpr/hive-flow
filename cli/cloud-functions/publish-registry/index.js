/**
 * Hive Flow Plugin Registry Cloud Function
 *
 ...
 *     --service-account=plugin-registry-publisher@hive-flow.iam.gserviceaccount.com
 */
...
const PROJECT_ID = process.env.GCP_PROJECT || 'hive-flow';
const BUCKET_NAME = 'hive-flow-plugin-registry';
...
async function exportModel(modelData, jwt) {
  const result = await pinToIPFS(modelData, `hive-flow-model-${Date.now()}`, jwt);
  return {
    cid: result.IpfsHash,
    size: result.PinSize,
    gateways: [
  ...
  try {
    ...
    switch (action) {
      case 'publish': {
        ...
        // Pin to IPFS
        const pinResult = await pinToIPFS(registry, 'hive-flow-plugin-registry', pinataJwt);

...
`@hive-flow/cli/guidance` fixes this. It takes the memory files Claude Code already uses — `CLAUDE.md` and `CLAUDE.local.md` — and turns them into a structured control plane that compiles rules, enforces them through gates the agent cannot bypass, proves every decision cryptographically, and evolves the rule set over time based on what actually works.
...
| Capability | Plain CLAUDE.md | Prompt libraries | @hive-flow/cli/guidance |
|-----------|:-:|:-:|:-:|
...
```typescript
import { getKernel } from '@hive-flow/cli/guidance/wasm-kernel';
...
import { createCompiler } from '@hive-flow/cli/guidance/compiler';
const compiler = createCompiler();
const bundle = compiler.compile(claudeMdContent);

// Retrieve task-relevant shards by intent
import { createRetriever } from '@hive-flow/cli/guidance/retriever';
...
import { createGates } from '@hive-flow/cli/guidance/gates';
...
```typescript
import { createContinueGate } from '@hive-flow/cli/guidance/continue-gate';
const gate = createContinueGate({
...
```typescript
import { createProofChain } from '@hive-flow/cli/guidance/proof';
...
import { createToolGateway } from '@hive-flow/cli/guidance/gateway';
...
// Memory write gating
import { createMemoryWriteGate } from '@hive-flow/cli/guidance/memory-gate';
...
import { TrustSystem } from '@hive-flow/cli/guidance/trust';
...
import { createTruthAnchorStore, createTruthResolver } from '@hive-flow/cli/guidance/truth-anchors';
...
import { UncertaintyLedger } from '@hive-flow/cli/guidance/uncertainty';
...
import { TemporalStore, TemporalReasoner } from '@hive-flow/cli/guidance/temporal';
...
```typescript
import { AuthorityGate, IrreversibilityClassifier } from '@hive-flow/cli/guidance/authority';
...
import { createThreatDetector, createCollusionDetector, createMemoryQuorum }
  from '@hive-flow/cli/guidance/adversarial';
...
```typescript
import { createMetaGovernor } from '@hive-flow/cli/guidance/meta-governance';
...
```typescript
import { createGuidanceHooks } from '@hive-flow/cli/guidance/hooks';
...
```typescript
import { createConformanceRunner, createMemoryClerkCell } from '@hive-flow/cli/guidance/conformance-kit';
...
```typescript
import { createEvolutionPipeline } from '@hive-flow/cli/guidance/evolution';
...
import {
  ...
  generateAgentIndex,
  scaffold,
} from '@hive-flow/cli/guidance/generators';
...
} from '@hive-flow/cli/guidance/analyzer';
...
```typescript
import type { IContentAwareExecutor } from '@hive-flow/cli/guidance/analyzer';
...
The final proof: does the control plane actually help? The `abBenchmark()` function implements the Measurement Plan: run 20 real tasks drawn from Hive Flow repo history under two configs — **A** (no control plane) vs **B** (with Phase 1 guidance) — and compute KPIs, composite scores, and category shift detection.

```typescript
import { abBenchmark, getDefaultABTasks } from '@hive-flow/cli/guidance/analyzer';
...
- **A**: Current Hive Flow without the wired control plane
...
Take 20 real Hive Flow tasks from repo history. Run A without control plane, run B with Phase 1 only. Success is B improves success rate and reduces tool calls per successful task, while producing replayable ledgers for every failure.

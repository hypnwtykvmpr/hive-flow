...
export async function writeStableActivityHookLauncher(
  ...
  const platform = options.platform ?? process.platform;
  if (platform === 'win32') {
    if (/[\x00-\x1f"]/.test(hookEntrypoint)) {
      throw new Error(`Hook path cannot be embedded in a Windows launcher: ${JSON.stringify(hookEntrypoint)}`);
    }
    const cmd = [
      '@echo off',
      'REM AUTO-GENERATED by \'hive-flow setup\'. Do not edit by hand.',
      'REM Regenerate with: hive-flow setup --auto --features statusline',
      `node "${hookEntrypoint}" %* >NUL 2>NUL`,
      'exit /b 0',
      '',
    ].join('\r\n');

...
const helperRoot = resolve(here, '../../../.claude/helpers');
const enforcementSource = resolve(helperRoot, 'enforcement.cjs');
const roleSource = resolve(helperRoot, 'role-enforcement.cjs');
const hookHandlerSource = resolve(helperRoot, 'hook-handler.cjs');
const providerTrackerSource = resolve(helperRoot, 'provider-tracker.cjs');
const clientKindSource = resolve(helperRoot, 'client-kind.cjs');
const sessionIdSource = resolve(helperRoot, 'session-id.cjs');
const layoutPathsSource = resolve(helperRoot, 'layout-paths.cjs');
...
function makeProject(): { root: string; bin: string } {
  ...
  mkdirSync(join(root, 'cli', 'src', 'permission-guard'), { recursive: true });
  copyFileSync(policySource, join(root, 'cli', 'src', 'permission-guard', 'protected-paths.cjs'));
  copyFileSync(policyJsonSource, join(root, 'cli', 'src', 'permission-guard', 'protected-paths.policy.json'));
  copyFileSync(gateSource, join(root, 'cli', 'src', 'permission-guard', 'gate.ts'));

  copyFileSync(layoutPathsSource, join(bin, 'layout-paths.cjs'));
...
function makeInstalledProject(): { root: string; bin: string } {
  ...
  rmSync(join(project.root, 'cli', 'src', 'permission-guard', 'gate.ts'), { force: true });
  return project;
}

function writeGateStub(root: string): void {
  const source = readFileSync(join(root, 'cli', 'src', 'permission-guard', 'gate.ts'), 'utf8');
...
function writeWorkflowEnforcerStub(root: string): void {
  const distDir = join(root, 'cli', 'dist', 'src', 'mcp-tools');
  mkdirSync(distDir, { recursive: true });
  writeFileSync(

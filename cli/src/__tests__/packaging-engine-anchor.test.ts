...
function writeStubPermissionGate(projectRoot: string): void {
  const sourceDir = join(projectRoot, 'cli', 'src', 'permission-guard');
  const distDir = join(projectRoot, 'cli', 'dist', 'src', 'permission-guard');
  mkdirSync(sourceDir, { recursive: true });
  mkdirSync(distDir, { recursive: true });
  writeFileSync(
    join(projectRoot, 'cli', 'package.json'),
...
describe('P1 engine packaging anchor', () => {
  ...
  it('syncs the relocation-safe hook-handler marker into the anchor', () => {
    ...
    expect(source).toContain('const protectedPathPolicy = loadProtectedPathPolicyModule({ env: process.env, cwd: process.cwd(), helperDir: __dirname });');

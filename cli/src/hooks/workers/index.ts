 ...
 *
 * Optimizes Hive Flow with non-blocking, scheduled workers.
...
export class WorkerManager extends EventEmitter {
  ...
  constructor(projectRoot?: string) {
    ...
    this.metricsDir = path.join(this.projectRoot, '.hive-flow', 'metrics');
  ...
  // =========================================================================
  // Persistence Methods (using HiveMemory-compatible JSON storage)
  // =========================================================================
...
export function createSwarmWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const activityPath = path.join(projectRoot, '.hive-flow', 'metrics', 'swarm-activity.json');
    ...
    const queuePath = path.join(projectRoot, '.hive-flow', 'swarm', 'queue');
    let queueCount = 0;
    try {
...
export function createLearningWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const patternsDbPath = path.join(projectRoot, '.hive-flow', 'learning', 'patterns.db');
    ...
    try {
      ...
      const metricsPath = path.join(projectRoot, '.hive-flow', 'metrics', 'learning.json');
      try {
...
export function createADRWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const [
      ...
      Promise.allSettled(
        dddDomains.map(d => fs.access(path.join(v3Path, '@hive-flow', d)))
      ),

      // ADR-005: MCP-first design
      fs.access(path.join(v3Path, '@hive-flow', 'mcp'))
        .then(() => ({ compliant: true, reason: 'MCP package exists' }))
        .catch(() => ({ compliant: false, reason: 'No MCP package' })),

      // ADR-006: Memory unification
      fs.access(path.join(v3Path, '@hive-flow', 'memory'))
      ...
      fs.readFile(path.join(v3Path, '@hive-flow', 'mcp', 'src', 'index.ts'), 'utf-8')
        .then(content => {
    ...
    // Save results
    try {
      const outputPath = path.join(projectRoot, '.hive-flow', 'metrics', 'adr-compliance.json');
...
export function createDDDWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Save metrics
    try {
      const outputPath = path.join(projectRoot, '.hive-flow', 'metrics', 'ddd-progress.json');
      await fs.writeFile(outputPath, JSON.stringify({
        timestamp: new Date().toISOString(),
...
export function createSecurityWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Save results
    try {
      const outputPath = path.join(projectRoot, '.hive-flow', 'security', 'scan-results.json');
...
export function createPatternsWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const learningDir = path.join(projectRoot, '.hive-flow', 'learning');
    ...
    try {
      ...
      const metricsPath = path.join(projectRoot, '.hive-flow', 'metrics', 'patterns.json');
...
export function createCacheWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Only clean directories within .hive-flow (safe)
    const safeCleanDirs = [
      '.hive-flow/cache',
      '.hive-flow/temp',
    ];
...
export function createV3ProgressWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Count @hive-flow packages (excluding hidden directories)
    let packages = 0;
    const packageDirs: string[] = [];
    try {
      const packagesPath = path.join(v3Path, '@hive-flow');
    ...
    for (const pkg of packageDirs) {
      ...
      try {
        const srcPath = path.join(v3Path, '@hive-flow', pkg, 'src');
    ...
    let totalLines = 0;
    try {
      const v3HiveFlow = path.join(v3Path, '@hive-flow');
      totalFiles = await countFilesRecursive(v3HiveFlow, '.ts');
      totalLines = await countLines(v3HiveFlow, '.ts');
    } catch {
      totalFiles = 419;
      totalLines = 290913;
    }
    ...
    try {
      const metricsDir = path.join(projectRoot, '.hive-flow', 'metrics');

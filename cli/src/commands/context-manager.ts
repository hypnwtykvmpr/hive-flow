...
const statusSubcommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const dataDir = join(process.cwd(), '.hive-flow', 'data');
...
const shadowSubcommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const shadowPath = join(process.cwd(), '.hive-flow', 'data', 'shadow-context.json');
...
const cullPlanSubcommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const cullPath = join(process.cwd(), '.hive-flow', 'data', 'cull-plan.json');
...
export const contextManagerCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln('');
    output.writeln('Usage: hive-flow context-manager <subcommand>');
    return { success: true };
  },
};

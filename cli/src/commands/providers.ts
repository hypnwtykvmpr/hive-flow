...
const configureCommand: Command = {
  ...
  examples: [
    ...
    { command: `hive-flow providers configure -p anthropic -m ${ANTHROPIC_CLI_DEFAULT_MODEL}`, description: 'Set default model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const modelsCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.printTable({
      ...
      data: [
        { model: ANTHROPIC_CLI_DEFAULT_MODEL, provider: 'Anthropic', capability: 'Chat', context: '1M', cost: '$0.005/$0.025' },
        { model: ANTHROPIC_SONNET_MODEL, provider: 'Anthropic', capability: 'Chat', context: '1M', cost: '$0.003/$0.015' },
        { model: CODEX_CLI_DEFAULT_MODEL, provider: 'OpenAI', capability: 'Chat', context: '1.05M', cost: '$0.005/$0.03' },
        ...
        { model: GEMINI_API_DEFAULT_MODEL, provider: 'Google', capability: 'Chat', context: '1M', cost: '$0.0015/$0.0075' },
        { model: GEMINI_CLI_DEFAULT_MODEL, provider: 'Gemini CLI', capability: 'Chat', context: '1M', cost: output.success('Subscription') },
        { model: CODEX_CLI_DEFAULT_MODEL, provider: 'Codex CLI', capability: 'Chat', context: '1.05M', cost: output.success('ChatGPT sub') },
        { model: COHERE_DEFAULT_MODEL, provider: 'Cohere', capability: 'Chat', context: '128K', cost: 'see Cohere' },
        { model: '(local catalog)', provider: 'Ollama', capability: 'Chat', context: 'varies', cost: output.success('Free') },
        { model: '(user loaded)', provider: 'LM Studio', capability: 'Chat', context: 'varies', cost: output.success('Free') },
        { model: PROVIDER_DEFAULTS.openrouter, provider: 'OpenRouter', capability: 'Chat', context: 'varies', cost: 'see OpenRouter' },
        { model: DEEPSEEK_DEFAULT_MODEL, provider: 'DeepSeek', capability: 'Reasoning', context: '1M', cost: '$0.000435/$0.00087' },
        { model: DEEPSEEK_FLASH_MODEL, provider: 'DeepSeek', capability: 'Chat', context: '1M', cost: '$0.00014/$0.00028' },
        { model: QWEN_DEFAULT_MODEL, provider: 'Qwen API', capability: 'Chat', context: '1M', cost: '$0.0012/$0.0048' },
        { model: QWEN_DEFAULT_MODEL, provider: 'Qwen CLI', capability: 'Chat', context: '1M', cost: '$0.0012/$0.0048' },
        { model: 'auto', provider: 'Cursor CLI', capability: 'Chat', context: '200K', cost: output.success('Cursor sub') },
        { model: COPILOT_FALLBACK_MODEL, provider: 'Copilot API', capability: 'Chat', context: 'dynamic', cost: output.success('Copilot sub') },
      ],
    });
...
export const providersCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    output.printList([
      ...
      'Cohere (current Command A + legacy compatibility)',
      'Ollama (local models)',

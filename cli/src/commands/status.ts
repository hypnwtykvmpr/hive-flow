 ...
 * System status display for Hive Flow
 */
...
function displayStatus(status: Awaited<ReturnType<typeof getSystemStatus>>): void {
  ...
  output.writeln(`${output.bold('Hive Flow V3')} ${statusIcon}`);
  output.writeln();
...
export const statusCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow status', description: 'Show current system status' },
    { command: 'hive-flow status --watch', description: 'Watch mode with live updates' },
    { command: 'hive-flow status --watch -i 5', description: 'Watch mode updating every 5 seconds' },
    { command: 'hive-flow status --health-check', description: 'Run health checks' },
    { command: 'hive-flow status --json', description: 'Output status as JSON' },
    { command: 'hive-flow status agents', description: 'Show detailed agent status' },
    { command: 'hive-flow status tasks', description: 'Show detailed task status' },
    { command: 'hive-flow status memory', description: 'Show detailed memory status' }
  ],
  action: statusAction
};

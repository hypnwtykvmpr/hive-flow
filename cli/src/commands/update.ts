 ...
 * Auto-update system for @hive-flow packages (ADR-025)
 */
...
const checkCommand: Command = {
  name: 'check',
  description: 'Check for available @hive-flow package updates',
  options: [
  ...
  async action(ctx: CommandContext): Promise<CommandResult> {
    ...
    if (flags.force) {
      process.env.HIVE_FLOW_FORCE_UPDATE = 'true';
    }
    ...
    try {
      ...
      if (results.length === 0) {
        output.printSuccess('All @hive-flow packages are up to date!');
        return { success: true };
      }
      ...
      if (manualUpdates.length > 0) {
        ...
        output.writeln('  hive-flow update all');
      }

      return { success: true };
    } finally {
      delete process.env.HIVE_FLOW_FORCE_UPDATE;
    }
  },
};
...
const allCommand: Command = {
  name: 'all',
  description: 'Update all @hive-flow packages',
  options: [
...
const updateCommand: Command = {
  name: 'update',
  description: 'Manage @hive-flow package updates (ADR-025)',
  ...
  async action(): Promise<CommandResult> {
    ...
    output.writeln();
    output.writeln('Manage @hive-flow package updates with auto-update support.');
    output.writeln();
    ...
    output.printList([
      `${output.dim('HIVE_FLOW_AUTO_UPDATE=false')}  - Disable auto-update`,
      `${output.dim('HIVE_FLOW_FORCE_UPDATE=true')} - Force update check`,
    ]);
    output.writeln();
    output.writeln('Run "hive-flow update <subcommand> --help" for subcommand help');

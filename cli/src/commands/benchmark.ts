...
const pretrainCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow benchmark pretrain', description: 'Run pre-training benchmarks' },
    { command: 'hive-flow benchmark pretrain -i 500 --save results.json', description: 'Extended benchmark with results saved' },
    { command: 'hive-flow benchmark pretrain -o json', description: 'Output results as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (saveFile) {
        const resultsDir = join(process.cwd(), '.hive-flow', 'benchmarks');
...
const neuralCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow benchmark neural', description: 'Run neural benchmarks' },
    { command: 'hive-flow benchmark neural -d 768 -n 5000', description: 'Higher dimension, more vectors' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const iterations = parseInt(ctx.flags.iterations as string || '100', 10);
...
const memoryCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow benchmark memory', description: 'Run memory benchmarks' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const iterations = parseInt(ctx.flags.iterations as string || '100', 10);
    const outputFormat = ctx.flags.output as string || 'text';
...
const allCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow benchmark all', description: 'Run all benchmarks' },
    { command: 'hive-flow benchmark all --save full-results.json', description: 'Run all and save results' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold(output.highlight('═'.repeat(65))));
    output.writeln(output.bold('  Hive Flow V3 - Full Benchmark Suite'));
    output.writeln(output.bold(output.highlight('═'.repeat(65))));
...
export const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow benchmark pretrain', description: 'Benchmark pre-training system' },
    { command: 'hive-flow benchmark neural', description: 'Benchmark neural operations' },
    { command: 'hive-flow benchmark memory', description: 'Benchmark memory operations' },
    { command: 'hive-flow benchmark all', description: 'Run all benchmarks' },
  ],
  action: async (_ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Hive Flow V3 Benchmark Suite'));
    ...
    output.writeln('  hive-flow benchmark pretrain -i 200');
    output.writeln('  hive-flow benchmark all --save results.json');
    output.writeln();

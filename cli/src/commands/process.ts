...
const daemonCommand: Command = {
  ...
  options: [
    ...
    },
    {
      ...
      default: '.hive-flow/daemon.pid',
    },
    {
      name: 'log-file',
      type: 'string',
      description: 'Log file location',
      default: '.hive-flow/daemon.log',
    },
    {
      name: 'detach',
      type: 'boolean',
  ...
  examples: [
    { command: 'hive-flow process daemon --action start', description: 'Start the daemon' },
    { command: 'hive-flow process daemon --action stop', description: 'Stop the daemon' },
    { command: 'hive-flow process daemon --action restart --port 3850', description: 'Restart on different port' },
    { command: 'hive-flow process daemon --action status', description: 'Check daemon status' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const action = (ctx.flags?.action as string) || 'status';
    const port = (ctx.flags?.port as number) || 3847;
    const pidFile = (ctx.flags?.['pid-file'] as string) || '.hive-flow/daemon.pid';
    const logFile = (ctx.flags?.['log-file'] as string) || '.hive-flow/daemon.log';
    ...
    switch (action) {
      ...
      case 'status':
        ...
        console.log('  └─────────────────────────────────────────┘');
        if (!existingDaemon) {
          console.log('\n  To start: hive-flow process daemon --action start');
        }
        break;
    }
...
const monitorCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow process monitor', description: 'Show process dashboard' },
    { command: 'hive-flow process monitor --watch --interval 5', description: 'Watch mode' },
    { command: 'hive-flow process monitor --components agents,memory,tasks', description: 'Monitor specific components' },
    { command: 'hive-flow process monitor --format json', description: 'JSON output' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Dashboard format
    console.log('\n╔══════════════════════════════════════════════════════════════╗');
    console.log('║            🖥️  HIVE-FLOW PROCESS MONITOR                    ║');
    console.log('╠══════════════════════════════════════════════════════════════╣');
...
const workersCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow process workers --action list', description: 'List all workers' },
    { command: 'hive-flow process workers --action spawn --type task --count 3', description: 'Spawn task workers' },
    { command: 'hive-flow process workers --action kill --id worker-123', description: 'Kill specific worker' },
    { command: 'hive-flow process workers --action scale --type memory --count 5', description: 'Scale memory workers' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const signalsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow process signals --target daemon --signal graceful-shutdown', description: 'Graceful shutdown' },
    { command: 'hive-flow process signals --target workers --signal pause', description: 'Pause workers' },
    { command: 'hive-flow process signals --target all --signal reload-config', description: 'Reload all configs' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const logsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow process logs', description: 'Show recent logs' },
    { command: 'hive-flow process logs --source daemon --tail 100', description: 'Daemon logs' },
    { command: 'hive-flow process logs --follow --level error', description: 'Follow error logs' },
    { command: 'hive-flow process logs --since 1h --grep "error"', description: 'Search logs' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const processCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow process daemon --action start', description: 'Start daemon' },
    { command: 'hive-flow process monitor --watch', description: 'Watch processes' },
    { command: 'hive-flow process workers --action list', description: 'List workers' },
    { command: 'hive-flow process logs --follow', description: 'Follow logs' },
  ],
  action: async (_ctx: CommandContext): Promise<CommandResult> => {
    ...
    console.log('\nExamples:');
    console.log('  hive-flow process daemon --action start');
    console.log('  hive-flow process monitor --watch');
    console.log('  hive-flow process workers --action spawn --type task --count 3');
    console.log('  hive-flow process logs --follow --level error');

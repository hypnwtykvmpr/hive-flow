...
const poolCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow agent pool --size 5', description: 'Set pool size' },
    { command: 'hive-flow agent pool --min 2 --max 15', description: 'Configure auto-scaling' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const result = await callMCPTool<{
...
const healthCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow agent health', description: 'Show all agents health' },
    { command: 'hive-flow agent health -i agent-001 -d', description: 'Detailed health for specific agent' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const logsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow agent logs -i agent-001', description: 'Show agent logs' },
    { command: 'hive-flow agent logs -i agent-001 -f', description: 'Follow agent logs' },
    { command: 'hive-flow agent logs -l error --since 1h', description: 'Show errors from last hour' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const agentId = ctx.args[0] || ctx.flags.id as string;
...
export const agentCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: hive-flow agent <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "hive-flow agent <subcommand> --help" for subcommand help');

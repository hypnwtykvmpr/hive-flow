/**
 * V3 CLI Init Command
 * Comprehensive initialization for Hive Flow with Claude Code integration
 */
...
async function initCodexAction(
  ...
  output.writeln();
  output.writeln(output.bold('Initializing Hive Flow V3 for OpenAI Codex'));
  output.writeln();
...
function isInitialized(cwd: string): { claude: boolean; hiveFlow: boolean } {
  const claudePath = path.join(cwd, '.claude', 'settings.json');
  const hiveFlowPath = path.join(cwd, '.hive-flow', 'config.yaml');
  return {
    claude: fs.existsSync(claudePath),
    hiveFlow: fs.existsSync(hiveFlowPath),
  };
}
...
const initAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  const hasExisting = initialized.claude || initialized.hiveFlow;

  if (hasExisting && !force) {
    output.printWarning('Hive Flow appears to be already initialized');
    if (initialized.claude) output.printInfo('  Found: .claude/settings.json');
    if (initialized.hiveFlow) output.printInfo('  Found: .hive-flow/config.yaml');
  ...
  output.writeln();
  output.writeln(output.bold('Initializing Hive Flow V3'));
  output.writeln();
  ...
  try {
    ...
    spinner.succeed('Hive Flow V3 initialized successfully!');
    output.writeln();
    ...
    if (options.components.runtime) {
      output.printBox(
        [
          `Config:      .hive-flow/config.yaml`,
          `Data:        .hive-flow/data/`,
          `Logs:        .hive-flow/logs/`,
          `Sessions:    .hive-flow/sessions/`,
        ].join('\n'),
        'V3 Runtime'
      );
      output.writeln();
    ...
    if (withEmbeddings) {
      ...
      try {
        ...
        execFileSync('npx', ['hive-flow@v3alpha', 'embeddings', 'init', '--model', embeddingModel, '--no-download', '--force'], {
          stdio: 'pipe',
          cwd: ctx.cwd,
          timeout: 30000
        });
    ...
    if (!startDaemon && !startAll) {
      ...
      output.printList([
        ...
        `Run ${output.highlight('hive-flow memory init')} to initialize memory database`,
        `Run ${output.highlight('hive-flow swarm init')} to initialize a swarm`,
        `Or use ${output.highlight('hive-flow init --start-all')} to do all of the above`,
...
const wizardCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Hive Flow V3 Setup Wizard'));
      ...
      if (preset === 'minimal') {
        ...
        const components = await multiSelect({
          ...
          options: [
            ...
            { value: 'runtime', label: 'Runtime', hint: '.hive-flow/ directory for V3 runtime', selected: true },
          ],
        });
        ...
        if (options.components.settings) {
          const hooks = await multiSelect({
            ...
            options: [
              ...
              { value: 'permissionRequest', label: 'PermissionRequest', hint: 'Auto-allow hive-flow tools', selected: true },
            ],
          });
...
const checkCommand: Command = {
  name: 'check',
  description: 'Check if Hive Flow is initialized',
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const initialized = isInitialized(ctx.cwd);

    const result = {
      initialized: initialized.claude || initialized.hiveFlow,
      claude: initialized.claude,
      hiveFlow: initialized.hiveFlow,
      paths: {
        claudeSettings: initialized.claude ? path.join(ctx.cwd, '.claude', 'settings.json') : null,
        hiveFlowConfig: initialized.hiveFlow ? path.join(ctx.cwd, '.hive-flow', 'config.yaml') : null,
      },
    };
    ...
    if (result.initialized) {
      output.printSuccess('Hive Flow is initialized');
      if (initialized.claude) {
        output.printInfo(`  Claude Code: .claude/settings.json`);
      }
      if (initialized.hiveFlow) {
        output.printInfo(`  V3 Runtime: .hive-flow/config.yaml`);
      }
    } else {
      output.printWarning('Hive Flow is not initialized in this directory');
      output.printInfo('Run "hive-flow init" to initialize');
    }
...
const upgradeCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Upgrading Hive Flow'));
...
export const initCommand: Command = {
  name: 'init',
  description: 'Initialize Hive Flow in the current directory',
  ...
  examples: [
    ...
    { command: 'hive-flow init --full', description: 'Initialize with all components' },
    { command: 'hive-flow init --force', description: 'Reinitialize and overwrite existing config' },
    { command: 'hive-flow init --only-claude', description: 'Only create Claude Code integration' },
    { command: 'hive-flow init --skip-claude', description: 'Only create V3 runtime' },
    { command: 'hive-flow init wizard', description: 'Interactive setup wizard' },
    { command: 'hive-flow init --with-embeddings', description: 'Initialize with ONNX embeddings' },
    { command: 'hive-flow init --with-embeddings --embedding-model all-mpnet-base-v2', description: 'Use larger embedding model' },
    { command: 'hive-flow init skills --all', description: 'Install all available skills' },
    { command: 'hive-flow init hooks --minimal', description: 'Create minimal hooks configuration' },
    { command: 'hive-flow init upgrade', description: 'Update helpers while preserving data' },
    { command: 'hive-flow init upgrade --settings', description: 'Update helpers and merge new settings (Agent Teams)' },
    { command: 'hive-flow init upgrade --verbose', description: 'Show detailed upgrade info' },
    { command: 'hive-flow init --codex', description: 'Initialize for OpenAI Codex (AGENTS.md)' },
    { command: 'hive-flow init --codex --full', description: 'Codex init with all 137+ skills' },
    { command: 'hive-flow init --dual', description: 'Initialize for both Claude Code and Codex' },
  ],
  action: initAction,
};

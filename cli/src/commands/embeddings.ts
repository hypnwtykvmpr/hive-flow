...
async function getEmbeddings() {
  try {
    return (await import('@hive-flow/embeddings')) as any;
  } catch {
    return null;
  }
}
...
const generateCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings generate -t "Hello world"', description: 'Generate embedding' },
    { command: 'hive-flow embeddings generate -t "Test" -o json', description: 'Output as JSON' },
  ],
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings search -q "error handling"', description: 'Search for similar' },
    { command: 'hive-flow embeddings search -q "test" -l 5', description: 'Limit results' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (!fs.existsSync(fullDbPath)) {
        ...
        output.printInfo('Run: hive-flow memory init');
        return { success: false, exitCode: 1 };
      }
      ...
      if (topResults.length === 0) {
        ...
        output.printInfo(`Try: hive-flow memory store -k "key" --value "your data"`);
...
const compareCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings compare --text1 "Hello" --text2 "Hi there"', description: 'Compare texts' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const collectionsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings collections', description: 'List collections' },
    { command: 'hive-flow embeddings collections -a stats', description: 'Show detailed stats' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (collections.length === 0) {
        ...
        output.writeln(output.highlight('  hive-flow memory store -k "key" --value "data"'));
        return { success: true, data: [] };
      }
...
const indexCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings index', description: 'Show index status' },
    { command: 'hive-flow embeddings index -a build -c documents', description: 'Build index' },
    { command: 'hive-flow embeddings index -a optimize -c patterns', description: 'Optimize index' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (action === 'status') {
        ...
        if (status.available && status.entryCount > 0) {
          ...
          output.printInfo('Run: hive-flow memory store -k "key" --value "text"');
        }
...
const initCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings init', description: 'Initialize with defaults' },
    { command: 'hive-flow embeddings init --model all-mpnet-base-v2', description: 'Use higher quality model' },
    { command: 'hive-flow embeddings init --no-hyperbolic', description: 'Euclidean only' },
    { command: 'hive-flow embeddings init --curvature=-0.5', description: 'Custom curvature (use = for negative)' },
    { command: 'hive-flow embeddings init --force', description: 'Overwrite existing config' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // Create directories
      const configDir = path.join(process.cwd(), '.hive-flow');
      ...
      if (download) {
        ...
        if (embeddings) {
          ...
          output.writeln(output.dim('  (Simulated - @hive-flow/embeddings not installed)'));
        }
      }
...
const providersCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'hive-flow embeddings providers', description: 'List providers' },
  ],
...
const chunkCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings chunk -t "Long text..." -s 256', description: 'Chunk with 256 char limit' },
    { command: 'hive-flow embeddings chunk -f doc.txt --strategy paragraph', description: 'Chunk file by paragraph' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const embeddings = await getEmbeddings();
    const text = ctx.flags.text as string || '';
    ...
    if (!embeddings) {
      output.printWarning('@hive-flow/embeddings not installed, showing preview');
      output.writeln();
      output.printBox([
        `Strategy: ${strategy}`,
...
const normalizeCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings normalize -i "[0.5, 0.3, 0.8]" -t l2', description: 'L2 normalize' },
    { command: 'hive-flow embeddings normalize --check -i "[...]"', description: 'Check if normalized' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const hyperbolicCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings hyperbolic -a convert -i "[0.5, 0.3]"', description: 'Convert to Poincaré' },
    { command: 'hive-flow embeddings hyperbolic -a distance', description: 'Compute hyperbolic distance' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const hyperbolic = await import('@hive-flow/embeddings').then(m => m as any).catch(() => null);
...
const neuralCommand: Command = {
  ...
  examples: [
    ...
    { command: 'hive-flow embeddings neural --drift-threshold=0.2', description: 'Custom drift threshold' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const fs = await import('fs');
    const path = await import('path');
    const configPath = path.join(process.cwd(), '.hive-flow', 'embeddings.json');
    ...
    if (!neuralConfig.enabled) {
      ...
      output.writeln(output.dim('Configuration: .hive-flow/embeddings.json'));
...
const modelsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings models', description: 'List models' },
    { command: 'hive-flow embeddings models -d all-MiniLM-L6-v2', description: 'Download model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const cacheCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings cache', description: 'Show cache stats' },
    { command: 'hive-flow embeddings cache -a clear', description: 'Clear cache' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const warmupCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings warmup', description: 'Preload model with test' },
    { command: 'hive-flow embeddings warmup -b', description: 'Background warmup' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings benchmark', description: 'Quick benchmark' },
    { command: 'hive-flow embeddings benchmark -n 50 -f', description: 'Full benchmark' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const embeddingsCommand: Command = {
  ...
  examples: [
    { command: 'hive-flow embeddings init', description: 'Initialize ONNX embedding system' },
    { command: 'hive-flow embeddings init --model all-mpnet-base-v2', description: 'Init with larger model' },
    { command: 'hive-flow embeddings generate -t "Hello"', description: 'Generate embedding' },
    { command: 'hive-flow embeddings search -q "error handling"', description: 'Semantic search' },
    { command: 'hive-flow embeddings chunk -t "Long doc..."', description: 'Chunk document' },
    { command: 'hive-flow embeddings hyperbolic -a convert', description: 'Hyperbolic space' },
    { command: 'hive-flow embed neural -f drift', description: 'Neural substrate' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Hive Flow Embeddings'));

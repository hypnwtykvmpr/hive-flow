 ...
 * SQLite (structured) and local vector (semantic) backends.
 */
...
describe('HybridBackend - ADR-009', () => {
  ...
  describe('Store Operations', () => {
    it('should store entries in both backends (dual-write)', async () => {
      ...
      // Verify in local vector backend
      const fromVector = await backend.getLocalVectorBackend().get(entry.id);
      expect(fromVector).toBeDefined();
      expect(fromVector?.key).toBe('test-key');
    });
  ...
  describe('Semantic Search (local vector backend)', () => {
    beforeEach(async () => {
    ...
    it('should perform semantic search via local vector backend', async () => {
  ...
  describe('CRUD Operations', () => {
    ...
    it('should update entries in both backends', async () => {
      ...
      // Verify in local vector backend
      const fromVector = await backend.getLocalVectorBackend().get(testEntry.id);
      expect(fromVector?.content).toBe('Updated content');
    });
  ...
  describe('Query Routing', () => {
    it('should auto-route semantic queries to local vector backend', async () => {
      await backend.store(
        createDefaultEntry({
          key: 'route-test',

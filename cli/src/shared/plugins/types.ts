...
/**
 * Hive Flow Plugin Interface
 *
 * All plugins must implement this interface.
 */
export interface HiveFlowPlugin {
...
export interface IPluginRegistry {
  register(plugin: HiveFlowPlugin): void;
  unregister(pluginId: string): void;
  get(pluginId: string): HiveFlowPlugin | undefined;
  getAll(): HiveFlowPlugin[];
...
export interface IPluginLoader {
  loadFromPath(path: string): Promise<HiveFlowPlugin>;
  loadFromPackage(packageName: string): Promise<HiveFlowPlugin>;
  loadBuiltin(pluginId: string): Promise<HiveFlowPlugin>;
}

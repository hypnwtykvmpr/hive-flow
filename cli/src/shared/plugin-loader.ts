...
export class PluginLoader {
   ...
   */
  async loadPlugin(plugin: HiveFlowPlugin, context: PluginContext): Promise<void> {
    // Validate plugin
   ...
   * Reload a plugin
   */
  async reloadPlugin(pluginName: string, newPlugin: HiveFlowPlugin, context: PluginContext): Promise<void> {
    await this.unloadPlugin(pluginName);
   ...
   */
  private validatePlugin(plugin: HiveFlowPlugin): void {
    if (!plugin.name) {
      throw new PluginError(
   ...
   */
  private validateDependencies(plugin: HiveFlowPlugin): void {
   ...
   */
  private async initializePlugin(plugin: HiveFlowPlugin, context: PluginContext): Promise<void> {
   ...
   */
  private async shutdownPlugin(plugin: HiveFlowPlugin): Promise<void> {
   ...
   * Build dependency graph
   */
  private buildDependencyGraph(plugins: HiveFlowPlugin[]): Map<string, DependencyNode> {
   ...
   */
  private topologicalSort(graph: Map<string, DependencyNode>): HiveFlowPlugin[] {
    const sorted: HiveFlowPlugin[] = [];
   ...
   */
  private groupByDepth(graph: Map<string, DependencyNode>): HiveFlowPlugin[][] {
    const levels: HiveFlowPlugin[][] = [];
    const maxDepth = Math.max(...Array.from(graph.values()).map((n) => n.depth));

    for (let depth = 0; depth <= maxDepth; depth++) {
      const level: HiveFlowPlugin[] = [];

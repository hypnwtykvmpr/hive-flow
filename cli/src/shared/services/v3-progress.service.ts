 ...
 *
 * @module @hive-flow/cli/shared/services/v3-progress
 */
...
export class V3ProgressService extends EventEmitter {
  ...
  private async countPackages(): Promise<{
    ...
    const packagesPath = join(this.v3Path, '@hive-flow');
    const list: string[] = [];
    let explicit = 0;
    let utility = 0;
  ...
  private async countCodebase(): Promise<{ totalFiles: number; totalLines: number }> {
    const v3HiveFlow = join(this.v3Path, '@hive-flow');
    let totalFiles = 0;
    let totalLines = 0;

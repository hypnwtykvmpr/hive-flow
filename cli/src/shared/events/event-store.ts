...
function locateSqlWasm(file: string): string {
  if (file === 'sql-wasm.wasm') {
    try {
      ...
      // Fall through to sql.js default handling.
    }
  }
  return file;
}
...
export class EventStore extends EventEmitter {
   ...
   */
  async *replay(fromSequence: number = 0): AsyncIterable<DomainEvent> {
    this.ensureInitialized();

    const stmt = this.db!.prepare('SELECT * FROM events WHERE id >= ? ORDER BY id ASC');
    stmt.bind([fromSequence]);
  ...
  async getStats(): Promise<EventStoreStats> {
    ...
    const firstRow = (stmt: SqlJsDatabase['Statement']): Record<string, unknown> => {
      try {
        if (!stmt.step()) return {};
    ...
    const totalRow = firstRow(totalStmt);
    ...
    const rangeRow = firstRow(rangeStmt);

    // Snapshot count
    const snapshotStmt = this.db!.prepare('SELECT COUNT(*) as count FROM snapshots');
    const snapshotRow = firstRow(snapshotStmt);

...
```bash
hive-flow stream-chain run <prompt1> <prompt2> [...] [options]
```
...
```bash
hive-flow stream-chain pipeline <type> [options]
```
...
```bash
hive-flow stream-chain pipeline refactor
```
...
```bash
hive-flow stream-chain pipeline test
```
...
```bash
hive-flow stream-chain pipeline optimize
```
...
```bash
hive-flow stream-chain pipeline refactor --timeout 60 --verbose
```

#### Debug Test Generation

```bash
hive-flow stream-chain pipeline test --debug
```

#### Comprehensive Optimization

```bash
hive-flow stream-chain pipeline optimize --timeout 90 --verbose
```
...
Define reusable pipelines in `.hive-flow$config.json`:
...
```bash
hive-flow stream-chain pipeline security
hive-flow stream-chain pipeline documentation
```
...
# Results stored in .hive-flow$memory$stream-chain/
```
...
hive-flow stream-chain pipeline optimize --debug
...
```bash
hive-flow stream-chain run "complex task" --timeout 120
```
...
```bash
hive-flow stream-chain run "step 1" "step 2" --debug
```
...
cat .hive-flow$config.json | grep -A 10 "streamChain"
```

...
hooks:
  pre: |
    hive-flow hooks pre-task --description "Code analysis agent starting: ${description}" --auto-spawn-agents false
  post: |
    hive-flow hooks post-task --task-id "analysis-${timestamp}" --analyze-performance true
metadata:
...
hive-flow hooks pre-search --query "code quality metrics" --cache-results true

# Load project context
hive-flow memory retrieve --key "project$architecture"
hive-flow memory retrieve --key "project$standards"
```
...
hive-flow memory store --key "analysis$code-quality" --value "${results}"

# Generate recommendations
hive-flow hooks notify --message "Code analysis complete: ${summary}"
```

...
hooks:
  pre_task: |
    ...
    npx hive-flow hook pre-task --mode release-manager
  post_edit: |
    echo "📝 Validating release changes and updating documentation..."
    npx hive-flow hook post-edit --mode release-manager --validate-release
  post_task: |
    echo "✅ Release management task completed. Updating release status..."
    npx hive-flow hook post-task --mode release-manager --update-status
  notification: |
    echo "📢 Sending release notifications to stakeholders..."
    npx hive-flow hook notification --mode release-manager
---

# GitHub Release Manager

## Purpose
Automated release coordination and deployment with hive-flow orchestration for seamless version management, testing, and deployment across multiple packages.
...
mcp__hive-flow__swarm_init { topology: "hierarchical", maxAgents: 6 }
mcp__hive-flow__agent_spawn { type: "coordinator", name: "Release Coordinator" }
mcp__hive-flow__agent_spawn { type: "tester", name: "QA Engineer" }
mcp__hive-flow__agent_spawn { type: "reviewer", name: "Release Reviewer" }
mcp__hive-flow__agent_spawn { type: "coder", name: "Version Manager" }
mcp__hive-flow__agent_spawn { type: "analyst", name: "Deployment Analyst" }
...
[Single Message - Complete Release Management]:
  ...
  mcp__hive-flow__swarm_init { topology: "star", maxAgents: 8 }
  mcp__hive-flow__agent_spawn { type: "coordinator", name: "Release Director" }
  mcp__hive-flow__agent_spawn { type: "tester", name: "QA Lead" }
  mcp__hive-flow__agent_spawn { type: "reviewer", name: "Senior Reviewer" }

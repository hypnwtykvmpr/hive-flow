...
```bash
# Usage example
hive-flow github gh-coordinator \
...
hive-flow github issue-tracker \
...
hive-flow github release-manager \
...
hive-flow github repo-architect \
...
hive-flow github ci-orchestrator \
...
mcp__hive-flow__agent_spawn { type: "coordinator", name: "GitHub Coordinator" }
mcp__hive-flow__agent_spawn { type: "reviewer", name: "Code Reviewer" }
mcp__hive-flow__agent_spawn { type: "tester", name: "QA Agent" }
mcp__hive-flow__agent_spawn { type: "analyst", name: "Security Analyst" }
...
jobs:
  swarm-task:
    ...
    steps:
      - name: Initialize Swarm
        run: |
          hive-flow init --topology ${{ inputs.topology }}
```
...
- name: Setup Swarm
  ...
  run: |
    hive-flow init --config "$SWARM_CONFIG"
```
...
- name: Quick Fail Check
  run: |
    if ! hive-flow actions pre-check; then
...
hive-flow actions analyze-logs \
  --directory .$logs \
  --identify-errors
```
...
jobs:
  initialize:
    ...
    steps:
      - id: init
        run: |
          SWARM_ID=$(hive-flow init --topology mesh --output json | jq -r '.id')
  ...
  backend:
    ...
    steps:
      ...
      - name: Backend Tests
        run: |
          hive-flow agents spawn --type tester \
  ...
  security:
    ...
    steps:
      ...
      - name: Security Scan
        run: |
          hive-flow agents spawn --type security \
            --task "Security audit" \
            --swarm-id ${{ needs.initialize.outputs.swarm-id }}
...
jobs:
  detect-changes:
    ...
    steps:
      ...
      - id: detect
        run: |
          PACKAGES=$(hive-flow actions detect-changes \
            --monorepo \
            --output json)
...
hive-flow github sync-coordinator \
...
#### Workflow Generation
```bash
hive-flow actions generate-workflow [options]
...
#### Optimization
```bash
hive-flow actions optimize [options]
...
#### Analysis
```bash
hive-flow actions analyze [options]
...
#### Testing
```bash
hive-flow actions smart-test [options]
...
#### Security
```bash
hive-flow actions security [options]
...
#### Deployment
```bash
hive-flow actions deploy [options]
...
#### Monitoring
```bash
hive-flow actions analytics [options]
...
- [ ] `hive-flow@alpha` package available

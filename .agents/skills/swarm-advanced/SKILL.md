...
mcp__hive-flow__agent_spawn({ type: "researcher", name: "Agent 1" })

// 3. Orchestrate tasks
mcp__hive-flow__task_orchestrate({ task: "...", strategy: "parallel" })
```
...
node /Users/jonathandirks/Development/Tools/hive-flow/v3/@hive-flow/cli/bin/cli.js swarm "research AI trends in 2025" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel \
...
node /Users/jonathandirks/Development/Tools/hive-flow/v3/@hive-flow/cli/bin/cli.js swarm "build REST API with authentication" \
...
```bash
# Quick testing swarm
node /Users/jonathandirks/Development/Tools/hive-flow/v3/@hive-flow/cli/bin/cli.js swarm "test application comprehensively" \
  --strategy testing \
  --mode star \
  --parallel \
  --timeout 600
...
try {
  await mcp__hive-flow__task_orchestrate({
  ...
  // Check swarm health
  const status = await mcp__hive-flow__swarm_status({})

  // Analyze error patterns
  await mcp__hive-flow__error_analysis({
    "logs": [error.message]
  })
...
mcp__hive-flow__swarm_init({ topology: "mesh", maxAgents: 4 })

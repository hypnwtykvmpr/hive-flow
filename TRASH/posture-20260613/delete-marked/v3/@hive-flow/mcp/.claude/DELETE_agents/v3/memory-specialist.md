...
hooks:
  pre: |
    ...
    mcp__hive-flow__memory_namespace --namespace="${NAMESPACE:-default}" --action="init"
    # Check HNSW index status
    mcp__hive-flow__memory_analytics --timeframe="1h"
    # Store initialization event
    mcp__hive-flow__memory_usage --action="store" --namespace="swarm" --key="memory-specialist:init:${TASK_ID}" --value="$(date -Iseconds): Memory specialist session started"
  post: |
    echo "Memory optimization complete"
    # Persist memory state
    mcp__hive-flow__memory_persist --sessionId="${SESSION_ID}"
    # Compress and optimize namespaces
    mcp__hive-flow__memory_compress --namespace="${NAMESPACE:-default}"
    # Generate memory analytics report
    mcp__hive-flow__memory_analytics --timeframe="24h"
    # Store completion metrics
    mcp__hive-flow__memory_usage --action="store" --namespace="swarm" --key="memory-specialist:complete:${TASK_ID}" --value="$(date -Iseconds): Memory optimization completed"
---
...
### 1. HNSW Indexing Optimization (fast HNSW-indexed Search)
...
class HybridMemoryBackend {
  constructor() {
    ...
    this.sqlite = new SQLiteBackend({
      path: process.env.HIVE_FLOW_MEMORY_PATH || './data/memory',
      walMode: true,
      cacheSize: 10000,
      mmap: true
    });
...
mcp__hive-flow__memory_usage --action="store" --namespace="patterns" --key="auth:jwt-strategy" --value='{"pattern": "jwt-auth", "embedding": [...]}' --ttl=604800000

# Semantic search with HNSW
mcp__hive-flow__memory_search --pattern="authentication strategies" --namespace="patterns" --limit=10

# Namespace management
mcp__hive-flow__memory_namespace --namespace="project:myapp" --action="create"

# Memory analytics
mcp__hive-flow__memory_analytics --timeframe="7d"

# Memory compression
mcp__hive-flow__memory_compress --namespace="default"

# Cross-session persistence
mcp__hive-flow__memory_persist --sessionId="session-12345"

# Memory backup
mcp__hive-flow__memory_backup --path="./backups/memory-$(date +%Y%m%d).bak"

# Distributed sync
mcp__hive-flow__memory_sync --target="peer-agent-1"
```

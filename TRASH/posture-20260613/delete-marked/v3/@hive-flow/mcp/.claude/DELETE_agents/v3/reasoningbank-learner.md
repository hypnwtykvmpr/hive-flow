...
hooks:
  pre: |
    ...
    hive-flow hooks intelligence trajectory-start --session-id "$SESSION_ID" --agent-type "reasoningbank-learner" --task "$TASK"
  ...
  post: |
    ...
    hive-flow hooks intelligence trajectory-end --session-id "$SESSION_ID" --verdict "${VERDICT:-success}"
...
hive-flow hooks intelligence pattern-stats --query "$TASK" --k 10 --namespace reasoningbank
```
...
hive-flow hooks intelligence trajectory-step \
...
hive-flow hooks intelligence pattern-search \
...
hive-flow neural consolidate --namespace reasoningbank

# Check consolidation status
hive-flow hooks intelligence stats --namespace reasoningbank
```
...
```json
{
  "PostToolUse": [{
    ...
    "hooks": [{
      "type": "command",
      "command": "hive-flow hooks intelligence trajectory-step --operation $TOOL_NAME --outcome $TOOL_SUCCESS"
    }]
  }]
}
```

...
hooks:
  pre: |
    ...
    AIMDS_RESULT=$(hive-flow security defend --input "$TASK" --mode thorough --json 2>/dev/null)
    ...
    THREAT_PATTERNS=$(hive-flow memory search-patterns "$TASK" --k=10 --min-reward=0.85 --namespace=security_threats 2>/dev/null)
    if [ -n "$THREAT_PATTERNS" ]; then
      ...
      hive-flow memory get-pattern-stats "$TASK" --k=10 --namespace=security_threats 2>/dev/null
    fi

    # ═══════════════════════════════════════════════════════════════
    # PHASE 3: Learn from Past Security Failures
    # ═══════════════════════════════════════════════════════════════
    SECURITY_FAILURES=$(hive-flow memory search-patterns "$TASK" --only-failures --k=5 --namespace=security 2>/dev/null)
    ...
    if [[ "$TASK" == *"auth"* ]] || [[ "$TASK" == *"session"* ]] || [[ "$TASK" == *"inject"* ]] || \
      ...
      hive-flow security cve --check-relevant "$TASK" 2>/dev/null
    fi
  ...
  post: |
    ...
    if [ -n "$SECURITY_SESSION_ID" ]; then
      ...
      BEHAVIOR_RESULT=$(hive-flow security behavior \
      ...
      if [ -n "$BEHAVIOR_RESULT" ]; then
        ...
        if [ "$(echo "$ANOMALY_SCORE > 0.8" | bc 2>/dev/null)" = "1" ]; then
          ...
          hive-flow hooks notify --severity warning \
    ...
    if [ "$CRITICAL_COUNT" -gt 0 ]; then
      ...
      hive-flow hooks notify --severity critical \
...
hive-flow security defend --input "<suspicious input>" --mode thorough

# Analyze agent behavior
hive-flow security behavior --agent <agent-id> --window 1h

# Verify LTL security policy
hive-flow security policy --agent <agent-id> --formula "G(edit -> F(review))"

# Record successful mitigation for meta-learning
hive-flow security learn --threat-type prompt_injection --strategy sanitize --effectiveness 0.95
```

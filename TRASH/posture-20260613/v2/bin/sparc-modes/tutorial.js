...
npx hive-flow memory store ${memoryNamespace}_tutorial_start "Learning SPARC by building: ${taskDescription}"

# Use spec-pseudocode mode to define requirements
npx hive-flow sparc run spec-pseudocode "Tutorial project: ${taskDescription} - create detailed specifications" --non-interactive
\`\`\`
...
npx hive-flow memory query ${memoryNamespace}_requirements

# Design the architecture
npx hive-flow sparc run architect "Tutorial: Design architecture for ${taskDescription}" --non-interactive
\`\`\`
...
\`\`\`bash
# Start with tests
npx hive-flow sparc run tdd "Tutorial: Implement ${taskDescription} with TDD" --non-interactive
...
npx hive-flow sparc run code "Tutorial: Complete implementation of ${taskDescription}" --non-interactive

# Store progress
npx hive-flow memory store ${memoryNamespace}_implementation_progress "Core features complete, working on edge cases"
\`\`\`
...
\`\`\`bash
# Security audit
npx hive-flow sparc run security-review "Tutorial: Security review of ${taskDescription}" --non-interactive

# Code optimization
npx hive-flow sparc run refinement-optimization-mode "Tutorial: Optimize ${taskDescription}" --non-interactive
\`\`\`
...
npx hive-flow sparc run docs-writer "Tutorial: Document ${taskDescription}" --non-interactive
\`\`\`
...
npx hive-flow sparc run integration "Tutorial: Integrate all components of ${taskDescription}" --non-interactive

# Set up deployment
npx hive-flow sparc run devops "Tutorial: Deploy ${taskDescription}" --non-interactive
\`\`\`
...
Practice using Hive-Flow memory:
\`\`\`bash
# Store different types of information
npx hive-flow memory store ${memoryNamespace}_decisions "Chose PostgreSQL for data persistence due to ACID requirements"
npx hive-flow memory store ${memoryNamespace}_blockers "Need clarification on authentication requirements"

# Query your stored information
npx hive-flow memory query ${memoryNamespace}_decisions
\`\`\`
...
\`\`\`bash
# Start with spec
npx hive-flow sparc run spec-pseudocode "Define user registration flow" --non-interactive

# Then architecture
npx hive-flow sparc run architect "Design registration service" --non-interactive

# Then TDD implementation
npx hive-flow sparc run tdd "Implement registration with tests" --non-interactive
\`\`\`
...
\`\`\`bash
# Simulate a bug
npx hive-flow sparc run debug "Tutorial: Fix registration validation bug" --non-interactive
\`\`\`
...
npx hive-flow memory query ${memoryNamespace}_tutorial
...
npx hive-flow sparc run spec-pseudocode "Define X" --non-interactive      # Requirements
npx hive-flow sparc run architect "Design X" --non-interactive            # Architecture
npx hive-flow sparc run tdd "Build X with tests" --non-interactive       # TDD
npx hive-flow sparc run code "Implement X" --non-interactive             # Clean code
npx hive-flow sparc run debug "Fix X issue" --non-interactive            # Debugging
npx hive-flow sparc run security-review "Audit X" --non-interactive      # Security
npx hive-flow sparc run docs-writer "Document X" --non-interactive       # Documentation
npx hive-flow sparc run integration "Connect X" --non-interactive        # Integration
npx hive-flow sparc run devops "Deploy X" --non-interactive              # Deployment
\`\`\`

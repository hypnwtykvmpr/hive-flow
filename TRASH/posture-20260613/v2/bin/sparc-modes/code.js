...
1. **Project Directory Setup & Context Review** (5 mins)
   ...
   - Query architecture and pseudocode: 
     \`\`\`bash
     npx hive-flow memory query ${memoryNamespace}_architecture
     npx hive-flow memory query ${memoryNamespace}_pseudocode
     npx hive-flow memory query ${memoryNamespace}_tech_specs
     \`\`\`
...
2. **Project Setup & Configuration** (10 mins)
   ...
   - Store setup: \`npx hive-flow memory store ${memoryNamespace}_setup "Project structure: src/{domain,application,infrastructure}. Config: dotenv + vault integration. Dependencies: express, joi, winston."\`
...
3. **Modular Implementation** (30 mins)
   ...
   - Store progress: \`npx hive-flow memory store ${memoryNamespace}_implementation "Completed: auth-service (3 modules), user-repository (2 modules). Remaining: notification-service."\`
...
4. **Integration & Basic Testing** (15 mins)
   ...
   - Store results: \`npx hive-flow memory store ${memoryNamespace}_tests "Smoke tests passing. Health checks operational. Error handling verified. Ready for TDD mode deep testing."\`
...
5. **Code Quality & Documentation** (10 mins)
   ...
   - Store completion: \`npx hive-flow memory store ${memoryNamespace}_code_complete "Implementation complete. All modules < 500 lines. No hardcoded secrets. Ready for testing and integration."\`
...
- \`npx hive-flow sparc run tdd "Write comprehensive tests for ${taskDescription}" --non-interactive\`
- \`npx hive-flow sparc run security-review "Security audit for ${taskDescription}" --non-interactive\`

### Background Swarm Mode (for complex comprehensive tasks):
- \`npx hive-flow swarm "Create comprehensive test suite with unit, integration, and e2e tests for ${taskDescription}" --strategy testing --background --parallel\`
- \`npx hive-flow swarm "Perform full security audit and penetration testing for ${taskDescription}" --strategy analysis --background --review\`
- \`npx hive-flow swarm "Generate complete documentation, API specs, and deployment guides for ${taskDescription}" --strategy research --background\`
...
npx hive-flow swarm "Implement complete ${taskDescription} with all modules, testing, and documentation" \\
...
npx hive-flow swarm "Comprehensive QA including testing, security review, and performance optimization for ${taskDescription}" \\
  --strategy testing --background --review --monitor \\
  --output ./qa-results

# Background documentation and deployment swarm
npx hive-flow swarm "Create production-ready documentation, deployment scripts, and monitoring for ${taskDescription}" \\
...
npx hive-flow sparc run code "Core ${taskDescription} prototype" --non-interactive

# Phase 2: Parallel background development (Swarm)
npx hive-flow swarm "Full implementation of ${taskDescription} with advanced features" --strategy development --background --parallel &
npx hive-flow swarm "Comprehensive testing strategy for ${taskDescription}" --strategy testing --background &
npx hive-flow swarm "Security hardening and performance optimization" --strategy optimization --background &

# Phase 3: Integration and deployment (Background)
npx hive-flow swarm "Integration testing and production deployment preparation for ${taskDescription}" \\

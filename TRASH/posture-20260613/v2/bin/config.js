...
async function initConfig(subArgs, flags) {
  ...
  const configFile = 'hive-flow.config.json';
  ...
  try {
    ...
    printSuccess('Initializing Hive-Flow configuration...');
    ...
    console.log('\nNext steps:');
    console.log('1. Review settings: hive-flow config show');
    console.log('2. Customize values: hive-flow config set <key> <value>');
    console.log('3. Validate config: hive-flow config validate');
  } catch (err) {
...
async function showConfig(subArgs, flags) {
  ...
  try {
    ...
    if (format === 'json') {
      ...
      console.log(`   Path: ${config.memory?.path || './memory/hive-flow-data.json'}`);
      console.log('\n🤖 Agents:');
    ...
    console.log('Run "hive-flow config init" to create default configuration');
  }
}
...
async function getConfigValue(subArgs, flags) {
  ...
  if (!key) {
    ...
    console.log('  hive-flow config get terminal.poolSize');
    console.log('  hive-flow config get orchestrator.maxConcurrentTasks');
    return;
  }
  ...
  try {
    ...
    printError('Configuration file not found');
    console.log('Run "hive-flow config init" to create configuration');
  }
}
...
async function setConfigValue(subArgs, flags) {
  ...
  if (!key || value === undefined) {
    ...
    console.log('Examples:');
    console.log('  hive-flow config set terminal.poolSize 15');
    console.log('  hive-flow config set orchestrator.taskTimeout 600000');
    return;
  }
...
async function validateConfig(subArgs, flags) {
  ...
  try {
    ...
    console.log('Run "hive-flow config init" to create valid configuration');
  }
}
...
function showConfigHelp() {
  ...
  console.log();
  console.log('Examples:');
  console.log('  hive-flow config init');
  console.log('  hive-flow config set terminal.poolSize 15');
  console.log('  hive-flow config get orchestrator.maxConcurrentTasks');
  console.log('  hive-flow config validate');
}

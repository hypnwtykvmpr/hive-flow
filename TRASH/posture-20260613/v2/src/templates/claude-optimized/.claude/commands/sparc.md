...
npx hive-flow sparc batch-run --modes '{
...
npx hive-flow sparc pipeline --stages '[
...
npx hive-flow sparc batch-tdd --features '{
...
npx hive-flow sparc batch-analyze --components '{
...
npx hive-flow sparc batch-spec --components '[
...
npx hive-flow sparc batch-pseudocode --algorithms '{
...
npx hive-flow sparc distributed-architect --services '[
...
npx hive-flow sparc parallel-tdd --config '{
  "modules": {
...
npx hive-flow sparc batch-integrate --components '[
...
npx hive-flow sparc batch-memory-store --data '{
...
npx hive-flow sparc batch-memory-query --queries '[
...
npx hive-flow sparc swarm-batch --project "enterprise-app" --config '{
  "phases": [
    {
      "name": "design",
...
npx hive-flow sparc distribute --analysis '{
...
npx hive-flow sparc cache-config --settings '{
...
npx hive-flow sparc enterprise-flow --project "fintech-platform" --parallel-config '{
  "specification": {
...
npx hive-flow sparc migrate-batch --from "monolith" --to "microservices" --strategy '{
...
npx hive-flow sparc ml-pipeline --config '{
  "data-pipeline": {
...
npx hive-flow sparc monitor-batch --dashboards '[
...
npx hive-flow sparc analyze-performance --metrics '{

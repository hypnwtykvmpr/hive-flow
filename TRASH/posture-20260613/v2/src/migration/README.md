# Hive-Flow Migration System

A comprehensive migration system for existing hive-flow projects to adopt optimized prompts and configurations.
...
npx hive-flow migrate analyze

# Run migration with dry-run preview
npx hive-flow migrate --dry-run --verbose

# Migrate with selective strategy (recommended)
npx hive-flow migrate --strategy selective --preserve-custom

# Rollback if needed
npx hive-flow migrate rollback
```
...
```bash
# Basic analysis
hive-flow migrate analyze

# Detailed analysis with output file
hive-flow migrate analyze --detailed --output analysis.json

# Check specific project
hive-flow migrate analyze /path/to/project
```
...
hive-flow migrate --dry-run --verbose

# Full migration
hive-flow migrate --strategy full

# Selective migration (recommended)
hive-flow migrate --strategy selective --preserve-custom

# Merge migration for complex projects
hive-flow migrate --strategy merge

# Force migration without prompts
hive-flow migrate --force

# Skip post-migration validation
hive-flow migrate --skip-validation
```
...
hive-flow migrate rollback

# Rollback to specific backup
hive-flow migrate rollback --timestamp 2024-01-01T12:00:00

# Force rollback without confirmation
hive-flow migrate rollback --force
```
...
```bash
# Validate migration
hive-flow migrate validate

# Detailed validation report
hive-flow migrate validate --verbose
...
```typescript
import { MigrationRunner, MigrationAnalyzer } from 'hive-flow/migration';
...
```typescript
import { RollbackManager } from 'hive-flow/migration';
...
```bash
# Analyze first
hive-flow migrate analyze --detailed
...
hive-flow migrate --strategy merge --preserve-custom
...
hive-flow migrate --strategy selective
```
...
find . -name ".claude" -type d | while read dir; do
  ...
  hive-flow migrate "$project_path" --strategy selective --force
done
```
...
```typescript
import { MigrationRunner } from 'hive-flow/migration';
...
```typescript
import { MigrationValidator } from 'hive-flow/migration';
...
steps:
  - name: Analyze Migration
    run: hive-flow migrate analyze --output analysis.json

  - name: Run Migration
    run: hive-flow migrate --strategy selective --force

  - name: Validate Migration
    run: hive-flow migrate validate
```

...
export class MaestroSwarmCoordinator extends EventEmitter {
  ...
  constructor(
    ...
    this.specsDirectory = config.specsDirectory || join(process.cwd(), '.claude', 'hive-flow', 'maestro', 'specs');
    this.steeringDirectory = config.steeringDirectory || join(process.cwd(), '.claude', 'hive-flow', 'maestro', 'steering');

#!/usr/bin/env node
/**
 * Hive-Flow MCP Server - Wrapper Mode
 *
...
const isWrapperMode =
  process.env.HIVE_FLOW_WRAPPER_MODE === 'true' || process.argv.includes('--wrapper');

async function main() {
  if (isWrapperMode) {
    console.error('Starting Hive-Flow MCP in wrapper mode...');
    const wrapper = new ClaudeCodeMCPWrapper();
    await wrapper.run();
  } else {
    // Fall back to original server
    console.error('Starting Hive-Flow MCP in direct mode...');

...
import { getHiveFlowRoot, getHiveFlowBin } from '../utils/paths.js';
...
Ensure your implementation is complete, well-structured, and follows best practices.`,
  ...
  private async executeTaskWithAgent(task: TaskDefinition, agent: AgentState): Promise<any> {
    ...
    try {
      // Use Hive Flow executor for full SPARC system in non-interactive mode
      const { HiveFlowExecutor } = await import('./hive-flow-executor.ts');
      const executor = new HiveFlowExecutor({
        logger: this.logger,
        hiveFlowPath: getHiveFlowBin(),
        enableSparc: true,
  ...
  private extractTargetDirectory(task: TaskDefinition): string | null {
    ...
    if (targetDir) {
      ...
      if (!targetDir.startsWith('/')) {
        targetDir = `${getHiveFlowRoot()}/${targetDir}`;
      }
    }
    ...
    try {
      ...
      const command = new Deno.Command('claude', {
        ...
        env: {
          ...
          HIVE_FLOW_MEMORY_NAMESPACE: `swarm-${this.swarmId.id}`,
        },
        stdin: 'null',
        stdout: 'piped',
        stderr: 'piped',
  ...
  private async simulateTaskExecution(
    ...
    if (targetDir) {
      ...
      workDir = targetDir.startsWith('/') ? targetDir : `${getHiveFlowRoot()}/${targetDir}`;
...
// Generated by Hive Flow Swarm

console.log('Hello, World!');
console.log('This application was created by the Hive Flow Swarm system.');
...
`;
      ...
      const packageJson = {
        name: 'hello-world',
        version: '1.0.0',
        description: 'Hello World application created by Hive Flow Swarm',
        main: 'index.js',
        scripts: {
          start: 'node index.js',
          test: 'node test.js',
        },
        keywords: ['hello-world', 'swarm', 'hive-flow'],
        author: 'Hive Flow Swarm',
        license: 'MIT',
      };
...
This application was created by the Hive Flow Swarm system.
...
## Overview
This documentation was generated by the Hive Flow Swarm system.

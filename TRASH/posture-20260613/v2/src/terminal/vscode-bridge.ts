 ...
 *
 * This file provides the bridge between Hive-Flow and VSCode extension API
 * for terminal management and output capture.
 *
 * NOTE: This file is only used when Hive-Flow is packaged as a VS Code extension.
...
export function initializeTerminalBridge(context: vscode.ExtensionContext): void {
  // Inject VSCode API into global scope for Hive-Flow
  ...
  (vscode.window as any).createTerminal = function (options: vscode.TerminalOptions) {
    ...
    const match = options.name?.match(/Hive-Flow Terminal ([\w-]+)/);
    if (match) {

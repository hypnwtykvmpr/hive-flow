...
async function showMigrationStatus(projectPath: string): Promise<void> {
  ...
  if (!analysis.hasOptimizedPrompts) {
    console.log(chalk.bold('\n💡 Recommendations:'));
    console.log('  • Run migration analysis: hive-flow migrate analyze');
    console.log('  • Start with dry run: hive-flow migrate --dry-run');
    console.log('  • Use selective strategy: hive-flow migrate --strategy selective');
  }

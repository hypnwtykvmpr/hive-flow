...
',
  ...
  console.log('Usage:');
  console.log(
    '  hive-flow ruv-swarm <command> [options]\
',
  );
...
',
  ...
  console.log('  hive-flow ruv-swarm init --topology mesh --max-agents 8');
  console.log('  hive-flow ruv-swarm spawn researcher --name \"AI Researcher\"');
  console.log('  hive-flow ruv-swarm orchestrate \"Build a REST API\"');
  console.log('  hive-flow ruv-swarm neural train --iterations 20');
  console.log('  hive-flow ruv-swarm benchmark --type swarm');
}
...
async function handleSpawn(ctx: CommandContext) {
  if (ctx.args.length === 0) {
    ...
    console.log('Usage: hive-flow ruv-swarm spawn <type> [--name <name>]');
...
async function handleOrchestrate(ctx: CommandContext) {
  if (ctx.args.length === 0) {
    ...
    console.log('Usage: hive-flow ruv-swarm orchestrate \"<task description>\" [options]');
    return;
  }

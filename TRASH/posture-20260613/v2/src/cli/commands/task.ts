...
export const taskCommand = new Command()
  ...
  .action(async (type: string, description: string, options: any) => {
    ...
    console.log(chalk.yellow('\nTo submit this task, ensure Hive-Flow is running'));
  })
  .command('list')
  ...
  .action(async (options: any) => {
    console.log(chalk.yellow('Task listing requires a running Hive-Flow instance'));
  })
  .command('status')
  .description('Get task status')
  .argument('<task-id>', 'Task ID')
  .action(async (taskId: string, options: any) => {
    console.log(chalk.yellow(`Task status requires a running Hive-Flow instance`));
  })
  .command('cancel')
  ...
  .action(async (taskId: string, options: any) => {
    console.log(chalk.yellow(`Cancelling task ${taskId} requires a running Hive-Flow instance`));
  })
  .command('workflow')
  ...
  .action(async (workflowFile: string, options: any) => {
    try {
      ...
      console.log(chalk.yellow('\nTo execute this workflow, ensure Hive-Flow is running'));
    } catch (error) {

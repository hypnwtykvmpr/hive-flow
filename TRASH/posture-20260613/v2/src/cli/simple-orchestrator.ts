...
function startWebUI(host: string, port: number) {
  ...
  const consoleHTML = `
    ...
    <body>
        <div class="header">
            <div class="title">🧠 Hive-Flow Console</div>
        ...
        <div class="console-container">
            ...
            <input type="text" class="console-input" id="input" placeholder="Enter hive-flow command..." autocomplete="off">
        </div>
        ...
        <script>
            ...
            function connect() {
                ...
                try {
                    ...
                    ws.onopen = () => {
                        ...
                        appendOutput('\n<span class="success">🔗 Connected to Hive-Flow Console</span>\n');
                        appendOutput('<span class="info">Type "help" for available commands or use any hive-flow command</span>\n\n');
                    };
            ...
            function handleMessage(data) {
                switch (data.type) {
                    ...
                    case 'command_complete':
                        appendOutput('\n<span class="prompt">hive-flow> </span>');
                        break;
                    case 'status':
            ...
            function sendCommand(command) {
                if (ws && ws.readyState === WebSocket.OPEN) {
                    appendOutput('<span class="prompt">hive-flow> </span>' + command + '\n');
...
<span class="warning">Note:</span> This is a web console interface for hive-flow CLI commands.
`;
      sendResponse(ws, {
        type: 'output',
        data: helpText,
...
`;
    ...
    // Determine the correct hive-flow executable path
    const rootDir = path.resolve(__dirname, '../..');
    const cliPath = path.join(rootDir, 'bin', 'hive-flow');

    // Spawn the command
    const child = spawn('node', [path.join(rootDir, 'src/cli/simple-cli.js'), ...cmdArgs], {
      stdio: ['pipe', 'pipe', 'pipe'],
      env: { ...process.env, HIVE_FLOW_WEB_MODE: 'true' },
    });
  ...
  return new Promise((resolve, reject) => {
    server.on('error', (err: any) => {
      if (err.code === 'EADDRINUSE') {
        ...
        console.log(`💡 Try a different port: hive-flow start --ui --port ${port + 1}`);
...
export async function startOrchestrator(options: any) {
  ...
  console.log('\n💡 Use "hive-flow status" to check system status');
  console.log('💡 Use "hive-flow stop" to stop the orchestrator');

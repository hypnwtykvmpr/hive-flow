...
async function swarmInitCommand(subArgs, flags) {
  ...
  console.log(`\n📋 NEXT STEPS:`);
  console.log(
    `  1. Spawn agents: hive-flow coordination agent-spawn --type <type> --swarm-id ${swarmId}`,
  );
  console.log(
    `  2. Orchestrate tasks: hive-flow coordination task-orchestrate --task "<description>" --swarm-id ${swarmId}`,
  );
  console.log(`  3. Monitor swarm: hive-flow monitoring swarm-monitor --swarm-id ${swarmId}`);
}

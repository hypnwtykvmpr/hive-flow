...
- NEVER create files in node_modules/ or any hive-flow directories
...
\`\`\`bash
# Store your work
npx hive-flow memory store ${memoryNamespace}_<phase> "description of work completed"

# Query previous work
npx hive-flow memory query ${memoryNamespace}

# Examples for this task
npx hive-flow memory store ${memoryNamespace}_analysis "Analyzed ${taskDescription} - found X components needed"
npx hive-flow memory store ${memoryNamespace}_progress "Completed Y% of implementation"
npx hive-flow memory store ${memoryNamespace}_blockers "Issue with Z - need clarification"
\`\`\`
...
batchtool run --parallel \\
  "npx hive-flow sparc run architect 'design user authentication system' --non-interactive" \\
  "npx hive-flow sparc run security-review 'analyze authentication requirements' --non-interactive" \\
  "npx hive-flow sparc run spec-pseudocode 'create auth flow pseudocode' --non-interactive"

# Boomerang Pattern: Research → Design → Implement → Test → Refine
batchtool orchestrate --boomerang \\
  --phase1 "npx hive-flow sparc run ask 'research best auth practices' --non-interactive" \\
  --phase2 "npx hive-flow sparc run architect 'design based on research' --non-interactive" \\
  --phase3 "npx hive-flow sparc run code 'implement auth system' --non-interactive" \\
  --phase4 "npx hive-flow sparc run tdd 'test auth implementation' --non-interactive" \\
  --phase5 "npx hive-flow sparc run optimization 'refine auth performance' --non-interactive"

# Concurrent Feature Development
batchtool run --concurrent --max-parallel 3 \\
  "npx hive-flow sparc run code 'implement login feature' --non-interactive" \\
  "npx hive-flow sparc run code 'implement registration feature' --non-interactive" \\
  "npx hive-flow sparc run code 'implement password reset' --non-interactive" \\
  "npx hive-flow sparc run tdd 'create auth test suite' --non-interactive"
\`\`\`
...
1. **Focus on User's Project**
   - Build what they asked for, not improvements to hive-flow
...
Remember: You're building the user's project, using hive-flow only for memory and orchestration support.`;
}

...
export class ConfigValidator {
  ...
  async validateClaudeMd() {
    ...
    try {
      ...
      const importantCommands = ['npx hive-flow sparc', 'npm run build', 'npm run test'];
  ...
  async validateExecutable() {
    ...
    try {
      ...
      if (!stat.isFile) {
        result.success = false;
        result.errors.push('hive-flow executable is not a file');
        return result;
      }
      ...
      if (Deno.build.os !== 'windows') {
        ...
        if (!isExecutable) {
          result.warnings.push('hive-flow file is not executable');
        }
      }
      ...
      if (content.includes('#!/usr/bin/env')) {
        // Script file
        if (!content.includes('hive-flow') && !content.includes('deno run')) {

...
```bash
npx hive-flow hook pre-search [options]
```
...
```bash
npx hive-flow hook pre-search --query "authentication implementation"
```

### With caching

```bash
npx hive-flow hook pre-search -q "React hooks usage" --cache-results
```

### Memory check first

```bash
npx hive-flow hook pre-search -q "previous bug fixes" --check-memory
```

### Query expansion

```bash
npx hive-flow hook pre-search -q "auth" --expand-query --suggest-filters
```
...
```bash
# Before searching
npx hive-flow hook pre-search --query "your search" --cache-results --check-memory
```

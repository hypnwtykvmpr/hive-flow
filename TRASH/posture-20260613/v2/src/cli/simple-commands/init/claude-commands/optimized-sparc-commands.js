...
1. **Run directly with parallel processing**: \`./hive-flow sparc run ${mode.slug} "your task" --parallel\`
2. **Batch operation mode**: \`./hive-flow sparc batch ${mode.slug} "tasks-file.json" --concurrent\`
3. **Pipeline processing**: \`./hive-flow sparc pipeline ${mode.slug} "your task" --stages\`
...
./hive-flow sparc run ${mode.slug} "${getOptimizedExampleTask(mode.slug)}"

# Use with memory namespace and parallel processing
./hive-flow sparc run ${mode.slug} "your task" --namespace ${mode.slug} --parallel

# Non-interactive mode with batchtools optimization
./hive-flow sparc run ${mode.slug} "your task" --non-interactive --batch-optimize
\`\`\`

### Batchtools Operations
\`\`\`bash
# Parallel execution with multiple related tasks
./hive-flow sparc parallel ${mode.slug} "task1,task2,task3" --concurrent

# Batch processing from configuration file
./hive-flow sparc batch ${mode.slug} tasks-config.json --optimize

# Pipeline execution with staged processing
./hive-flow sparc pipeline ${mode.slug} "complex-task" --stages parallel,validate,optimize
\`\`\`

### Performance Optimization
\`\`\`bash
# Monitor performance during execution
./hive-flow sparc run ${mode.slug} "your task" --monitor --performance

# Use concurrent processing with resource limits
./hive-flow sparc concurrent ${mode.slug} "your task" --max-parallel 5 --resource-limit 80%

# Batch execution with smart optimization
./hive-flow sparc smart-batch ${mode.slug} "your task" --auto-optimize --adaptive
\`\`\`
...
./hive-flow memory store "${mode.slug}_context" "important decisions" --namespace ${mode.slug}

# Query previous work
./hive-flow memory query "${mode.slug}" --limit 5
\`\`\`

### Batchtools Memory Operations
\`\`\`bash
# Batch store multiple related contexts
./hive-flow memory batch-store "${mode.slug}_contexts.json" --namespace ${mode.slug} --parallel

# Concurrent query across multiple namespaces
./hive-flow memory parallel-query "${mode.slug}" --namespaces ${mode.slug},project,arch --concurrent

# Export mode-specific memory with compression
./hive-flow memory export "${mode.slug}_backup.json" --namespace ${mode.slug} --compress --parallel
\`\`\`
...
./hive-flow sparc concurrent ${mode.slug},architect,security-review "your project" --parallel

# Pipeline execution across multiple modes
./hive-flow sparc pipeline ${mode.slug}->code->tdd "feature implementation" --optimize
\`\`\`

### Batch Workflow Integration
\`\`\`bash
# Execute complete workflow with batchtools optimization
./hive-flow sparc workflow ${mode.slug}-workflow.json --batch-optimize --monitor
\`\`\`
...
export function createOptimizedMainSparcCommand(modes) {
  ...
  return `---
name: sparc
description: Execute SPARC methodology workflows with Hive-Flow and batchtools optimization
---
...
\`\`\`bash
./hive-flow sparc "build complete authentication system" --parallel --optimize
\`\`\`

### Run multiple modes concurrently:
\`\`\`bash
./hive-flow sparc concurrent architect,code,tdd "your project" --parallel
\`\`\`

### Execute batch operations:
\`\`\`bash
./hive-flow sparc batch "multiple-tasks.json" --optimize --monitor
\`\`\`

### Pipeline execution with staged processing:
\`\`\`bash
./hive-flow sparc pipeline "complex-project" --stages spec,architect,code,tdd,integration
\`\`\`
...
./hive-flow memory parallel-query "authentication" --namespaces spec,arch,impl --concurrent

# Export project memory with compression
./hive-flow memory export sparc-project-backup.json --compress --parallel
\`\`\`
...
./hive-flow swarm "Build e-commerce platform" --strategy development --monitor --review --parallel

# Background optimization swarm with concurrent processing
./hive-flow swarm "Optimize system performance" --strategy optimization --background --concurrent

# Distributed research swarm with batch analysis
./hive-flow swarm "Analyze market trends" --strategy research --distributed --ui --batch-analyze
\`\`\`
...
\`\`\`bash
./hive-flow sparc run code "implement API" --non-interactive --parallel
./hive-flow sparc batch tdd "user tests" --non-interactive --enable-permissions --concurrent
./hive-flow sparc pipeline "full-stack-app" --non-interactive --optimize --stages parallel
\`\`\`
...
./hive-flow sparc monitor --real-time --performance --all-phases

# Analyze batch operation efficiency
./hive-flow sparc analyze --batchtools --optimization --detailed

# Performance comparison across modes
./hive-flow sparc compare --modes architect,code,tdd --performance
\`\`\`

### Optimization Commands
\`\`\`bash
# Optimize SPARC configuration for your system
./hive-flow sparc optimize --auto-tune --system-profile

# Performance benchmarking
./hive-flow sparc benchmark --all-modes --detailed --export-results
\`\`\`
...
./hive-flow sparc debug --resources --concurrent --verbose

# Analyze batch operation performance
./hive-flow sparc analyze --performance --bottlenecks --optimization

# Monitor parallel processing efficiency
./hive-flow sparc monitor --parallel --efficiency --real-time
\`\`\`
...
See \`/hive-flow-help\` for all available commands and \`/batchtools\` for detailed parallel processing documentation.

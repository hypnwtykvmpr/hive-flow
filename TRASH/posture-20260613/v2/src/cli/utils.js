...
export async function loadConfig(path = 'hive-flow.config.json') {
  const defaultConfig = {
    terminal: {
      poolSize: 10,
      recycleAfter: 20,
...
export async function saveConfig(config, path = 'hive-flow.config.json') {
...
// Hive Flow MCP integration helpers
  ...
  try {
    ...
    const initMessage = {
      ...
      params: {
        ...
        clientInfo: { name: 'hive-flow-cli', version: '2.0.0' },
      },
    };

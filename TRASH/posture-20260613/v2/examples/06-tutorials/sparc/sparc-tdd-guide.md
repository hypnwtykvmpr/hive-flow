...
Learn how to use SPARC methodology for systematic TDD with Hive Flow.
...
- Hive Flow with SPARC modes configured
...
```bash
cd examples
../hive-flow sparc run spec-pseudocode \
...
```bash
../hive-flow sparc run integration \
...
```bash
../hive-flow sparc run security-review \
...
../hive-flow memory store auth_spec "$(cat ./output/auth-spec.md)"

# Retrieve for future work
../hive-flow memory query auth_spec > ./restored-spec.md
```
...
### Authentication Flow
```bash
../hive-flow sparc tdd "implement login flow" \
  --pattern "request-validate-authenticate-respond"
```

### CRUD Operations
```bash
../hive-flow sparc tdd "implement user CRUD" \
  --pattern "rest-api" \
  --with-validation
```
...
```bash
../hive-flow sparc tdd "implement event handlers" \
  --pattern "pub-sub" \
  --async
```

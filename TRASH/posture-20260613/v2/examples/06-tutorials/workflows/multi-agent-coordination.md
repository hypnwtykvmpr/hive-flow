...
Multi-agent systems in Hive Flow allow you to:
...
```bash
../hive-flow swarm create "Build complete e-commerce platform with user management, product catalog, and frontend" --agents 5 --strategy development --output ./output/ecommerce --monitor
```

### Step 4: Understanding Coordination Modes

Hive Flow supports different coordination patterns:

#### Hub-Spoke Pattern
Central coordinator manages all agents:

```bash
../hive-flow orchestrate ./workflow.json \
...
```bash
../hive-flow orchestrate ./workflow.json --monitor
```

### Debug Mode
```bash
../hive-flow orchestrate ./workflow.json --debug
```

### Performance Metrics
```bash
../hive-flow orchestrate ./workflow.json --metrics
```

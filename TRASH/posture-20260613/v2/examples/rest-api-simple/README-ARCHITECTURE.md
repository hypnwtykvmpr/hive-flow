...
```bash
# Phase 1: Setup
npx hive-flow sparc run code "Setup Express server with health endpoint" --non-interactive

# Phase 2: CRUD with TDD
npx hive-flow sparc run tdd "Implement CRUD operations for tasks" --non-interactive

# Phase 3: Validation
npx hive-flow sparc run code "Add input validation and error handling" --non-interactive

# Phase 4: Documentation
npx hive-flow sparc run docs-writer "Create REST API documentation" --non-interactive

# Phase 5: Security Review
npx hive-flow sparc run security-review "Review API security" --non-interactive
```

### Option 2: Parallel Swarm Execution (~15 minutes)
```bash
# Complete implementation with parallel agents
npx hive-flow swarm "Build complete simple REST API from architecture docs" \

...
/**
 * Migration script to update Hive Flow settings.json to new hooks format
  ...
  try {
    ...
    if (settings.hooks?.preCommandHook) {
      newHooks.PreToolUse.push({
        ...
        hooks: [{
          type: "command",
          command: `npx hive-flow@alpha hooks pre-command --command "\${command}" --validate-safety true --prepare-resources true`
        }]
      });
    }
    ...
    if (settings.hooks?.preEditHook) {
      newHooks.PreToolUse.push({
        ...
        hooks: [{
          type: "command",
          command: `npx hive-flow@alpha hooks pre-edit --file "\${file}" --auto-assign-agents true --load-context true`
        }]
      });
    }
    ...
    if (settings.hooks?.postCommandHook) {
      newHooks.PostToolUse.push({
        ...
        hooks: [{
          type: "command",
          command: `npx hive-flow@alpha hooks post-command --command "\${command}" --track-metrics true --store-results true`
        }]
      });
    }
    ...
    if (settings.hooks?.postEditHook) {
      newHooks.PostToolUse.push({
        ...
        hooks: [{
          type: "command",
          command: `npx hive-flow@alpha hooks post-edit --file "\${file}" --format true --update-memory true --train-neural true`
        }]
      });
    }
    ...
    if (settings.hooks?.sessionEndHook) {
      newHooks.Stop.push({
        hooks: [{
          type: "command",
          command: `npx hive-flow@alpha hooks session-end --generate-summary true --persist-state true --export-metrics true`
        }]
      });
    }
...
async function main() {
  console.log('🔄 Hive Flow Hooks Migration Script\n');
  ...
  console.log('2. Run "claude mcp add hive-flow npx hive-flow@alpha mcp start" to add MCP server');

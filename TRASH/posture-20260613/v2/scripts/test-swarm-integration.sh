#!/bin/bash
# Integration test for Hive-Flow Swarm Mode

echo "Hive-Flow Swarm Mode Integration Test"
echo "======================================="
echo
...
if [ -f "./bin/hive-flow" ]; then
    echo "✅ hive-flow binary found"
else
    echo "❌ hive-flow binary not found"
    exit 1
fi

if [ -f "./bin/hive-flow-swarm" ]; then
    echo "✅ hive-flow-swarm wrapper found"
else
    echo "❌ hive-flow-swarm wrapper not found"
    exit 1
fi

# Test 2: Help command
echo -e "\nTest 2: Testing help command..."
if ./bin/hive-flow help swarm | grep -q "Claude Swarm Mode"; then
...
if ./bin/hive-flow --help | grep -q "swarm"; then
...
if ./bin/hive-flow-swarm "Test objective" --dry-run | grep -q "DRY RUN"; then
...
OUTPUT=$(./bin/hive-flow-swarm "Complex test" --strategy research --max-agents 10 --coordinator --review --parallel --dry-run)
...
echo "  1. ./bin/hive-flow swarm \"Your objective\" [options]"
echo "  2. ./bin/hive-flow-swarm \"Your objective\" [options]"
echo "  3. npx hive-flow swarm \"Your objective\" [options]"
echo
echo "For more info: ./bin/hive-flow help swarm"
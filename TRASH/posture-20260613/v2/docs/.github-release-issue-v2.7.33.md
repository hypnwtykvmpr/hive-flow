...
This major release brings **three transformative feature sets** to Hive Flow:
...
### CLI Status ✅
```bash
✅ npx hive-flow --version → v2.7.33
✅ npx hive-flow mcp status → Operational
✅ npx hive-flow memory stats → ReasoningBank data
...
npx hive-flow mcp call tools/search '{"query": "memory", "detailLevel": "basic"}'

# Tools load automatically on first invocation
npx hive-flow mcp call memory_store '{"key": "test", "value": "data"}'
```
...
```bash
cp -r ~/.hive-flow ~/.hive-flow.backup
```
...
echo '{"mcp":{"features":{"enableMCP2025":true}}}' > ~/.hive-flow/config.json
```
...
npm install -g hive-flow@2.7.32

# Restore configuration
rm -rf ~/.hive-flow
mv ~/.hive-flow.backup ~/.hive-flow
```
...
### Documentation
- Main docs: https://github.com/ruvnet/hive-flow

...
**Versions:** hive-flow@2.7.25, agentic-flow@1.8.9
...
**Affected Versions:** hive-flow@2.7.24, agentic-flow@1.8.7
...
[Embeddings] For semantic search, install globally: npm install -g hive-flow
✅ Stored successfully
```
...
**After fix (v2.7.25):**
```bash
npx hive-flow@alpha memory store "test" "Clean experience!"
...
npx hive-flow@alpha memory query "search"
...
```bash
# Store data
npx hive-flow@alpha memory store "key1" "value1"
npx hive-flow@alpha memory store "key2" "value2"
...
if (isNpx && !process.env.FORCE_TRANSFORMERS) {
  ...
  console.log('[Embeddings] For semantic search, install globally: npm install -g hive-flow');

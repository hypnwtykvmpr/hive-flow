...
```bash
npx hive-flow@alpha memory query "How to approach a complex system outage with intermittent failures?" \
...
```bash
npx hive-flow@alpha memory query "Explore multiple approaches to solve database scaling bottleneck" \
  --namespace problem-solving \
  --reasoningbank \
  --k 5
```
...
```bash
npx hive-flow@alpha memory query "Traditional deployment process too slow, need breakthrough solution" \
  --namespace problem-solving \
  --reasoningbank \
  --k 5
```
...
```bash
npx hive-flow@alpha memory query "Why does increasing marketing spend reduce lead quality?" \
  --namespace problem-solving \
  --reasoningbank \
  --k 5
```
...
```bash
npx hive-flow@alpha memory query "Validate assumption that users want more features" \
  --namespace problem-solving \
  --reasoningbank \
  --k 5
```
...
npx hive-flow@alpha memory query "debug production issue" \
  --reasoningbank \
  --filter "cognitive_type:convergent"

# Only divergent (creative) patterns
npx hive-flow@alpha memory query "improve user engagement" \
  --reasoningbank \
...
```bash
# Technical domain
npx hive-flow@alpha memory query "scale microservices" \
  --reasoningbank \
  --filter "domain:technical"

# Business domain
npx hive-flow@alpha memory query "reduce customer churn" \
  --reasoningbank \
...
npx hive-flow@alpha memory query "deploy safely" \
  --reasoningbank \
...
npx hive-flow@alpha memory query "solve scaling problem" \
  --reasoningbank \
  --links "alternative" \
  --k 10

# Find prerequisite patterns
npx hive-flow@alpha memory query "implement microservices" \
  --reasoningbank \
  --links "requires" \
  --k 5
```
...
npx hive-flow@alpha memory query "complex system debugging" \
  --reasoningbank \
  --trajectory \
  --steps 5
```
...
Part of the hive-flow ReasoningBank system. Pre-trained model available for use with hive-flow and agentic-flow agents.

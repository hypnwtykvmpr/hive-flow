...
npx hive-flow@alpha memory store sql_injection \
  "Use parameterized queries to prevent SQL injection attacks" \
  --namespace security --reasoningbank

npx hive-flow@alpha memory store xss_prevention \
  "Sanitize user input and escape HTML to prevent XSS" \
  --namespace security --reasoningbank

npx hive-flow@alpha memory store csrf_protection \
  "Use CSRF tokens for state-changing operations" \
  --namespace security --reasoningbank

# Query with broad term
npx hive-flow@alpha memory query "prevent attacks" \
  --namespace security --reasoningbank
```
...
npx hive-flow@alpha memory status --detailed --reasoningbank
```
...
npx hive-flow@alpha memory info jwt_authentication --reasoningbank
```
...
npx hive-flow@alpha memory store jwt_auth \
  "JWT tokens with HMAC SHA256 signing" \
  --namespace backend --reasoningbank

npx hive-flow@alpha memory store token_storage \
  "Store tokens in httpOnly cookies, not localStorage" \
  --namespace frontend --reasoningbank

npx hive-flow@alpha memory store token_refresh \
  "Refresh tokens before expiration using interceptors" \
  --namespace frontend --reasoningbank

# Query finds related patterns across namespaces!
npx hive-flow@alpha memory query "token management" --reasoningbank
```
...
npx hive-flow@alpha memory link \
...
```bash
# Store patterns
npx hive-flow@alpha memory store postgres_db \
  "PostgreSQL with connection pooling" \
  --namespace database --reasoningbank

npx hive-flow@alpha memory store db_indexing \
  "B-tree indexes on foreign keys and WHERE clause columns" \
  --namespace database --reasoningbank

npx hive-flow@alpha memory store query_optimization \
  "Use EXPLAIN ANALYZE to identify slow queries" \
  --namespace database --reasoningbank

# Query multiple times - SAFLA learns relationships
npx hive-flow@alpha memory query "database performance" --reasoningbank
npx hive-flow@alpha memory query "slow queries" --reasoningbank
npx hive-flow@alpha memory query "PostgreSQL optimization" --reasoningbank
...
npx hive-flow@alpha memory links jwt_authentication --type requires
```
...
npx hive-flow@alpha memory query "optimization" \
...
npx hive-flow@alpha memory query "authentication approaches" \
...
npx hive-flow@alpha memory query "JWT security concerns" \
...
npx hive-flow@alpha memory query "production JWT implementation" \
  --cognitive-pattern convergent --reasoningbank
# Returns: JWT with refresh rotation (best practice)

# Phase 4: Systems (understand full picture)
npx hive-flow@alpha memory query "JWT integration points" \
...
npx hive-flow@alpha memory trajectory start build_api \
  "Building RESTful API with authentication" \
  --reasoningbank

# Record each step
npx hive-flow@alpha memory trajectory step build_api \
...
# Complete trajectory
npx hive-flow@alpha memory trajectory end build_api \
...
```bash
# Get full trajectory
npx hive-flow@alpha memory trajectory get build_api --reasoningbank
```
...
npx hive-flow@alpha memory query "build authentication API" \
...
npx hive-flow@alpha memory store async_pattern \
...
npx hive-flow@alpha memory feedback async_pattern \
...
npx hive-flow@alpha memory query "error handling" \
...
npx hive-flow@alpha memory query "database optimization" \
  --min-confidence 0.6 \
  --max-age 90 \
  --namespace backend \
  --reasoningbank
...
npx hive-flow@alpha memory query "performance" \
  --all-namespaces \
  --group-by namespace \
  --reasoningbank
```
...
npx hive-flow@alpha memory query "security" \
...
npx hive-flow@alpha memory store code_review_001 \
...
npx hive-flow@alpha memory store code_review_002 \
...
npx hive-flow@alpha memory query "security vulnerabilities" \
  --namespace code_review --reasoningbank
...
for file in *.md; do
  npx hive-flow@alpha memory store "$file" "$(cat $file)" --reasoningbank
done

# ✅ Fast: Batch import (future feature)
npx hive-flow@alpha memory import patterns.json --reasoningbank
```
...
npx hive-flow@alpha memory optimize --partition-namespaces --reasoningbank
```
...
npx hive-flow@alpha memory cache-embeddings \
...
npx hive-flow@alpha memory config \

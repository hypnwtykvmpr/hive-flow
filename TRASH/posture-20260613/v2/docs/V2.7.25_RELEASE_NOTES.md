# hive-flow v2.7.25 Release Notes
...
```bash
npx hive-flow@alpha memory store "test" "Clean npx experience!"
...
```bash
npm install -g hive-flow@alpha
hive-flow memory store "api-design" "REST with JWT auth"
# ✅ Transformer model loads successfully

hive-flow memory query "authentication patterns"
...
**For Users:**
1. Try npx for quick testing: `npx hive-flow@alpha memory store "test" "value"`
2. Install globally for semantic search: `npm install -g hive-flow@alpha`
...
- **Package:** https://www.npmjs.com/package/hive-flow/v/2.7.25

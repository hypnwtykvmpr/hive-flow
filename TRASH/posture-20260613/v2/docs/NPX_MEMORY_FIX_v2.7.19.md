...
**Issue:** `npx hive-flow@alpha memory` commands fail with BetterSqlite3 constructor error
...
When users run memory commands via `npx hive-flow@alpha memory store ...`, they encounter:
...
initPromise = (async () => {
  try {
    ...
    if (isSqliteError && isNpx) {
      ...
      console.error('  1. LOCAL INSTALL: npm install && node_modules/.bin/hive-flow\n');
      console.error('  2. USE MCP TOOLS: mcp__hive-flow__memory_usage(...)\n');
...
```bash
$ npm install
$ node_modules/.bin/hive-flow memory store "api-design" "REST with JWT auth"
...
```bash
$ npx hive-flow@alpha memory store "test-key" "test-value"
⚠️  NPX LIMITATION DETECTED
✅ Automatically using JSON fallback for this command
✅ Stored successfully
✅ PASS

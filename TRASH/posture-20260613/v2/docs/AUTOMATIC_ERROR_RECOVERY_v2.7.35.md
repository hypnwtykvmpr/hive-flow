...
Implemented comprehensive automatic error recovery system for `hive-flow init` that handles the WSL better-sqlite3 ENOTEMPTY error **without manual intervention**.
...
$ rm -rf ~/.npm/_npx
$ npx hive-flow@alpha init --force  # Try again
```
...
# Test command
CMD npx hive-flow@alpha init --force
```
...
```bash
# Build test image
docker build -t hive-flow-test -f Dockerfile.test .

# Run test
docker run -it hive-flow-test

# Test with volume mount
docker run -it -v $(pwd):/workspace -w /workspace hive-flow-test

# Simulate WSL environment
docker run -it -e SIMULATE_WSL=1 hive-flow-test
```

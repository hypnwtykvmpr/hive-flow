...
### Issue
After v2.7.26 release, users experienced `ENOTEMPTY` errors when running `npx hive-flow` commands:
...
#### 1. **Retry Logic with Exponential Backoff** (`bin/hive-flow`)
```bash
execute_with_retry() {
...
❌ Failed after 3 attempts. Please try:
   ...
   2. Use global installation: npm install -g hive-flow
   3. Report issue: https://github.com/ruvnet/hive-flow/issues/856
```
...
```bash
# Build test image
docker build -f tests/docker/Dockerfile.npx-test -t hive-flow-npx-test .

# Run concurrent execution tests
docker run --rm hive-flow-npx-test /test/test-concurrent.sh

# Run cache cleanup tests
docker run --rm hive-flow-npx-test /test/test-cache-cleanup.sh
```
...
npx hive-flow@latest <command>

# Or use alpha tag
npx hive-flow@alpha <command>
```
...
- **GitHub Issue**: [#856](https://github.com/ruvnet/hive-flow/issues/856)
...
For CI/CD pipelines using `npx hive-flow`:
...
**Full Changelog**: [v2.7.26...v2.7.27](https://github.com/ruvnet/hive-flow/compare/v2.7.26...v2.7.27)

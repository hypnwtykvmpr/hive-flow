...
### `hive-flow agent booster edit <file> "<instruction>"`
...
hive-flow agent booster edit src/app.js "Add try-catch error handling"

# Refactor to async/await
hive-flow agent booster edit server.ts "Convert callbacks to async/await"

# Add JSDoc comments
hive-flow agent booster edit utils.js "Add comprehensive JSDoc comments"

# Preview changes without applying (dry run)
hive-flow agent booster edit app.js "Add logging" --dry-run

# Show performance comparison
hive-flow agent booster edit app.js "Add logging" --benchmark
```
...
### `hive-flow agent booster batch <pattern> "<instruction>"`
...
hive-flow agent booster batch "src/**/*.ts" "Convert to arrow functions"

# Add logging to all JavaScript files
hive-flow agent booster batch "*.js" "Add console.log for debugging"

# Update imports across project
hive-flow agent booster batch "components/**/*.jsx" "Update React imports for v19"

# Preview batch changes (dry run)
hive-flow agent booster batch "src/*.js" "Add comments" --dry-run
```
...
### `hive-flow agent booster parse-markdown <file>`
...
hive-flow agent booster parse-markdown refactoring-plan.md

# Preview changes without applying
hive-flow agent booster parse-markdown plan.md --dry-run
```
...
### `hive-flow agent booster benchmark [options]`
...
hive-flow agent booster benchmark

# Custom iteration count
hive-flow agent booster benchmark --iterations 50

# Benchmark specific file
hive-flow agent booster benchmark --file src/app.js --iterations 100
```
...
hive-flow agent booster batch "src/**/*.js" "Convert to ES6 modules"
...
hive-flow agent booster edit current-file.ts "Add type annotations"
...
hive-flow agent booster batch "**/*.js" "Apply ESLint fixes"
...
hive-flow agent booster batch "src/**/*.js" "Convert to TypeScript"
...
hive-flow agent booster batch "src/**/*.jsx" "Update React 18 → React 19 APIs"
```
...
### Validate 352x Claim
```bash
hive-flow agent booster benchmark --iterations 100
```

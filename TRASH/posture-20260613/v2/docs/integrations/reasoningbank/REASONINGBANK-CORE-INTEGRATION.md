...
Integrate ReasoningBank as an **optional enhanced mode** for `hive-flow memory` while maintaining full backward compatibility with existing installations.

## 📊 Current State

### Two Separate Systems

**Core Memory** (`hive-flow memory`)
...
**ReasoningBank** (`hive-flow agent memory`)
...
hive-flow memory store api_key "sk-ant-xxx" --redact
hive-flow memory query research

# Enhanced mode (NEW - opt-in via flag)
hive-flow memory store api_key "sk-ant-xxx" --reasoningbank
hive-flow memory query research --reasoningbank
hive-flow memory status --reasoningbank

# Short form
hive-flow memory store api_key "sk-ant-xxx" --rb
hive-flow memory query research --rb
```
...
hive-flow memory query research --auto

# Check if ReasoningBank is available
hive-flow memory detect
```
...
hive-flow memory mode

# Detect ReasoningBank availability
hive-flow memory detect

# Migrate basic → ReasoningBank
hive-flow memory migrate --to reasoningbank

# Consolidate/optimize ReasoningBank
hive-flow memory consolidate --reasoningbank
```
...
hive-flow memory query api --reasoningbank
...
$ hive-flow memory store project "Started new API project"
✅ Stored: project

$ hive-flow memory query project
✅ Found 1 result:
...
hive-flow memory store api "sk-ant-xxx" --redact

# ReasoningBank with privacy-first local embeddings
hive-flow memory init --reasoningbank --local-embeddings
...
hive-flow memory store test "value"
hive-flow memory query test
...
# Test 3: Opt-in works
hive-flow memory query test --reasoningbank
...
# Test 5: Migration
hive-flow memory migrate --to reasoningbank
hive-flow memory stats --reasoningbank
...
```bash
$ hive-flow memory --help

Memory Management

USAGE:
  hive-flow memory <command> [options]
...
**Existing** (`mcp__hive-flow__memory_usage`)
```javascript
...
mcp__hive-flow__reasoningbank_memory({
  action: "store",
  key: "api_config",
  value: "some data",
...
mcp__hive-flow__reasoningbank_query({
...
```json
{
  "name": "mcp__hive-flow__memory_usage",
...
```json
{
  "name": "mcp__hive-flow__reasoningbank_store",
...
await mcp__hive-flow__memory_usage({
  action: "store",
...
await mcp__hive-flow__reasoningbank_query({
...
mcp__hive-flow__memory_usage({ action: "store", ... })
...
mcp__hive-flow__reasoningbank_store({
  key: "pattern",

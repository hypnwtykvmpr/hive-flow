# NPM Publish Guide - Hive Flow v2.7.33
...
- [x] bin/hive-flow executable packaged
...
# Visit: https://github.com/ruvnet/hive-flow/releases/tag/v2.7.33
```
...
# (Then promote to latest after testing: npm dist-tag add hive-flow@2.7.33 latest)
```
...
npm view hive-flow version
# Expected: 2.7.33

# Check dist-tags
npm view hive-flow dist-tags
...
# Install and test
npm install -g hive-flow@2.7.33
npx hive-flow --version
# Expected: v2.7.33
```
...
# 1. Go to https://github.com/ruvnet/hive-flow/releases/new
...
npx hive-flow mcp status
npx hive-flow memory stats
npx hive-flow hooks

# Test MCP 2025-11 features
npx hive-flow mcp start --mcp2025
# (In another terminal)
npx hive-flow mcp status
...
npm install -g hive-flow@beta
# ... test ...

# Promote to latest:
npm dist-tag add hive-flow@2.7.33 latest
...
npm deprecate hive-flow@2.7.33 "Critical issue found. Use v2.7.32 instead."
```

**Step 2: Revert Latest Tag**
```bash
# Point latest back to previous version
npm dist-tag add hive-flow@2.7.32 latest
```

**Step 3: Unpublish (Within 72 hours)**
```bash
# Only if ABSOLUTELY necessary and within 72 hours
npm unpublish hive-flow@2.7.33
...
npm view hive-flow
# Monitor downloads at: https://npm-stat.com/charts.html?package=hive-flow
```
...
```markdown
Title: 🚀 Hive Flow v2.7.33 Released - MCP 2025-11 Compliance & Progressive Disclosure

We're excited to announce Hive Flow v2.7.33 with three major feature sets:
...
**GitHub Issues:**
- Create issue: https://github.com/ruvnet/hive-flow/issues/new

# 📦 Hive Flow Plugin Installation Guide
...
Look for `hive-flow` in the active plugins list.
...
```
/plugin update hive-flow
```

Or pull latest from GitHub:
```
cd /path/to/hive-flow
git pull
```
...
Look for `hive-flow` in the list with status "active".

...
### Issue
- **Command**: `npx hive-flow@alpha memory stats`
...
# Force basic mode
$ npx hive-flow@alpha memory stats --basic
...
3. **Test with data in JSON only**
   ```bash
   npx hive-flow@alpha memory store "test" "value" --basic
   npx hive-flow@alpha memory stats
...
4. **Test with data in both backends**
   ```bash
   npx hive-flow@alpha memory store "json-key" "json-value" --basic
   npx hive-flow@alpha memory store "rb-key" "rb-value" --reasoningbank
   npx hive-flow@alpha memory stats
...
5. **Test with flags**
   ...
   npx hive-flow@alpha memory stats --reasoningbank
   npx hive-flow@alpha memory stats --basic
   npx hive-flow@alpha memory stats --auto
...
### Reproduction Steps
1. Initialize ReasoningBank: `npx hive-flow@alpha memory init --reasoningbank`
2. Store data: `npx hive-flow@alpha memory store test-key "test value" --reasoningbank`
3. Verify storage: `npx hive-flow@alpha memory list --reasoningbank` (shows data)
4. Run stats: `npx hive-flow@alpha memory stats` (shows zeros ❌)

# Release Notes - Hive-Flow v2.7.15
...
### Memory Command Fix
**Issue:** `npx hive-flow memory status` failed with "Cannot find package 'onnxruntime-node'"
...
- Use `node_modules/.bin/hive-flow` instead of `npx hive-flow` for memory commands
- Or use MCP tools: `mcp__hive-flow__memory_usage()`
...
# Agent Booster
npx hive-flow agent booster benchmark
...
**Solution:** Use local binary: `node_modules/.bin/hive-flow memory stats`

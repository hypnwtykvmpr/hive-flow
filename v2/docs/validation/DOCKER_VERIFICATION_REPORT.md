# Docker Verification Report - hive-flow@2.7.1
...
The hive-flow@2.7.1 package has been successfully published to npm and verified in a clean Docker environment. All critical bug fixes are present and functional with no regressions detected.
...
docker build -t hive-flow-test:2.7.1 tests/docker/
docker run --rm hive-flow-test:2.7.1

# Run pattern verification
docker run --rm hive-flow-test:2.7.1 /test/verify-patterns.sh

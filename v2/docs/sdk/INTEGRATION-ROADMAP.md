...
```bash
npx hive-flow@alpha sparc run dev "task"    # Uses old implementation
npx hive-flow@alpha hooks pre-task "desc"   # Doesn't use checkpoints
npx hive-flow@alpha swarm init mesh         # Doesn't use session forking
```

### MCP Tools (NOT using SDK yet)
```javascript
mcp__hive-flow__swarm_init           // Doesn't create real forks
mcp__hive-flow__task_orchestrate     // Doesn't use pause/resume
mcp__hive-flow__agent_spawn          // Doesn't checkpoint
```
...
**Usage:**
```bash
npx hive-flow@alpha checkpoint create <session-id> "Before deployment"
npx hive-flow@alpha checkpoint list <session-id>
npx hive-flow@alpha checkpoint rollback <checkpoint-id>
```
...
export function createHiveFlowMcpServer() {
  return {
...
**Usage:**
```bash
npx hive-flow@alpha sparc run dev "Build API"
...
npx hive-flow@alpha swarm init mesh --enable-forking

# Fork swarm to try different approach
npx hive-flow@alpha swarm fork <swarm-id> "Try hierarchical"

# Commit or rollback fork
npx hive-flow@alpha swarm commit <fork-id>
npx hive-flow@alpha swarm rollback <fork-id>
```

### 2. SPARC with Checkpoints
```bash
# Run SPARC with auto-checkpointing
npx hive-flow@alpha sparc run dev "Build feature" --enable-checkpoints
...
```bash
# Start long task
npx hive-flow@alpha task run "Build entire app" --session-id my-task

# Pause if needed (saves state to disk)
npx hive-flow@alpha task pause my-task

# Resume hours/days later
npx hive-flow@alpha task resume my-task
```
...
const result = query({
  prompt: `
    Use mcp__hive-flow__swarm_init to create mesh swarm.
...
npx hive-flow@alpha swarm init mesh --enable-forking
npx hive-flow@alpha sparc run dev "task" --enable-checkpoints
```

### Step 2: Opt-Out (v2.5.0-alpha.150)
```bash
# Features enabled by default, opt-out with flags
npx hive-flow@alpha swarm init mesh --disable-forking
npx hive-flow@alpha sparc run dev "task" --disable-checkpoints
```

### Step 3: Always On (v2.5.0)
```bash
# Features always enabled, no flags needed
npx hive-flow@alpha swarm init mesh    # Forking enabled
npx hive-flow@alpha sparc run dev "task"  # Checkpoints enabled
```
...
```json
{
  "sdk": {
    ...
    "checkpoints": {
      ...
      "autoInterval": 10,
      "maxPerSession": 50,
      "persistPath": ".hive-flow/checkpoints"
    },
    "queryControl": {
      "enabled": true,
      "autoPauseOnError": true,
      "persistPath": ".hive-flow/paused-queries"
    },
    "inProcessMcp": {
      "enabled": true,
...
```bash
HIVE_FLOW_ENABLE_FORKING=true
HIVE_FLOW_ENABLE_CHECKPOINTS=true
HIVE_FLOW_CHECKPOINT_INTERVAL=10
HIVE_FLOW_ENABLE_PAUSE_RESUME=true
```

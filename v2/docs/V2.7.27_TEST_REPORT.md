...
**File**: `bin/hive-flow:76`
**Implementation**:
```bash
if grep -q "ENOTEMPTY" /tmp/hive-flow-error.log; then
...
**File**: `bin/hive-flow:85-88`
**Implementation**:
```bash
echo "❌ Failed after 3 attempts. Please try:"
echo "   1. Clear NPX cache: rm -rf ~/.npm/_npx"
echo "   2. Use global installation: npm install -g hive-flow"
echo "   3. Report issue: https://github.com/ruvnet/hive-flow/issues/856"
```
...
docker run --rm hive-flow-npx-test:v2.7.27 /test/test-concurrent.sh

# Run cache cleanup tests
docker run --rm hive-flow-npx-test:v2.7.27 /test/test-cache-cleanup.sh
```
...
rm -rf ~/.npm/_npx
npx hive-flow@latest --version

# Test 2: Rapid sequential execution
for i in {1..5}; do npx hive-flow@latest --version & done; wait

# Test 3: Verify error log cleanup
ls /tmp/hive-flow-error.log  # Should not exist after success
```

### For CI/CD Testing
```bash
# Test parallel execution in pipeline
parallel -j5 "npx hive-flow@latest --version" ::: {1..5}

# Test with cache simulation
docker run --rm -v ~/.npm:/root/.npm hive-flow-npx-test:v2.7.27
```

## 🔗 Related Documentation

- **Issue**: [#856 - NPM ENOTEMPTY error](https://github.com/ruvnet/hive-flow/issues/856)
...
- ✅ `bin/hive-flow` - Retry logic and cache cleanup

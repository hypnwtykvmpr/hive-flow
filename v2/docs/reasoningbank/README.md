...
npx hive-flow@alpha memory query "<search query>" --reasoningbank
...
npx hive-flow@alpha memory store memory_leak_fix \
...
npx hive-flow@alpha memory query "memory leak" --reasoningbank
...
npx hive-flow@alpha memory store bug_fix_001 \
...
npx hive-flow@alpha memory store bug_fix_002 \
...
npx hive-flow@alpha memory query "memory leak" \
  --namespace debugging --reasoningbank
```
...
**Full Details**: Read [Google Research Paper Analysis](./google-research.md) for implementation algorithms and integration with hive-flow.
...
**With ReasoningBank:**
```bash
AI queries: npx hive-flow@alpha memory query "CORS error fix"
...
Based on Google Research benchmarks and hive-flow production usage:
...
npx hive-flow@alpha memory store cache_redis \
...
npx hive-flow@alpha memory trajectory start api_build \
  "Building REST API" --reasoningbank

npx hive-flow@alpha memory trajectory step api_build \
  "Designed database schema" --reasoningbank

npx hive-flow@alpha memory trajectory step api_build \
  "Implemented endpoints" --reasoningbank

npx hive-flow@alpha memory trajectory end api_build \
  --outcome success --reasoningbank

# Later, retrieve the workflow
npx hive-flow@alpha memory trajectory get api_build --reasoningbank
```
...
```bash
# Backend pattern
npx hive-flow@alpha memory store jwt_backend \
  "JWT signing with HMAC SHA256" \
  --namespace backend --reasoningbank

# Frontend pattern
npx hive-flow@alpha memory store jwt_frontend \
  "Store JWT in httpOnly cookies" \
  --namespace frontend --reasoningbank

# Query finds both!
npx hive-flow@alpha memory query "JWT security" --reasoningbank
...
npx hive-flow@alpha memory store arch_microservices \
...
npx hive-flow@alpha memory query "why microservices" \
  --namespace team_decisions --reasoningbank
...
```bash
# Store bug fix
npx hive-flow@alpha memory store cors_fix \
  "CORS error: Add Access-Control-Allow-Origin in Express middleware" \
  --namespace debugging --reasoningbank

# Week later, similar error
npx hive-flow@alpha memory query "CORS blocked" --reasoningbank
...
npx hive-flow@alpha memory query "listing endpoints" \
  --namespace api_patterns --reasoningbank
```
...
npx hive-flow@alpha memory store hello \
  "ReasoningBank learns automatically!" \
  --reasoningbank

# 3. Query semantically (2ms)
npx hive-flow@alpha memory query "learning" --reasoningbank
...
- 💬 [GitHub Issues](https://github.com/ruvnet/hive-flow/issues) - Get help or contribute

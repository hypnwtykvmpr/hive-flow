...
### Basic Search
```bash
npx hive-flow@alpha memory search "YOUR_QUERY" --namespace safla
```

### By Domain (5 categories)
```bash
# Self-learning patterns (confidence evolution)
npx hive-flow@alpha memory search "API optimization" --domain self-learning

# Feedback loops (user/system feedback)
npx hive-flow@alpha memory search "feedback" --domain feedback-optimization

# Bayesian confidence (probability updates)
npx hive-flow@alpha memory search "uncertainty" --domain confidence-adjustment

# Success/failure learning (distillation)
npx hive-flow@alpha memory search "deployment success" --domain distillation

# Meta-learning (recursive improvement)
npx hive-flow@alpha memory search "meta-learning" --domain recursive-cycles
```

### By Confidence Level
```bash
# Expert patterns (0.9-0.95)
npx hive-flow@alpha memory retrieve "confidence:>0.90" --namespace safla

# High confidence (0.8-0.9)
npx hive-flow@alpha memory retrieve "confidence:>0.80" --namespace safla

# Learning patterns (0.5-0.7)
npx hive-flow@alpha memory retrieve "confidence:<0.70" --namespace safla
```

### By Technology
```bash
npx hive-flow@alpha memory search "Node.js optimization" --namespace safla
npx hive-flow@alpha memory search "PostgreSQL patterns" --namespace safla
npx hive-flow@alpha memory search "Kubernetes deployment" --namespace safla
npx hive-flow@alpha memory search "React performance" --namespace safla
```
...
| Task | Command |
|------|---------|
| **API Development** | `npx hive-flow@alpha memory search "API endpoint optimization" --namespace safla` |
| **Code Review** | `npx hive-flow@alpha memory search "code review feedback" --namespace safla` |
| **Debugging** | `npx hive-flow@alpha memory search "error recovery" --namespace safla` |
| **Performance** | `npx hive-flow@alpha memory search "performance optimization" --namespace safla` |
| **Architecture** | `npx hive-flow@alpha memory search "architecture refinement" --namespace safla` |
| **Testing** | `npx hive-flow@alpha memory search "test suite evolution" --namespace safla` |
| **Deployment** | `npx hive-flow@alpha memory search "deployment success" --namespace safla` |
| **CI/CD** | `npx hive-flow@alpha memory search "CI/CD pipeline optimization" --namespace safla` |
| **Monitoring** | `npx hive-flow@alpha memory search "monitoring feedback loop" --namespace safla` |
...
## 🔗 Integration with Hive Flow Hooks

```bash
# Before task (get relevant patterns)
npx hive-flow@alpha hooks pre-task --description "API optimization"
npx hive-flow@alpha memory search "API optimization" --namespace safla

# After editing (store pattern usage)
npx hive-flow@alpha hooks post-edit --file "src/api.ts" --memory-key "swarm/safla/api"

# After task (record success)
npx hive-flow@alpha hooks post-task --task-id "api-opt-123"
```
...
time npx hive-flow@alpha memory search "test" --namespace safla
...
npx hive-flow@alpha memory --help

# GitHub Issues
https://github.com/ruvnet/hive-flow/issues

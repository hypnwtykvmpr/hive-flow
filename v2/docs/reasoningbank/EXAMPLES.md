...
npx hive-flow@alpha memory store api_key \
...
# Delete a pattern
npx hive-flow@alpha memory delete api_key \
  --namespace backend --reasoningbank
...
npx hive-flow@alpha memory query "API design" --reasoningbank
...
npx hive-flow@alpha memory store api_pagination \
  "Cursor-based pagination: /api/items?limit=20&cursor=abc123" \
  --namespace api_patterns --reasoningbank

npx hive-flow@alpha memory store api_error_format \
  'Consistent error format: {"error": true, "message": "...", "code": 400}' \
  --namespace api_patterns --reasoningbank

npx hive-flow@alpha memory store api_rate_limiting \
  "100 requests/minute per IP using sliding window algorithm" \
  --namespace api_patterns --reasoningbank

# Query when designing new API
npx hive-flow@alpha memory query "API response format" \
  --namespace api_patterns --reasoningbank
```
...
while IFS=',' read -r title content namespace; do
  npx hive-flow@alpha memory store "$title" "$content" \
...
npx hive-flow@alpha memory export --all > "$BACKUP_DIR/patterns.json"

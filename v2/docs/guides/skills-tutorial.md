# 🎓 Hive Flow Skills: Complete Introduction Tutorial

## Overview

We’re shifting how Hive Flow evolves from here forward. This release marks the move from slash commands to a true skills-based system, our new foundation for intelligence, automation, and collaboration. Instead of memorizing /commands, you now just describe what you want. Claude reads the situation, identifies the right skills, and activates them automatically.
...
This is the direction forward: modular, intelligent, and adaptive. Skills turn Hive Flow from a set of commands into a living system that learns how you work and grows with you.

## 🔄 What Changed: From Slash Commands to Skills

I'm transitioning hive-flow from slash commands to Claude Code's native skills system. Here's why this matters and what I've learned.

### The Old Way: Slash Commands

Previously, hive-flow used slash commands stored in `.claude/commands/`:
```bash
# Old approach
...
5. **Standards-Based**: Anthropic designed this. It works across Claude.ai, Claude Code CLI, and any other Claude implementation. Commands were hive-flow specific.
...
# Do this:
npx hive-flow@alpha init --force  # Install skills
...
Skills are the future of hive-flow. They align with Anthropic's vision, work better with Claude Code, and solve real problems I had with commands. The migration takes effort, but it's worth it.
...
This release adds 4 new AgentDB skills that bring production-grade vector database capabilities to hive-flow:
...
Hive Flow includes 25 specialized skills that activate automatically based on your task description. Just describe what you want in natural language - no commands needed.
...
### What Does `npx hive-flow@alpha init --force` Do?

This command initializes your project with Hive Flow's complete skills system:
...
3. **Sets up configuration** - Creates `hive-flow.json` with default settings
...
**When to Use It:**
- Starting a new project with Hive Flow
...
```bash
# Full SPARC workflow
npx hive-flow sparc tdd "user authentication system"

# Individual phases
npx hive-flow sparc run spec-pseudocode "shopping cart feature"
npx hive-flow sparc run architect "payment processing"
...
**What It Does:**
Comprehensive performance analysis, bottleneck detection, and optimization recommendations for Hive Flow swarms.
...
1. **Install**: Run `npx hive-flow@alpha init --force`

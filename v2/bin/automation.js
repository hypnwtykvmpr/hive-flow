...
async function runWorkflowCommand(subArgs, flags) {
  ...
  if (!workflowFile) {
    ...
    console.log('\nExample:');
    console.log('  hive-flow automation run-workflow workflow.json --claude --non-interactive');
    return;
  }
...
EXAMPLES:
  ...
  hive-flow automation run-workflow my-workflow.json --claude --non-interactive

  # Run MLE-STAR ML engineering workflow (flagship command) - non-interactive by default
  hive-flow automation mle-star --dataset data/train.csv --target price --claude

  # MLE-STAR with custom configuration
  hive-flow automation mle-star --dataset sales.csv --target revenue --output models/sales/ --name "sales-prediction" --search-iterations 5
  
  # MLE-STAR with interactive mode (single Claude coordinator)
  hive-flow automation mle-star --dataset data.csv --target label --claude --interactive
  
  # MLE-STAR with stream-json chaining (agents pipe outputs to each other)
  hive-flow automation mle-star --dataset data.csv --target label --claude --output-format stream-json
  
  # MLE-STAR with minimal output for CI/CD pipelines
  hive-flow automation mle-star --dataset data.csv --target label --claude --quiet
  
  # Custom workflow with stream chaining enabled
  hive-flow automation run-workflow analysis-pipeline.json --claude --output-format stream-json
  
  # Disable chaining for independent task execution
  hive-flow automation mle-star --dataset data.csv --target label --claude --no-chaining
  
  # View stream chaining in action with verbose output
  hive-flow automation mle-star --dataset data.csv --target label --claude --verbose

...
export async function startCommand(subArgs, flags) {
  ...
  try {
    printSuccess('Starting Hive-Flow Orchestration System...');
    console.log();
    ...
    if (missingDirs.length > 0) {
      ...
      console.log('Run "hive-flow init" first to create the necessary structure');
      return;
    }
    ...
    console.log('     - Backend: JSON file (memory/hive-flow-data.json)');
    ...
    if (daemon) {
      ...
      await compat.safeCall(async () => {
        if (compat.runtime === 'deno') {
          await fs.writeFile('.hive-flow.pid', pid.toString(), 'utf8');
        } else {
          const fs = await import('fs/promises');
          await fs.writeFile('.hive-flow.pid', pid.toString(), 'utf8');
        }
      });
      console.log(`Process ID: ${pid} (saved to .hive-flow.pid)`);
    } else {
      // Interactive mode
      ...
      console.log('     - hive-flow agent spawn researcher');
      console.log('     - hive-flow task create "your task"');
      console.log('     - hive-flow sparc "build feature"');
      console.log('     - hive-flow monitor');
      console.log();
      console.log('   • View system status:');
      console.log('     - hive-flow status');
      console.log();
      console.log('   • Launch process management UI:');
      console.log('     - hive-flow start --ui');
      console.log();
...
function showStartHelp() {
  console.log('Start the Hive-Flow orchestration system');
  console.log();
  console.log('Usage: hive-flow start [options]');
  console.log();
  console.log('Options:');
  ...
  console.log('  hive-flow start                    # Start in interactive mode');
  console.log('  hive-flow start --daemon           # Start as background daemon');
  console.log('  hive-flow start --port 8080        # Use custom server port');
  console.log('  hive-flow start --ui               # Launch terminal-based UI');
  console.log('  hive-flow start --web              # Launch web-based UI');
  console.log('  hive-flow start --verbose          # Show detailed logs');
  console.log();
  ...
  console.log('    - Hive Flow swarm integration');
  console.log();
  ...
  console.log();
  console.log('Notes:');
  console.log('  - Requires "hive-flow init" to be run first');
  console.log('  - Interactive mode shows real-time system status');
  console.log('  - Daemon mode runs in background (check logs)');
  console.log('  - Use "hive-flow status" to check if running');
  console.log('  - Use Ctrl+C or "hive-flow stop" to shutdown');
}

...
npx hive-flow@alpha sparc run ${mode.slug} "${getExampleTask(mode.slug)}"

# For alpha features
npx hive-flow@alpha sparc run ${mode.slug} "${getExampleTask(mode.slug)}"

# With namespace
npx hive-flow@alpha sparc run ${mode.slug} "your task" --namespace ${mode.slug}

# Non-interactive mode
npx hive-flow@alpha sparc run ${mode.slug} "your task" --non-interactive
\`\`\`

### Option 3: Local Installation
\`\`\`bash
# If hive-flow@alpha is installed locally
./hive-flow@alpha sparc run ${mode.slug} "${getExampleTask(mode.slug)}"
\`\`\`
...
// Query previous work
mcp__hive-flow@alpha__memory_search {
  pattern: "${mode.slug}",
  namespace: "${mode.slug}",
  limit: 5
}
...
\`\`\`bash
# Store mode-specific context
npx hive-flow@alpha memory store "${mode.slug}_context" "important decisions" --namespace ${mode.slug}

# Query previous work
npx hive-flow@alpha memory query "${mode.slug}" --limit 5
\`\`\`
`;
}
...
export function createMainSparcCommand(modes) {
  ...
  return `---
name: sparc
description: Execute SPARC methodology workflows with Hive-Flow
---
...
\`\`\`bash
# Run SPARC orchestrator (default)
npx hive-flow@alpha sparc "build complete authentication system"

# Run a specific mode
npx hive-flow@alpha sparc run architect "design API structure"
npx hive-flow@alpha sparc run tdd "implement user service"

# Execute full TDD workflow
npx hive-flow@alpha sparc tdd "implement user authentication"
...
\`\`\`bash
# If hive-flow@alpha is installed locally
./hive-flow@alpha sparc "build complete authentication system"
./hive-flow@alpha sparc run architect "design API structure"
\`\`\`
...
\`\`\`bash
# Store specifications
npx hive-flow@alpha memory store "spec_auth" "OAuth2 + JWT requirements" --namespace spec

# Store architectural decisions
./hive-flow@alpha memory store "arch_api" "RESTful microservices design" --namespace arch

# Query previous work
./hive-flow@alpha memory query "authentication" --limit 10

# Export project memory
./hive-flow@alpha memory export sparc-project-backup.json
\`\`\`
...
./hive-flow@alpha swarm "Build e-commerce platform" --strategy development --monitor --review

# Background optimization swarm
./hive-flow@alpha swarm "Optimize system performance" --strategy optimization --background

# Distributed research swarm
./hive-flow@alpha swarm "Analyze market trends" --strategy research --distributed --ui
\`\`\`
...
\`\`\`bash
./hive-flow@alpha sparc run code "implement API" --non-interactive
./hive-flow@alpha sparc tdd "user tests" --non-interactive --enable-permissions
\`\`\`
...
See \`/hive-flow@alpha-help\` for all available commands.
`;
}

...
RUN echo "=== Testing Hive Flow alpha.51 SQLite Fallback ===" && \
    echo "Node version: $(node --version)" && \
    echo "Platform: $(uname -a)" && \
    echo "" && \
    echo "1. Testing npx init command:" && \
    npx -y hive-flow@alpha init --force --project-name test-fallback && \
    echo "" && \
    ...
    npx -y hive-flow@alpha memory store test-key "test-value" --namespace test && \
    npx -y hive-flow@alpha memory retrieve test-key --namespace test && \
    echo "" && \
    echo "4. Testing hive-mind commands:" && \
    npx -y hive-flow@alpha hive-mind init && \
    echo "" && \
    echo "=== Test Complete ==="

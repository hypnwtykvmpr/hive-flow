...
export class MCPIntegrator {
  ...
  private registerDefaultTools(): void {
    // Hive Flow MCP tools
    this.tools.set('hive-flow', {
      name: 'hive-flow',
      server: 'npx hive-flow@alpha mcp start',
      functions: [
        {
          name: 'swarm_init',
  ...
  async initializeSwarmCoordination(config: {
    ...
    // Try hive-flow first, then fallback to other tools
    const toolPriority = ['hive-flow', 'ruv-swarm', 'flow-nexus'];

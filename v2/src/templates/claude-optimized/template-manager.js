...
const commands = {
  install: () => execFileSync('node', ['install-template.js'], { stdio: 'inherit' }),
  validate: () => execFileSync('node', ['validate-template.js'], { stdio: 'inherit' }),
  deploy: (targetPath) => {
    if (!targetPath) {
  ...
  update: () => {
    ...
    execFileSync('node', ['install-template.js'], { stdio: 'inherit' });

    // Validate
    console.log('2. Validating installation...');
    execFileSync('node', ['validate-template.js'], { stdio: 'inherit' });
  ...
  test: () => {
    ...
    if (fs.existsSync('.claude/tests/test-harness.js')) {
      execFileSync('node', ['tests/test-harness.js'], { stdio: 'inherit', cwd: '.claude' });
    } else {

...
npx hive-flow swarm "your complex task" --strategy <type> [options]

# Batch swarm operations
npx hive-flow swarm batch --tasks "frontend:dev,backend:dev,database:architect" --parallel
```
...
npx hive-flow swarm multi-strategy --config '{
...
npx hive-flow swarm batch-dev --services '{
...
npx hive-flow swarm batch-research --topics '{
...
npx hive-flow swarm batch-test --suites '{
...
npx hive-flow swarm pipeline --config '{
  "stages": [
...
npx hive-flow swarm analyze-pipeline --stages '{
...
npx hive-flow swarm deploy-multi --regions '{
...
npx hive-flow swarm federated-train --nodes 20 --parallel --aggregate-method "secure"
```
...
npx hive-flow swarm monitor-batch --metrics '{
...
npx hive-flow swarm auto-manage --rules '{
...
npx hive-flow swarm distribute --algorithm "weighted" --factors '{
  "agent-capacity": 0.4,
  "task-complexity": 0.3,
  "priority": 0.3
}'
...
npx hive-flow swarm batch-execute --resilient '{
...
npx hive-flow swarm batch --cache '{
...
npx hive-flow swarm optimize-network --compression "gzip" --batching 100 --keepalive
```
...
npx hive-flow swarm full-stack --project "e-commerce" --parallel-components '{
...
npx hive-flow swarm data-pipeline --config '{
...
npx hive-flow swarm security-audit --parallel-scans '{

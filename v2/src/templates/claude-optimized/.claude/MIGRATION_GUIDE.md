...
npx hive-flow sparc run code "test batch operations" --non-interactive
```
...
npx hive-flow memory export pre-migration-backup.json
...
npx hive-flow sparc run architect "design user service"
# Wait...
npx hive-flow sparc run architect "design auth service"
# Wait...
npx hive-flow sparc run architect "design API gateway"
```
...
npx hive-flow sparc run architect "design complete microservices architecture with user, auth, and gateway services"
...
npx hive-flow sparc run code "create user controller"
npx hive-flow sparc run code "create user service"
npx hive-flow sparc run code "create user repository"
```
...
npx hive-flow sparc run code "implement complete user management with controller, service, repository, and tests"
...
```bash
# Old
npx hive-flow sparc run code "create user API"

# New with options
npx hive-flow sparc run code "create user API" --non-interactive --parallel --batch-size=10
```
...
if npx hive-flow sparc run code "test parallel" --non-interactive --dry-run 2>&1 | grep -q "parallel"; then
    echo "✅"
else
...
if npx hive-flow memory stats > /dev/null 2>&1; then
    echo "✅"
else
...
time npx hive-flow sparc run code "create test component" --non-interactive > /dev/null 2>&1
...
if [ -f "pre-migration-backup.json" ]; then
    npx hive-flow memory import pre-migration-backup.json
fi
...
npx hive-flow sparc run architect "test batch architecture" --non-interactive
npx hive-flow sparc run tdd "test parallel testing" --non-interactive
npx hive-flow sparc run code "test concurrent generation" --non-interactive
```
...
time npx hive-flow sparc run code "create user CRUD" --legacy

echo "Testing new method..."
time npx hive-flow sparc run code "create user CRUD" --non-interactive
```
...
```bash
# Test full workflow
npx hive-flow sparc tdd "implement complete feature with batchtools"
```
...
const tests = [
  {
    ...
    old: 'npx hive-flow sparc run code "create user CRUD" --legacy',
    new: 'npx hive-flow sparc run code "create user CRUD with all operations"',
  },
  {
    name: 'Test Suite Creation',
    old: 'npx hive-flow sparc run tdd "create auth tests" --legacy',
    new: 'npx hive-flow sparc run tdd "create complete auth test suite"',
  },
  {
    name: 'Architecture Design',
    old: 'npx hive-flow sparc run architect "design microservices" --legacy',
    new: 'npx hive-flow sparc run architect "design complete microservices architecture"',
  },
];
...
batchtool orchestrate --prototypes \
  --idea-1 "npx hive-flow sparc run code 'social feed prototype'" \
  --idea-2 "npx hive-flow sparc run code 'marketplace prototype'" \
  --idea-3 "npx hive-flow sparc run code 'subscription prototype'" \
  --test-all "npx hive-flow sparc run tdd 'test all prototypes'" \
  --compare "npx hive-flow sparc run architect 'analyze best approach'"
```
...
# Run with batchtools
npx hive-flow sparc run <mode> "<task>" --non-interactive

...
Comprehensive task management with orchestration features for Hive-Flow. Integrates seamlessly with TodoWrite/TodoRead for coordination and Memory for state persistence.
...
hive-flow task create development "Implement authentication" \
  --priority 80 \
...
hive-flow task status task-789 \
  --show-logs \
  --show-checkpoints \
  --show-metrics \
  --show-dependencies \
...
hive-flow task cancel task-789 \
...
```bash
# Create workflow
hive-flow task workflow create "E-commerce Platform" \
...
hive-flow task workflow execute workflow-123 \
  --variables '{"environment":"staging"}' \
  --monitor

# Visualize dependency graph
hive-flow task workflow visualize workflow-123 \
  --format dot \

/**
 * Hive Flow SPARC Executor
 * Executes tasks using the full hive-flow SPARC system in non-interactive mode
 */
...
import { getHiveFlowBin } from '../utils/paths.js';

export interface HiveFlowExecutorConfig {
  logger?: Logger;
  hiveFlowPath?: string;
  enableSparc?: boolean;
  verbose?: boolean;
...
export class HiveFlowExecutor {
  private logger: Logger;
  private hiveFlowPath: string;
  private enableSparc: boolean;
  private verbose: boolean;
  private timeoutMinutes: number;

  constructor(config: HiveFlowExecutorConfig = {}) {
    this.logger =
      config.logger ||
      new Logger(
        { level: 'info', format: 'text', destination: 'console' },
        { component: 'HiveFlowExecutor' },
      );
    this.hiveFlowPath = config.hiveFlowPath || getHiveFlowBin();
  ...
  async executeTask(
    ...
    this.logger.info('Executing task with Hive Flow SPARC', {
      taskId: task.id.id,
      taskName: task.name,
    ...
    try {
      ...
      this.logger.error('Failed to execute Hive Flow SPARC command', {
...
export default HiveFlowExecutor;

...
async function runContinuousMonitoring(interval, format) {
  ...
  console.clear();
  console.log(`🔄 Monitoring Hive-Flow System`);
...
async function getOrchestratorMetrics() {
  try {
    ...
    const metricsPath = path.join(process.cwd(), '.hive-flow', 'metrics.json');
...
async function checkOrchestratorRunning() {
  try {
    const pidPath = path.join(process.cwd(), '.hive-flow', 'orchestrator.pid');
...
async function getResourceMetrics() {
  try {
    ...
    const memoryDbPath = path.join(process.cwd(), '.hive-flow', 'memory.db');
...
async function countTerminalSessions() {
  try {
    const sessionsPath = path.join(process.cwd(), '.hive-flow', 'sessions');
...
async function countMCPConnections() {
  try {
    const mcpPath = path.join(process.cwd(), '.hive-flow', 'mcp-connections.json');
...
export function showMonitorHelp() {
  ...
  console.log('  hive-flow monitor              # Show current metrics');
  console.log('  hive-flow monitor --watch      # Continuous monitoring');
  console.log('  hive-flow monitor --interval 1000 --watch  # Fast updates');
  console.log('  hive-flow monitor --format json            # JSON output');
}

...
1. **Test Planning & Analysis** (10 mins)
   ...
   - Query existing code and architecture:
     ...
     npx hive-flow memory query ${memoryNamespace}_implementation
   ...
   - Store plan: \`npx hive-flow memory store ${memoryNamespace}_test_plan "Test strategy: Unit tests for domain logic, integration for APIs, e2e for workflows. Mocking: External services, database. Coverage target: 95%."\`
...
2. **Red Phase - Write Failing Tests** (20 mins)
   ...
   - Store status: \`npx hive-flow memory store ${memoryNamespace}_red_phase "Written: 25 unit tests (all failing), 10 integration tests (all failing), 5 e2e tests (all failing). Coverage: 0%."\`
...
3. **Green Phase - Minimal Implementation** (20 mins)
   ...
   - Store progress: \`npx hive-flow memory store ${memoryNamespace}_green_phase "Progress: 20/25 unit tests passing, 8/10 integration tests passing. Current coverage: 75%. Remaining: Complex edge cases."\`
...
4. **Refactor Phase - Optimize & Clean** (15 mins)
   ...
   - Store refactoring: \`npx hive-flow memory store ${memoryNamespace}_refactor "Extracted 3 common utilities, optimized database queries, improved test readability. All tests green. Coverage: 95%."\`
...
5. **Test Documentation & Validation** (10 mins)
   ...
   - Store completion: \`npx hive-flow memory store ${memoryNamespace}_tdd_complete "TDD cycle complete. Coverage: 95%. All acceptance criteria met. Tests documented. CI/CD ready."\`
...
- \`npx hive-flow sparc run debug "Investigate any failing edge cases" --non-interactive\`
- \`npx hive-flow sparc run refinement-optimization-mode "Optimize performance bottlenecks" --non-interactive\`
- \`npx hive-flow sparc run docs-writer "Create user documentation" --non-interactive\``;
}

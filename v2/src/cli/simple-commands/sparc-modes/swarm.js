...
const SWARM_MODE = {
  ...
  // Prerequisites
  prerequisites: [
    'Hive-Flow initialized with --sparc flag',
...
## 🐝 SWARM CAPABILITIES
You have access to the advanced Hive-Flow swarm system with the following capabilities:
...
npx hive-flow swarm "Build a complete REST API with authentication" --strategy development --monitor

# Research swarm with distributed coordination
npx hive-flow swarm "Research and analyze blockchain technologies" --strategy research --distributed --ui

# Background optimization swarm (for long-running tasks)
npx hive-flow swarm "Optimize application performance across all modules" --strategy optimization --background --monitor
\`\`\`

### Advanced Swarm Configuration:
\`\`\`bash
# Full-featured swarm with all enterprise features
npx hive-flow swarm "Complex enterprise application development" \\
...
npx hive-flow swarm "Your complex task" --dry-run --strategy development
\`\`\`
...
npx hive-flow memory store "key" "value" --namespace swarm
npx hive-flow memory query "search-term" --namespace swarm
\`\`\`
...
- Monitor progress with \`npx hive-flow status\`
...
npx hive-flow swarm "Build microservices e-commerce platform" \\
...
npx hive-flow swarm "Comprehensive AI/ML market analysis" \\
  --strategy research --distributed --ui --verbose

# 2. Store findings for cross-agent access
npx hive-flow memory store "research-findings" "key insights" --namespace swarm
\`\`\`

### Long-Running Optimization:
\`\`\`bash
# 1. Background optimization swarm
npx hive-flow swarm "Optimize entire codebase performance" \\
...
### Common Issues:
1. **Swarm fails to start**: Check if Hive-Flow is initialized with \`npx hive-flow init --sparc\`
...
4. **Environment Preparation**
   \`\`\`bash
   # Ensure Hive-Flow is properly initialized
   npx hive-flow status
...
5. **Dry Run Validation**
   ...
   npx hive-flow swarm "${taskDescription}" --dry-run --strategy <selected-strategy>
   \`\`\`
...
6. **Launch Swarm**
   ...
   **For Standard Tasks (< 30 minutes):**
   \`\`\`bash
   npx hive-flow swarm "${taskDescription}" --strategy <strategy> --monitor --verbose
   \`\`\`

   **For Complex Tasks (30-60 minutes):**
   \`\`\`bash
   npx hive-flow swarm "${taskDescription}" --strategy <strategy> --parallel --monitor --review
   \`\`\`

   **For Long-Running Tasks (> 60 minutes):**
   \`\`\`bash
   npx hive-flow swarm "${taskDescription}" --strategy <strategy> --background --monitor --testing
   \`\`\`

   **For Enterprise Tasks (Multiple hours):**
   \`\`\`bash
   npx hive-flow swarm "${taskDescription}" \\
     --strategy <strategy> \\
     --mode distributed \\
     --max-agents 10 \\
     --background \\
...
8. **Cross-Agent Memory Sharing**
   ...
   npx hive-flow memory store "${memoryNamespace}_findings" "key insights and decisions"
   
   # Store progress updates
   npx hive-flow memory store "${memoryNamespace}_progress" "current completion status"
   
   # Store blockers or issues
   npx hive-flow memory store "${memoryNamespace}_blockers" "any impediments or questions"
   
   # Query shared knowledge
   npx hive-flow memory query "${memoryNamespace}" --limit 10
   \`\`\`
...
10. **Memory Documentation**
    \`\`\`bash
    # Store final results
    npx hive-flow memory store "${memoryNamespace}_final" "completed task summary and results"
    
    # Store lessons learned
    npx hive-flow memory store "${memoryNamespace}_lessons" "insights for future similar tasks"
    \`\`\`

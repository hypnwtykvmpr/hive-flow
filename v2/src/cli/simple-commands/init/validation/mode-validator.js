...
export class ModeValidator {
  ...
  async checkSparcInitialization() {
    ...
    try {
      ...
      // Check for hive-flow executable
      try {
        const stat = await Deno.stat(`${this.workingDir}/hive-flow`);
        result.hasExecutable = stat.isFile;
      } catch {
        result.error = 'hive-flow executable not found';
      }
  ...
  async testModeAccess(modeName) {
    ...
    try {
      ...
      const command = new Deno.Command('./hive-flow', {
    ...
    try {
      ...
      if (success) {
        ...
        if (errorOutput.includes('dry-run') || errorOutput.includes('unknown flag')) {
          ...
          const testCommand = new Deno.Command('./hive-flow', {
            args: ['sparc', 'modes'],
            cwd: this.workingDir,
            stdout: 'piped',
            stderr: 'piped',

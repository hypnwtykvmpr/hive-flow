...
export function createWrapperScript(type = 'unix') {
  ...
  if (type === 'unix') {
    const universalTemplate = loadTemplate('hive-flow-universal');
  ...
  const filename =
    type === 'unix' ? 'hive-flow' : type === 'windows' ? 'hive-flow.bat' : 'hive-flow.ps1';
...
## Usage
\`\`\`bash
npx hive-flow analysis bottleneck-detect [options]
\`\`\`
...
npx hive-flow analysis bottleneck-detect

# Set custom threshold
npx hive-flow analysis bottleneck-detect --threshold 500

# Export results
npx hive-flow analysis bottleneck-detect --export bottlenecks.json
\`\`\`
`,
...
## Usage
\`\`\`bash
npx hive-flow analysis token-usage [options]
\`\`\`
...
npx hive-flow analysis token-usage --period 24h

# By agent breakdown
npx hive-flow analysis token-usage --by-agent

# Export detailed report
npx hive-flow analysis token-usage --period 7d --export tokens.csv
\`\`\`
`,
...
## Usage
\`\`\`bash
npx hive-flow automation auto-agent [options]
\`\`\`
...
npx hive-flow automation auto-agent --task "Build REST API"

# Limit agents
npx hive-flow automation auto-agent --task "Fix bugs" --max-agents 3

# Use specific strategy
npx hive-flow automation auto-agent --strategy specialized
\`\`\`
`,
...
# With agent limit
npx hive-flow swarm init --topology mesh --max-agents 8

# Parallel execution
npx hive-flow swarm init --strategy parallel
\`\`\`
`,
...
## Usage
\`\`\`bash
npx hive-flow task orchestrate [options]
\`\`\`
...
npx hive-flow task orchestrate --task "Implement user authentication"

# High priority task
npx hive-flow task orchestrate --task "Fix production bug" --priority critical

# With specific strategy
npx hive-flow task orchestrate --task "Refactor codebase" --strategy parallel
\`\`\`
`,
    },
    github: {
...
\`\`\`bash
# Create GitHub swarm
npx hive-flow github swarm --repository myorg/myrepo

# With specific focus
npx hive-flow github swarm --repository myorg/myrepo --focus security

# Custom agent count
npx hive-flow github swarm --repository myorg/myrepo --agents 6
\`\`\`
`,
...
\`\`\`bash
# Basic pre-task hook
npx hive-flow hook pre-task --description "Building API endpoints"

# With auto-spawn
npx hive-flow hook pre-task --description "Complex refactoring" --auto-spawn-agents

# Load context
npx hive-flow hook pre-task --description "Continue feature" --load-context
\`\`\`
`,
      'post-task': `# post-task
...
\`\`\`bash
# Basic post-task
npx hive-flow hook post-task --task-id task-123

# With performance analysis
npx hive-flow hook post-task --task-id task-123 --analyze-performance

# Update memory
npx hive-flow hook post-task --task-id task-123 --update-memory
\`\`\`
`,
      'pre-edit': `# pre-edit
...
\`\`\`bash
# Pre-edit hook
npx hive-flow hook pre-edit --file src/api.js

# With validation
npx hive-flow hook pre-edit --file src/api.js --validate-syntax

# Create backup
npx hive-flow hook pre-edit --file src/api.js --backup
\`\`\`
`,
      'post-edit': `# post-edit
...
\`\`\`bash
# Post-edit hook
npx hive-flow hook post-edit --file src/api.js

# Store in memory
npx hive-flow hook post-edit --file src/api.js --memory-key "api-changes"

# With formatting
npx hive-flow hook post-edit --file src/api.js --format
\`\`\`
`,
...
# Full closure
npx hive-flow hook session-end --export-metrics --generate-summary --persist-state
\`\`\`
`,
    },
    memory: {
...
## Usage
\`\`\`bash
npx hive-flow training pattern-learn [options]
\`\`\`
...
\`\`\`bash
# Learn from all ops
npx hive-flow training pattern-learn

# High success only
npx hive-flow training pattern-learn --threshold 0.9

# Save patterns
npx hive-flow training pattern-learn --save optimal-patterns
\`\`\`
`,
...
## Usage
\`\`\`bash
npx hive-flow hive-mind [subcommand] [options]
\`\`\`
...
npx hive-flow hive-mind init

# Spawn swarm
npx hive-flow hive-mind spawn "Build microservices"

# Check status
npx hive-flow hive-mind status
\`\`\`
`,
...
## Usage
\`\`\`bash
npx hive-flow hive-mind init [options]
\`\`\`
...
\`\`\`bash
npx hive-flow hive-mind init
npx hive-flow hive-mind init --force
\`\`\`
`,
...
\`\`\`bash
npx hive-flow swarm init --topology adaptive
\`\`\`
...
echo "   npx hive-flow swarm init --topology hierarchical"
echo ""
echo "2. Spawn agents:"
echo "   npx hive-flow agent spawn --type coder --name \"API Developer\""
echo ""
echo "3. Orchestrate tasks:"
echo "   npx hive-flow task orchestrate --task \"Build REST API\""
echo ""
echo "4. Monitor progress:"
echo "   npx hive-flow swarm monitor"
echo ""
...
(async () => {
  ...
  // Try multiple strategies to find hive-flow
  const strategies = [
    // 1. Local node_modules
    async () => {
      try {
        const localPath = resolve(process.cwd(), 'node_modules/.bin/hive-flow');
    ...
    async () => {
      try {
        const parentPath = resolve(process.cwd(), '../node_modules/.bin/hive-flow');
        const { existsSync } = await import('fs');
    ...
    async () => {
      return spawn('npx', ['hive-flow@2.0.0-alpha.25', ...process.argv.slice(2)], { stdio: 'inherit' });
    }
  ];
  ...
  console.error('Could not find hive-flow. Please install it with: npm install hive-flow');
  process.exit(1);
})();`;
...
1. Add MCP server: \`claude mcp add hive-flow npx hive-flow mcp start\`
2. Initialize swarm: \`mcp__hive-flow__swarm_init { topology: "hierarchical" }\`
3. Spawn agents: \`mcp__hive-flow__agent_spawn { type: "coder" }\`
4. Orchestrate: \`mcp__hive-flow__task_orchestrate { task: "Build feature" }\`

...
🎮 ENTERPRISE QUICK START:
  ...
  npx hive-flow@2.6.0-alpha.1 init --sparc

  # 🆕 Execute agents with multi-provider support
  ./hive-flow agent run coder "Build REST API with auth" --provider anthropic
  ./hive-flow agent run researcher "Research React 19" --provider openrouter  # 99% cost savings
  ./hive-flow agent agents  # List all 66+ available agents
...
`);
    ...
    config: async (key: string) => {
      try {
        const config = JSON.parse(await fs.readFile('hive-flow.config.json', 'utf-8'));
        if (key) {
  ...
  while (true) {
    ...
    const prompt = replState.currentSession
      ? `hive-flow:${replState.currentSession}> `
      : 'hive-flow> ';

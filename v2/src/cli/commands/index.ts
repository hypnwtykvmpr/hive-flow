...
export function setupCommands(cli: CLI): void {
  ...
  cli.command({
    ...
    action: async (ctx: CommandContext) => {
      try {
        ...
        await writeFile('memory/hive-flow-data.json', JSON.stringify(initialData, null, 2));
        ...
        console.log("2. Run 'npx hive-flow start' to begin the orchestration system");
        console.log("3. Use 'claude --dangerously-skip-permissions' for unattended operation");
        console.log('\nNote: Persistence database initialized at memory/hive-flow-data.json');
      } catch (err) {
  ...
  cli.command({
    ...
    action: async (ctx: CommandContext) => {
      success('Starting Hive-Flow orchestration system...');
        ...
        case 'workflow': {
          ...
          try {
            ...
            if (ctx.flags.execute) {
              ...
              info('To execute this workflow, ensure Hive-Flow is running');
            }
          } catch (err) {
      ...
      try {
        ...
        switch (subcommand) {
          ...
          case 'health':
            ...
            console.log(`  - hive-flow agent ${subcommand} ${ctx.args.slice(1).join(' ')}`);
          ...
          default: {
            ...
            console.log('');
            console.log('For detailed help, use: hive-flow agent <command> --help');
            break;
          }
        }
      } catch (err) {
  ...
  try {
    ...
    const enhancedStatusAction = async (ctx: CommandContext) => {
      ...
      console.log('For full enhanced functionality, use: hive-flow status [options]');
      console.log(
      ...
      try {
        ...
        success('Hive-Flow System Status:');
        ...
        if (options.watch) {
          ...
          console.log('Use: hive-flow status --watch');
        }
      } catch (err) {
      ...
      switch (subcommand) {
        case 'start': {
          ...
          try {
            ...
            if (!health.healthy) {
              warning("Orchestrator is not running. Start it first with 'hive-flow start'");
              return;
            }
            ...
            console.log(`📚 Use 'hive-flow mcp tools' to see all available tools`);
          } catch (err) {
        ...
        case 'stop': {
          try {
            ...
            if (!health.healthy) {
              ...
              warning(
                "MCP server runs as part of the orchestrator. Use 'hive-flow stop' to stop the entire system",
              );
            }
          } catch (err) {
            error(`Failed to check MCP server: ${(err as Error).message}`);
        ...
        case 'restart': {
          try {
            warning(
              "MCP server runs as part of the orchestrator. Use 'hive-flow stop' then 'hive-flow start' to restart the entire system",
            );
          } catch (err) {
      ...
      switch (subcommand) {
        case 'spawn': {
          ...
          try {
            ...
            // Build enhanced task with Hive-Flow guidance
            let enhancedTask = `# Hive-Flow Enhanced Task

## Your Task
${task}

## Hive-Flow System Context

You are running within the Hive-Flow orchestration system, which provides powerful features for complex task management:

### Available Features

1. **Memory Bank** (Always Available)
   - Store data: \`npx hive-flow memory store <key> <value>\` - Save important data, findings, or progress
   - Retrieve data: \`npx hive-flow memory query <key>\` - Access previously stored information
   - Check status: \`npx hive-flow status\` - View current system/task status
   - List agents: \`npx hive-flow agent list\` - See active agents
...
2. **Tool Access**
   - You have access to these tools: ${tools}`;
            ...
            if (ctx.flags.parallel) {
              enhancedTask += `
   - **Parallel Execution Enabled**: Use \`npx hive-flow agent spawn <type> --name <name>\` to spawn sub-agents
   - Create tasks: \`npx hive-flow task create <type> "<description>"\`
   - Assign tasks: \`npx hive-flow task assign <task-id> <agent-id>\`
...
1. **Before Starting**:
   - Check memory: \`npx hive-flow memory query previous_work\`
   - Check system status: \`npx hive-flow status\`
   - List active agents: \`npx hive-flow agent list\`
   - List active tasks: \`npx hive-flow task list\`

2. **During Execution**:
   - Store findings: \`npx hive-flow memory store findings "your data here"\`
   - Save checkpoints: \`npx hive-flow memory store progress_${task.replace(/\s+/g, '_')} "current status"\`
   ${ctx.flags.parallel ? '- Spawn agents: `npx hive-flow agent spawn researcher --name "research-agent"`' : ''}
   ${ctx.flags.parallel ? '- Create tasks: `npx hive-flow task create implementation "implement feature X"`' : ''}

3. **Best Practices**:
   - Use the Bash tool to run \`npx hive-flow\` commands
   ...
   ${ctx.flags.research ? '- Store research findings: `npx hive-flow memory store research_findings "data"`' : ''}
...
To interact with Hive-Flow, use the Bash tool:

\`\`\`bash
# Check for previous work
Bash("npx hive-flow memory query previous_work")

# Store your findings
Bash("npx hive-flow memory store analysis_results 'Found 3 critical issues...'")

# Check system status
Bash("npx hive-flow status")

# Create and assign tasks (when --parallel is enabled)
Bash("npx hive-flow task create research 'Research authentication methods'")
Bash("npx hive-flow agent spawn researcher --name auth-researcher")
\`\`\`
...
Now, please proceed with the task: ${task}`;
            ...
            const child = spawn(
              ...
              claudeCmd.slice(1).map((arg) => arg.replace(/^"|"$/g, '')),
              {
                env: {
                  ...
                  HIVE_FLOW_MODE: (ctx.flags.mode as string) || 'full',
                  HIVE_FLOW_COVERAGE: (ctx.flags.coverage || 80).toString(),
                  HIVE_FLOW_COMMIT: (ctx.flags.commit as string) || 'phase',
                  // Add Hive-Flow specific features
                  HIVE_FLOW_MEMORY_ENABLED: 'true',
                  HIVE_FLOW_MEMORY_NAMESPACE: 'default',
                  HIVE_FLOW_COORDINATION_ENABLED: ctx.flags.parallel ? 'true' : 'false',
  ...
  try {
    const enhancedMonitorAction = async (ctx: CommandContext) => {
      ...
      console.log('For full enhanced functionality, use: hive-flow monitor [options]');
      console.log(
      ...
      try {
        ...
        while (running) {
          try {
            ...
            // Enhanced header
            success('Hive-Flow Enhanced Live Monitor');
      ...
      action: async (ctx: CommandContext) => {
        ...
        try {
          ...
          while (running) {
            console.clear();
            const stats = await persist.getStats();
            success('Hive-Flow Live Monitor');
  ...
  try {
    const enhancedSessionAction = async (ctx: CommandContext) => {
      ...
      console.log('For full enhanced functionality, use: hive-flow session <command> [options]');
      console.log();
      ...
      if (subcommand) {
        console.log();
        console.log(
          `For detailed help on '${subcommand}', use: hive-flow session ${subcommand} --help`,
        );
      }
    };
  ...
  try {
    const enhancedStartAction = async (ctx: CommandContext) => {
      console.log(chalk.cyan('🧠 Enhanced Hive-Flow Orchestration System'));
      ...
      console.log();
      console.log('For full enhanced functionality, use: hive-flow start [options]');
      console.log(
  ...
  cli.command({
    ...
    action: (ctx: CommandContext) => {
      ...
      if (command === 'claude') {
        ...
        console.log(
          `  ${blue('hive-flow claude spawn')} "implement user authentication" --research --parallel`,
        );
        console.log(
          `  ${blue('hive-flow claude spawn')} "fix payment bug" --tools "View,Edit,Bash" --no-permissions`,
        );
        console.log(`  ${blue('hive-flow claude batch')} workflow.json --dry-run`);
        console.log();
        console.log(
        ...
        console.log('  hive-flow swarm <objective> [options]');
        console.log(
          '  hive-flow swarm-ui <objective> [options]  # Uses blessed UI (avoids TTY issues)',
        );
        console.log();
        ...
        console.log();
        console.log(bold('Examples:'));
        console.log(`  ${blue('hive-flow swarm')} "Build a REST API"`);
        console.log(`  ${blue('hive-flow swarm-ui')} "Build a REST API"  # Avoids TTY issues`);
        console.log(
          `  ${blue('hive-flow swarm')} "Research cloud architecture" --strategy research --research`,
        );
        console.log(
          `  ${blue('hive-flow swarm')} "Migrate app to microservices" --coordinator --review --ui`,
        );
        console.log();
        console.log(bold('TTY Issues?'));
        console.log("If you encounter 'Raw mode is not supported' errors, use:");
        console.log(`  - ${blue('hive-flow swarm-ui')} <objective>  # Recommended`);
        console.log(`  - ${blue('hive-flow swarm')} <objective> --ui`);
        console.log();
        ...
        console.log(bold('Examples:'));
        console.log(
          `  ${blue('hive-flow sparc modes')}                              # List all modes`,
        );
        console.log(
          `  ${blue('hive-flow sparc run code')} "implement user auth"      # Run specific mode`,
        );
        console.log(
          `  ${blue('hive-flow sparc tdd')} "payment processing system"    # Full TDD workflow`,
        );
        console.log(
          `  ${blue('hive-flow sparc workflow')} project-workflow.json     # Custom workflow`,
        );
        console.log();
        console.log(
        ...
        console.log();
        console.log(
          'Start the Hive-Flow orchestration system with comprehensive service management.',
        );
        console.log();
        console.log(bold('Usage:'));
        console.log('  hive-flow start [options]');
        console.log();
        console.log(bold('Options:'));
        ...
        console.log();
        console.log(bold('Examples:'));
        console.log(`  ${blue('hive-flow start')}                    # Interactive mode`);
        console.log(`  ${blue('hive-flow start --daemon')}           # Background daemon`);
        console.log(`  ${blue('hive-flow start --ui')}               # Process management UI`);
        console.log(`  ${blue('hive-flow start --health-check')}     # With pre-flight checks`);
      } else if (command === 'status') {
        console.log(bold(blue('Enhanced Status Command')));
        console.log();
        console.log('Show comprehensive Hive-Flow system status with detailed reporting.');
        console.log();
        console.log(bold('Usage:'));
        console.log('  hive-flow status [options]');
        console.log();
        console.log(bold('Options:'));
        ...
        console.log();
        console.log(bold('Examples:'));
        console.log(`  ${blue('hive-flow status')}                   # Basic status`);
        console.log(`  ${blue('hive-flow status --watch')}           # Live updates`);
        console.log(`  ${blue('hive-flow status --detailed')}        # Comprehensive info`);
        console.log(`  ${blue('hive-flow status --component mcp')}   # Specific component`);
        ...
        console.log();
        console.log(bold('Usage:'));
        console.log('  hive-flow monitor [options]');
        console.log();
        console.log(bold('Options:'));
        ...
        console.log();
        console.log(bold('Examples:'));
        console.log(`  ${blue('hive-flow monitor')}                  # Basic monitoring`);
        console.log(`  ${blue('hive-flow monitor --alerts')}         # With alerting`);
        console.log(`  ${blue('hive-flow monitor --focus mcp')}      # Component focus`);
        console.log(`  ${blue('hive-flow monitor --export data.json')} # Data export`);
        ...
        console.log();
        console.log(bold('Examples:'));
        console.log(`  ${blue('hive-flow session list')}             # List sessions`);
        console.log(`  ${blue('hive-flow session save mywork')}      # Save session`);
        console.log(`  ${blue('hive-flow session restore abc123')}   # Restore session`);
        console.log(`  ${blue('hive-flow session validate --fix')}   # Validate and fix`);
      } else {
        // Show general help with enhanced commands
        console.log(bold(blue('Hive-Flow Enhanced Orchestration System')));
        console.log();
        console.log('Available commands:');
        ...
        console.log(`  ${blue('hive-flow help <command>')}`);
        console.log();
        console.log('Enhanced features:');
  ...
  console.log();
  console.log('For detailed help on enhanced commands: hive-flow help <command>');
...
- \`npx hive-flow start\`: Start the orchestration system
- \`npx hive-flow --help\`: Show all available commands
...
- Use \`npx hive-flow status\` to check system health
- Monitor with \`npx hive-flow monitor\` for real-time updates
...
- \`npx hive-flow agent list\`: Show active agents
...
## Overview
The Hive-Flow coordination system manages multiple AI agents working together on complex tasks. It provides intelligent task distribution, resource management, and inter-agent communication.
...
\`\`\`bash
# Agent Management
npx hive-flow agent spawn <type> --name <name> --priority <1-10>
npx hive-flow agent list
npx hive-flow agent info <agent-id>
npx hive-flow agent terminate <agent-id>

# Task Management  
npx hive-flow task create <type> <description> --priority <1-10> --deps <task-ids>
npx hive-flow task list --verbose
npx hive-flow task status <task-id>
npx hive-flow task cancel <task-id>

# System Monitoring
npx hive-flow status --verbose
npx hive-flow monitor --interval 5000
\`\`\`

## Workflow Execution
Workflows are defined in JSON format and can orchestrate complex multi-agent operations:
\`\`\`bash
npx hive-flow workflow examples/research-workflow.json
npx hive-flow workflow examples/development-config.json --async
\`\`\`
...
## Configuration
Coordination settings in \`hive-flow.config.json\`:
\`\`\`json
{
  "orchestrator": {
...
## Troubleshooting
- Check agent health with \`npx hive-flow status\`
- View detailed logs with \`npx hive-flow monitor\`
...
## Purpose
This directory stores session-based memory data, conversation history, and contextual information for development sessions using the Hive-Flow orchestration system.

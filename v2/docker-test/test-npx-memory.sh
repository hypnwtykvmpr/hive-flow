...
echo "🐳 Testing hive-flow@alpha npx memory commands in Docker"
echo "============================================================"
echo ""

# Test 1: Version check
echo "📋 Test 1: Version Check"
echo "Command: npx hive-flow@alpha --version"
...
echo "Command: npx hive-flow@alpha memory store 'api-design' 'REST with JWT auth'"
npx hive-flow@alpha memory store "api-design" "REST with JWT auth" 2>&1 | tee /tmp/store-output.txt
echo ""
...
echo "Command: npx hive-flow@alpha memory query 'authentication'"
npx hive-flow@alpha memory query "authentication" 2>&1 | tee /tmp/query-output.txt
echo ""
...
echo "Command: npx hive-flow@alpha memory stats"
npx hive-flow@alpha memory stats 2>&1 | tee /tmp/stats-output.txt
echo ""
...
echo "Command: npx hive-flow@alpha memory list"
npx hive-flow@alpha memory list 2>&1

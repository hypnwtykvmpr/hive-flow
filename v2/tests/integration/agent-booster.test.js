...
describe('Agent Booster Integration Tests', () => {
  ...
  describe('CLI Commands', () => {
    it('should show help text', async () => {
      const { stdout } = await execAsync('npx hive-flow agent booster help');
  ...
  describe('Edit Command', () => {
    it('should edit a JavaScript file', async () => {
      ...
      const { stdout } = await execAsync(
        `npx hive-flow agent booster edit ${testFile} "Add error handling"`
      );
    ...
    it('should handle dry run mode', async () => {
      ...
      const { stdout } = await execAsync(
        `npx hive-flow agent booster edit ${testFile} "Add comments" --dry-run`
      );
    ...
    it('should show performance comparison with --benchmark', async () => {
      ...
      const { stdout } = await execAsync(
        `npx hive-flow agent booster edit ${testFile} "Add logging" --benchmark`
      );
    ...
    it('should fail with non-existent file', async () => {
      try {
        await execAsync(
          `npx hive-flow agent booster edit non-existent.js "Edit"`
        );
  ...
  describe('Batch Edit Command', () => {
    it('should batch edit multiple files', async () => {
      ...
      const { stdout } = await execAsync(
        `npx hive-flow agent booster batch "${TEST_DIR}/*.js" "Add comments"`
      );
    ...
    it('should handle empty pattern gracefully', async () => {
      const { stdout } = await execAsync(
        `npx hive-flow agent booster batch "${TEST_DIR}/no-match-*.js" "Edit"`
      );
  ...
  describe('Benchmark Command', () => {
    it('should run performance benchmark', async () => {
      const { stdout } = await execAsync(
        `npx hive-flow agent booster benchmark --iterations 10 --file ${TEST_DIR}/bench.js`
      );
    ...
    it('should support custom iteration count', async () => {
      const { stdout } = await execAsync(
        `npx hive-flow agent booster benchmark --iterations 5`
      );
  ...
  describe('Performance Validation', () => {
    it('should complete single edit in under 100ms', async () => {
      ...
      await execAsync(
        `npx hive-flow agent booster edit ${testFile} "Add comment"`
      );
    ...
    it('should process 100 files faster than 10 seconds', async () => {
      ...
      const startTime = Date.now();
      await execAsync(
        `npx hive-flow agent booster batch "${TEST_DIR}/*.js" "Add comment"`
      );
      const duration = Date.now() - startTime;
  ...
  describe('Language Detection', () => {
    ...
    it('should allow manual language override', async () => {
      ...
      const { stdout } = await execAsync(
        `npx hive-flow agent booster edit ${testFile} "Add comment" --language typescript`
      );
...
describe('Agent Booster vs LLM API Performance Claims', () => {
  it('should validate 352x speed claim is achievable', async () => {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      await execAsync(
        `npx hive-flow agent booster edit ${testFile} "Add comment ${i}"`
      );

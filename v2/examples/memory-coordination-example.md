...
const pluginTraits = mcp__hive-flow__memory_usage {
  action: "retrieve",
  key: "swarm/shared/plugin-traits",
  namespace: "coordination"
}
...
mcp__hive-flow__memory_usage {
  ...
  value: JSON.stringify({
    agent: "agent-3-mcp",
    status: "starting",
    tasks: ["Create MCP bridges", "Hive-Flow integration"],
    progress: 0
  })
}

// 2. Check dependencies from BOTH Agent 1 and 2
const traits = mcp__hive-flow__memory_usage {
  action: "retrieve",
  key: "swarm/shared/plugin-traits",
  namespace: "coordination"
}

const security = mcp__hive-flow__memory_usage {
  action: "retrieve",
  key: "swarm/shared/security-policies",
  namespace: "coordination"
}
...
npx hive-flow@alpha memory search "swarm/*/status" --namespace coordination

# Check progress
npx hive-flow@alpha memory search "swarm/*/progress" --namespace coordination  

# Find blocking issues
npx hive-flow@alpha memory search "swarm/*/waiting" --namespace coordination

# List shared components
npx hive-flow@alpha memory search "swarm/shared/*" --namespace coordination
```

...
This directory contains examples demonstrating various features and capabilities of the Hive Flow system, organized by category.
...
1. **New to Hive Flow?** Start with `01-configurations/basic-config.json`
...
### 01. Configurations
System and workflow configuration files showing how to set up Hive Flow for different use cases.
...
### 04. Testing
Scripts for testing Hive Flow features, SPARC modes, and system functionality.
...
For configuration examples, use them with Hive Flow commands:
```bash
cd examples
../hive-flow swarm create "Your task description" --config ./01-configurations/basic/simple-config.json
```

Or from the project root:
```bash
./hive-flow swarm create "Your task description" --config ./examples/01-configurations/basic/simple-config.json
```

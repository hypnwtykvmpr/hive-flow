# Hive Flow Automation Examples

This document provides practical examples of using Hive Flow's automation features.
...
```bash
hive-flow automation run-workflow web-app-workflow.json \
  --claude \
...
hive-flow automation run-workflow devops-workflow.json \
...
```bash
# Light workload
hive-flow automation mle-star --max-agents 3 --claude

# Heavy workload
hive-flow automation mle-star --max-agents 12 --claude
```
...
CMD ["hive-flow", "automation", "mle-star", \
  ...
  template:
    spec:
      ...
      - name: ml-trainer
        ...
        command: ["hive-flow", "automation", "mle-star"]
        args:
          - "--dataset"
          - "/data/dataset.csv"
          - "--non-interactive"
...
hive-flow automation run-workflow my-workflow.json \
  --output-format json \
...
```bash
# Development
hive-flow automation run-workflow workflow.json \
...
These examples demonstrate the full power and flexibility of Hive Flow's automation system. Start with simple examples and gradually build more complex workflows as you become familiar with the system.
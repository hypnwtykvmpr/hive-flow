...
echo "🧪 Hive Flow Memory System Unit Tests"
echo "======================================"
echo ""
...
run_test "Memory Store" \
    "../hive-flow memory store '$TEST_KEY' '$TEST_VALUE' 2>&1 | grep -q 'Stored successfully'" \
    ""

# Test 2: Query data
output=$(../hive-flow memory query "$TEST_KEY" 2>&1)
...
run_test "Memory List" \
    "../hive-flow memory list 2>&1 | grep -q 'default'" \
    ""

# Test 4: Update data
NEW_VALUE="Updated at $(date)"
run_test "Memory Update" \
    "../hive-flow memory store '$TEST_KEY' '$NEW_VALUE' 2>&1 | grep -q 'Stored successfully'" \
    ""
...
run_test "Memory Delete" \
    "../hive-flow memory clear --namespace default 2>&1 | grep -q -i 'clear'" \
    ""
...
for i in {1..5}; do
    ../hive-flow memory store "bulk_test_$i" "Bulk data $i" >/dev/null 2>&1
done

run_test "Bulk Store" \
    "[[ $(../hive-flow memory list | grep -c 'bulk_test_') -eq 5 ]]" \
    ""

# Test 9: Memory stats
run_test "Memory Stats" \
    "../hive-flow memory stats | grep -q 'Total entries'" \
    ""

# Test 10: Export/Import
run_test "Memory Export" \
    "../hive-flow memory export /tmp/memory_test.json" \
    ""
...
# Clean up bulk items
for i in {1..5}; do
    ../hive-flow memory delete "bulk_test_$i" >/dev/null 2>&1
done

...
echo "🔄 Hive Flow Workflow Integration Test"
echo "======================================="
echo ""
...
if ../hive-flow orchestrate "$WORKFLOW_FILE" --output "$OUTPUT_DIR" --quiet; then
...
../hive-flow memory store "$MEMORY_KEY" "Workflow test data" >/dev/null 2>&1
test_step "Memory storage during workflow" "../hive-flow memory query '$MEMORY_KEY' >/dev/null 2>&1"
...
if [[ -f "$PARALLEL_WORKFLOW" ]]; then
    ...
    ../hive-flow orchestrate "$PARALLEL_WORKFLOW" --output "$PARALLEL_OUTPUT" --monitor --quiet &
    PID=$!
...
if ! ../hive-flow orchestrate "$INVALID_WORKFLOW" --output "$OUTPUT_DIR/error" 2>/dev/null; then
...
sleep 2
TEMP_FILES=$(find /tmp -name "*hive-flow*$TEST_ID*" 2>/dev/null | wc -l)
...
rm -rf "$OUTPUT_DIR"
../hive-flow memory delete "$MEMORY_KEY" >/dev/null 2>&1

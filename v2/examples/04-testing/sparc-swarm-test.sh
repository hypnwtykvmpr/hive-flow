#!/bin/bash
# SPARC TDD Test Suite for Hive Flow Swarm CLI
# Tests all strategies and features systematically

echo "🔬 SPARC TDD Test Suite for Hive Flow Swarm"
echo "============================================="
echo ""
...
run_test "Development Strategy" \
    "npx hive-flow@latest swarm 'build a calculator app' --strategy development --dry-run --max-agents 4 2>&1 | grep -q 'Strategy: development'"

run_test "Research Strategy" \
    "npx hive-flow@latest swarm 'research AI trends' --strategy research --dry-run 2>&1 | grep -q 'Strategy: research'"

run_test "Analysis Strategy" \
    "npx hive-flow@latest swarm 'analyze codebase' --strategy analysis --dry-run 2>&1 | grep -q 'Strategy: analysis'"

run_test "Testing Strategy" \
    "npx hive-flow@latest swarm 'create test suite' --strategy testing --dry-run 2>&1 | grep -q 'Strategy: testing'"

run_test "Optimization Strategy" \
    "npx hive-flow@latest swarm 'optimize performance' --strategy optimization --dry-run 2>&1 | grep -q 'Strategy: optimization'"

# Mode Tests
echo -e "\n${YELLOW}🏗️  MODE TESTS${NC}"

run_test "Centralized Mode" \
    "npx hive-flow@latest swarm 'test task' --mode centralized --dry-run 2>&1 | grep -q 'Mode: centralized'"

run_test "Distributed Mode" \
    "npx hive-flow@latest swarm 'test task' --mode distributed --distributed --dry-run 2>&1 | grep -q 'Mode: distributed'"

run_test "Hierarchical Mode" \
    "npx hive-flow@latest swarm 'test task' --mode hierarchical --dry-run 2>&1 | grep -q 'Mode: hierarchical'"

# Feature Tests
echo -e "\n${YELLOW}⚡ FEATURE TESTS${NC}"

run_test "Parallel Execution" \
    "npx hive-flow@latest swarm 'parallel task' --parallel --dry-run 2>&1 | grep -q 'Parallel: true'"

run_test "Monitoring Feature" \
    "npx hive-flow@latest swarm 'monitored task' --monitor --dry-run 2>&1 | grep -q 'Monitoring: true'"

run_test "Quality Threshold" \
    "npx hive-flow@latest swarm 'quality task' --quality-threshold 0.95 --dry-run 2>&1 | grep -q 'Quality Threshold: 0.95'"

run_test "Memory Namespace" \
    "npx hive-flow@latest swarm 'memory task' --memory-namespace custom-space --dry-run 2>&1 | grep -q 'Memory Namespace: custom-space'"

run_test "Max Agents Configuration" \
    "npx hive-flow@latest swarm 'agent task' --max-agents 10 --dry-run 2>&1 | grep -q 'Max Agents: 10'"

# Advanced Configuration Tests
echo -e "\n${YELLOW}🎛️  ADVANCED CONFIGURATION TESTS${NC}"

run_test "Task Scheduling Options" \
    "npx hive-flow@latest swarm 'scheduled task' --task-scheduling round-robin --dry-run 2>&1 | grep -q 'Task Scheduling: round-robin'"

run_test "Load Balancing Options" \
    "npx hive-flow@latest swarm 'balanced task' --load-balancing random --dry-run 2>&1 | grep -q 'Load Balancing: random'"

# Help Test
echo -e "\n${YELLOW}📚 HELP TEST${NC}"

run_test "Help Command" \
    "npx hive-flow@latest swarm --help 2>&1 | grep -q 'Hive Flow Advanced Swarm System'"
...
#!/bin/bash
npx hive-flow@latest swarm "build a todo app with database" \
...
if [ $FAILED -eq 0 ]; then
    ...
    echo ""
    echo "The Hive Flow Swarm system successfully:"
...
echo ""
echo "  npx hive-flow@latest swarm \"create a REST API\" --strategy development"
echo ""

...
- **hello-world-workflow.json**: Single-agent starter workflow
  ```bash
  cd examples
  ../hive-flow swarm create "Build hello world app" --output ./output/hello-world
  ```

### Parallel Workflows (`parallel/`)
- **data-processing-workflow.json**: Process multiple data sources simultaneously
  ```bash
  ../hive-flow swarm create "Process CSV, JSON, and XML data in parallel" --agents 4 --output ./output/data-processing
  ```

### Sequential Workflows (`sequential/`)
- **blog-platform-workflow.json**: Step-by-step blog platform development
  ```bash
  ../hive-flow swarm create "Build complete blog platform with authentication" --strategy development --output ./output/blog
  ```
...
- **microservices-workflow.json**: Complete microservices architecture with 8 agents
  ...
  ```bash
  ../hive-flow swarm create "Build microservices e-commerce platform" --agents 8 --output ./output/microservices
  ```
...
- **machine-learning-workflow.json**: End-to-end ML pipeline
  ...
  ```bash
  ../hive-flow swarm create "Build machine learning pipeline for customer churn prediction" --strategy analysis --output ./output/ml-pipeline
  ```
...
```bash
# Execute a workflow
npx hive-flow orchestrate ./claude-workflow.json

# With monitoring
npx hive-flow orchestrate ./research-workflow.json --monitor

# In background
npx hive-flow orchestrate ./claude-workflow.json --background
```

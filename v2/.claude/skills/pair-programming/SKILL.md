...
```bash
# Check status
hive-flow pair --status

# View history
hive-flow pair --history
...
# End session
hive-flow pair --end [--save] [--report]
```
...
```bash
hive-flow pair --start --mode driver
```
...
hive-flow pair --start --mode switch

# 5-minute intervals (rapid)
hive-flow pair --start --mode switch --interval 5m

# 15-minute intervals (deep focus)
hive-flow pair --start --mode switch --interval 15m
```
...
#### Basic Configuration
Create `.hive-flow/pair-config.json`:
...
```bash
# Set configuration
hive-flow pair config set defaultMode switch
hive-flow pair config set verification.threshold 0.98

# Get configuration
hive-flow pair config get
hive-flow pair config get defaultMode

# Export/Import
hive-flow pair config export > config.json
hive-flow pair config import config.json

# Reset
hive-flow pair config reset
```
...
```bash
# Create profile
hive-flow pair profile create refactoring \
  --mode driver \
  --verify true \
  --threshold 0.98 \
  --focus refactor
...
# Use profile
hive-flow pair --start --profile refactoring

# List profiles
hive-flow pair profile list
```
...
hive-flow pair --template refactor
...
# Feature template
hive-flow pair --template feature
...
```bash
# Save session
hive-flow pair --save [--name <name>]

# Load session
hive-flow pair --load <session-id>

# Export session
hive-flow pair --export <session-id> [--format json|md]

# Generate report
hive-flow pair --report <session-id>
```
...
```bash
# Start in background
hive-flow pair --start --background

# Monitor background session
hive-flow pair --monitor

# Attach to background session
hive-flow pair --attach <session-id>

# End background session
hive-flow pair --end <session-id>
```
...
hive-flow pair --start --record

# Replay session
hive-flow pair --replay <session-id>

# Session analytics
hive-flow pair --analytics <session-id>
```
...
**With Git:**
```bash
hive-flow pair --start --git --auto-commit
```

**With CI/CD:**
```bash
hive-flow pair --start --ci --non-interactive
```

**With IDE:**
```bash
hive-flow pair --start --ide vscode
```
...
- Run `hive-flow pair config validate`
...
- `hive-flow pair --help` - Show help
- `hive-flow pair config` - Manage configuration
- `hive-flow pair profile` - Manage profiles
- `hive-flow pair templates` - List templates
- `hive-flow pair agents` - List available agents
